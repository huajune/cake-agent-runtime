...
`candidate-consultation` 场景注册 14 个叶子：两个静态手册实例（`base-manual`、
`final-check`），以及 `channel`、`stage-overview`、`red-lines`、`thresholds`、`identity`、
`memory`、`turn-hints`、`hard-constraints`、`datetime`、`group-inventory`、`stage-strategy`、
`critical-turn-guard`。
...
3. 动态尾部：`memory → turn-hints → hard-constraints → datetime → group-inventory → stage-strategy → critical-turn-guard`

`stage-overview` 展示全阶段地图且不读取 `currentStage`；`stage-strategy` 只展示当前阶段，因此留在动态
尾部。空 section 不产生 block。`critical-turn-guard` 是场景末位且按需出现；输入安全 guard 在
preparation 中插到它之前，主动跟进 directive 则追加到全部场景 block 之后。
...
主聊走 Qwen 隐式前缀缓存，当前代码没有显式缓存断点，也没有 provider 缓存适配层。静态前缀的
字节级测试会用不同时间、记忆和当前阶段输入对拍，保证其中不混入轮变化内容。
