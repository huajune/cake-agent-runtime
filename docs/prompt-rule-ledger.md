...
| 工具 | 字符数 | 内容轮廓 | 台账备注 |
|---|---|---|---|
| duliday_job_list | ~~13,144~~ **11,481**（2026-08-21 首批去重 -12.7%） | 召回策略/字段开关/展示要求/无岗动作链 | ✅ 首批去重完成：regionNameList 三遍教学合一、展示要求围绕四行卡片铁律收敛、福利追问并入通用补查、阶梯原文唯一居所化、保险社保回归全局规则（手册 T9+守卫）、同岗不重复介绍回归手册 G10/T3+守卫、badcase 叙事压缩为锚点 id。示例→枚举接口化重构仍为后续批 |
