...
17 条自检项（FC1 定位发送一致性、FC2 报名完成时态、FC3 未来预约保证、FC4 健康证岗位存在性、FC5 拉群承诺与两群区分、FC6 名额保留、FC7 距离数字接地、FC8 薪酬工时接地、FC9 组合排班两维、FC10 健康证一般询问、FC11 收资前薪资已告知、FC12 时间不合适先澄清、FC13 户籍门槛不改名泄漏、FC14 暂不报名即收口、FC15 内部表述、FC16 逐句查重、FC17 元规则）。
**行业对照裁定（2026-08-21）：final-check 位于末尾不碍前缀缓存，精简须按 recitation 视角，勿并入前缀。** 多数条目是"教侧"规则的完成时态复核面，与守卫拦截规则天然成对——精简时优先删与守卫硬规则完全同构的条目（候选：FC5 拉群完成时态 ↔ 守卫拉群假宣称；FC2 ↔ `booking_promise_without_booking`）。

## 四、DB red-lines（strategy_config.red_lines，Dashboard 可改零 review）

| # | 规则摘要 | 状态 |
|---|---|---|
| R1 | 咖啡→M Stand 默认 | ⚠️ **铁律违例②：与手册 B1 双居所重复**，手册版更全（含解除条件）。提案：删 R1，B1 为唯一居所 |
| R2 | 发薪/工资不确定只说"我帮你确认下" | ⚠️ **铁律违例⑤：与手册 T10/T11 口径冲突**——T10 明确"说帮你确认下就必须当轮转人工"；T11 已有 2026-08-06 合作模式新口径。R2 是旧口径残留。提案：删 R2 |
...
| R8 | "成都你六姐"是完整品牌名仅上海在招 | ⚠️ 时效性：随品牌开城变化，无 TTL 标注。提案：加"开城即失效"标注 |
| R9 | 禁跨城市推荐 | 唯一居所，保留 |
| R10 | 禁通融式推荐 | 与手册 C3 部分重叠（C3 更全）。观察项 |
| R11 | 地域筛选对外中性婉拒 | 与手册 T8 成对（T8 全量版）。可收敛为指针 |
| R12 | 禁 markdown 输出 | 唯一居所，保留 |
| R13 | 用工形式按字段如实介绍+不主动反问全兼 | ⚠️ **铁律违例①：与手册 F19+F21 几乎逐字重复**。提案：删 R13，手册为唯一居所 |
...
| 阶段 | 审计发现 |
|---|---|
| trust_building | disallowed"渠道来源身份否认"与手册 P1 逐字重复（**铁律违例④**，提案：改指针或删）；"首问地址"教学与手册 N8 呼应（各有侧重，保留） |
| qualify_candidate | 无重复，健康 |
| job_consultation | "无岗时 invite_to_group"与 job_list description 无岗动作链重叠（观察项）；包餐偏好重查是唯一居所 |
| interview_scheduling | ①"收资料直接发 templateText…以 missingFields 为字段范围"——提案同步 T1 对齐：字段范围改 `requiredFieldsToCollectNow`；②末条"2 轮时间硬冲突→handoff"与手册 T14② 逐字重复（**铁律违例③**，提案：删 DB 侧，T14 为唯一居所）；③"今天可以吗必须先 precheck"在 ctaStrategy 与 disallowedActions 各出现一次（段内自重复，可合并） |
| onboard_followup | 与 BK5 呼应（阶段管方向、BK 管动作），保留 |

> **DB 侧修改提案统一待 F5 裁定后执行**（Dashboard 改动约束，治理方案裁定 6"审完再议"）；本台账先登记现状与提案，不直接改生产配置。

## 六、工具 description（章节级登记）

| 工具 | 字符数 | 内容轮廓 | 台账备注 |
|---|---|---|---|
| duliday_job_list | 13,144 | 召回策略/字段开关/展示要求/无岗动作链/示例 | P3-3 主攻目标（占 41%）；示例→枚举语义化改造需 A/B |
| request_handoff | 4,388 | reasonCode 枚举语义/触发契约 | P3-3 次攻 |
| invite_to_group | 3,804 | 拉群时机/两轮协议/城市门 | 拉群时机簇 0820 新口径已入 |
