...
| duliday_cancel_work_order                      | 2,000                                        | 取消契约                                      | 与 BK2 成对；**2026-09-16 运营裁定：海绵工单状态滞后不可信，取消/改约不看 currentStatus / interviewPassTime**，B5-2 状态拦截与 `SELF_CANCEL_BLOCKED_STATUSES` 已拆；2026-09-08：+「系统无工单≠无预约——真人带外约面/候选人指认具体已约面试时 request_handoff(modify_appointment)，禁说没有预约记录」（badcase a9jahy31）；**2026-09-22**（PRD `prd-ops-followup-2026-09.md` R5.3；提交 ea25c716d）：接口/字典失败回执自带 general_handoff 副作用（`modify_appointment` + 工单号/岗位/阶段/失败原因），outcome 出口在投递后落底账/暂停/告警；删「说一句『我让同事帮你确认一下』并调 request_handoff」互斥指令，改为「如实告诉候选人这次取消暂时处理不了、已转同事跟进，本轮不要再调 request_handoff」；无工单号时仍回退 `request_handoff(modify_appointment)`。长期 |
