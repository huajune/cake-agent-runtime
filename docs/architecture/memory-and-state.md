...
| 产物落点 | 草稿纸（`ruleFacts` / 本轮系统疑似识别） | 作为 `extract_facts` 的辅助上下文，经证据与准入链后才可能进入 `sessionFacts` |
...
| 一回合 | 本轮系统疑似识别（`ruleFacts`） | 进程内存 | preparation（规则轨） | 提示词 hints、工具上下文 | 同上 |
...
- **本轮系统疑似识别与 turnState 分立**：一个是 prep 期的派生只读数据，一个是执行期的累积总线。合并无收益。**不动。**
