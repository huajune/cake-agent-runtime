...
- P1：确认问答裁决（`confirmation` 渠道，确定性模板+肯定词表）；带外事实回写（`human_oob`，MOBILE_PUSH 经理消息中的拒面/已面试 + 工单状态，复聊/提醒闸门消费）；删 `tools/shared/district-city-map.ts`（其自身 TODO）+ resolution/geo 行政区划库从 12 城白名单升级为按海绵在售岗位城市自动生成——业务开到哪，地理认知跟到哪。
- P2：长期画像继承降级标记全字段铺开；准入矩阵覆盖 booking 全字段（即 §1-§14 的 Claim 体系落地）。

### 16.4 发版后验证

- `invite.city_unverified` 周量（基线 156）应下降 >80%；
- `sessionFacts.pref.city` 出现 source='tool' 落库；
- 预填表单出处拦截：观察 [用户档案] 规则生效后"整表预填无披露"类 badcase 是否归零。
