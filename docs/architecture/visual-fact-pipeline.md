...
通道① 进查询参数（确定性，仅四类字段够格）
    brand / brand_id / store / candidate_address（含 map_location 定位点）
...
| A2 | **品牌ID** | `品牌ID[10006]`（我方平台截图自带） | 无人消费（白捡的确定性锚点） | **通道①**，直通 brandIdList，绕过别名歧义 |
...
| A6 | 发布方/招聘者信息 | 跃橙云服·祝东升、百胜咨询 | 进过品牌解析（发布方品牌劫持案）；**经理微信号进了真实报名（本 P0 案）** | **废弃**：不进任何档案与解析 |
...
| B2 | 健康证：有证/发证日期/从业范围 | 姓名毛梦港/2026-08-02/食品 | 无结构化消费，靠话术重问 | **通道③**，直接补齐收资字段少问一轮 |
