...
规则提示消费侧授权位于 `resolution/turn-hints/admission.ts`，不再由 visual 域拥有：
...
**规则轨授权域**（`resolveExtractionScope`，居所 `resolution/turn-hints/admission.ts`，由 rule-track producer 逐条消息调用）：
...
**LLM 轨配套门**（`memory/short-term/facts.service.ts`，memory 记录 dropped 观测）：phone 自陈出处门语料 = 手打 + 简历/证件 sheet 消息；explicit-upgrade 的 phone quote 只认手打文本；is_student/健康证话题证据门同语料收窄。
