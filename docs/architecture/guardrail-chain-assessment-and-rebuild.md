...
| 那本方案算什么 | **架构调整**（非参数调优、非推翻）。两处：① repair 权力模型重划——能改候选人文本的组件从两个收敛为一个（replan 拆解，§2）；② 体系补环——从单一投递前闸门变成事前防线 + 事后检测双环（§3/§4）。前三期的词表/白名单/提示词修补属参数调优，其天花板正是本次升级到架构层的依据（周二→周一案发生在全部提示层保护生效之后） |
| 真正的架构缺陷 | **只有事前环没有事后环**：67% 回合无人看、投递物无人复核、跨轮问题结构上不可见；外加 repair 的整段重写权（replan 重灾） |
| 落地风险分级 | 补环是纯增量离线改动，零热路径风险，**先行**；replan 拆解动投递热路径，须 feature flag / 新旧路径 shadow 对照 / 灰度切换，**后行**——且事后环产出的每日数据正是拆解效果的验收工具 |
