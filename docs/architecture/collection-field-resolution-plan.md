...
两条会话、同两位候选人（下文以化名王五/兮兮指代）**两人合报**岗位 528855（成都你六姐-上海陆家嘴软件园店-前厅服务员（夜宵）-小时工）。岗位配置 collect 型补充标签「有无本地健康证」（即 §8.0 全局字典 labelId 13，健康证族 392 岗重复面的头部标签）。
...
- chat `6a4229f2ce406a6aeea72c81`：候选人换入口重报。09:02 已明确回答「都有健康证，都是本地的健康证」，09:12 仍被再次追问；09:16 仅王五报名成功（工单 457339），兮兮失败且未被播报（§10.4）。
...
precheck 存档键为 `pc_<traceId>_<jobId>`，同轮两位候选人共用一个档位。兮兮的 booking 被「姓名或联系电话与预检结果不一致」一致性闸门拒绝，**失败未向用户播报**（Agent 只播报了王五成功）。09:21 用户重发兮兮资料，又被「近期已有预约工单」判重拦截（trace `batch_6a4229f2ce406a6aeea72c81_1786699295795`）——但 `ops_events` 中兮兮无任何 `booking.succeeded` 底账，该在途工单疑似真人经理带外手建，Agent 侧无从核实。两人合报是本组会话的真实业务形态，当前结构下第二人必然过不去一致性闸门。
