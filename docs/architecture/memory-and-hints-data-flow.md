...
- 给大模型：尽量完整展示事实和线索，但必须带 `confidence/source/evidence`。
- 给程序化工具：默认只消费 `confidence=high`；低/中置信字段只能作为模型参考或要求候选人确认。
- 写 Redis `sessionFacts`：通过回合结束的后置事实提取链路写入，不直接持久化 `onTurnStart` 的那个 `highConfidenceFacts` 对象。
- 写长期 `profile_facts`：通过 booking、settlement、enrichment 三条路径写入；settlement 只从已校验/清洗过的 `sessionFacts` 抽画像字段。
...
| 长期记忆 | `profile_facts` / `summary_data` | Supabase `agent_long_term_memories` + Redis 2h cache | 永久 | 跨 session 复用的画像和历史摘要 |
...
- 长期 `profile_facts` 读取后会完整保留 `confidence/source/evidence/updatedAt` 给模型。
...
| Prompt 段 | 来源 | 内容 |
|-----------|------|------|
| `[用户档案]` | 长期 `profile_facts` | 全字段值 + 置信度 + 来源 + 证据 + 更新时间 |
...
2. 会话记忆：来自 `memory.sessionMemory`，包含 `sessionFacts`、岗位池、当前焦点岗位等。
3. 活跃 recruitment case：如存在，会附加到记忆块中。

`sessionFacts` 和 `highConfidenceFacts` 渲染时使用统一字段列表，字段行会带内联 metadata，例如：
...
flowchart TD
  ...
  G --> H["LLM 结构化提取"]
  G --> I["规则高置信重跑"]
  H --> J["merge/sanitize/apply metadata"]
  I --> J
  J --> K["saveFacts deepMerge 到 Redis"]
```

`extract_facts` 是 `sessionFacts` 的主写入链路：

1. 从当前 normalized messages 拆出 `conversationHistory` 和 `currentMessage`。
2. 如果 Redis 已有 `facts`，只回看最近 `SESSION_EXTRACTION_INCREMENTAL_MESSAGES` 条历史。
3. 重新拉品牌表，重新跑规则/别名识别。
4. LLM 根据对话和规则线索输出结构化事实。
5. sanitizer 清理明显错误姓名等污染。
6. 规则高置信事实补位或标记 metadata。
7. `saveFacts()` 与旧 Redis state 深度合并。
...
flowchart TD
  ...
  C --> D["读取长期 summary_data.lastSettledMessageAt"]
  C --> E["读取 chat_messages 中 lastSettled 后的消息"]
  E --> F["寻找第一处 gap >= settlementGapSeconds"]
  F --> G["prevSessionMessages"]
  G --> H["LLM 生成 summary"]
  B --> H
  H --> I["appendSummary 到 summary_data.recent/archive"]
  B --> J["writeFromSettlement"]
  J --> K["从 sessionFacts.interview_info 抽身份字段"]
  K --> L["写 profile_facts source=extraction"]
```
...
| 输出 | 来源 | 写入位置 |
|------|------|----------|
| 对话摘要 | `chat_messages` 中待沉淀的旧消息片段 + `sessionFacts` 摘要辅助 | `summary_data.recent/archive` |
| 长期画像 | Redis `sessionFacts.interview_info` | `profile_facts` |

也就是说，summary 可以从聊天原文生成；profile_facts 不从聊天原文重新抽，而是从 sessionFacts 抽。
...
- 如果 Redis `sessionFacts` 已过期，summary 仍可基于 `chat_messages` 写入，但 `profile_facts` 不会从过期事实中恢复。
- `summary_data.lastSettledMessageAt` 是沉淀边界；冷启动时只初始化边界，不做历史全量沉淀。
- 一次 `detectAndSettle` 只沉淀第一段已闭合旧会话。

## 9. 长期画像写入路径

| 路径 | 写入方法 | confidence | source | evidence |
|------|----------|------------|--------|----------|
| 预约成功 | `LongTermService.writeFromBooking` | `high` | `booking` | `报名成功后写入` |
| 会话沉淀 | `LongTermService.writeFromSettlement` | `medium` | `extraction` | `会话沉淀提取；原字段来源=...` |
| 外部补全 | `LongTermService.saveProfile` | `medium` | `enrichment` | `外部补充字段写入长期档案` |

DB 端 RPC `upsert_long_term_profile_facts` 有置信度守卫：已有 `high` 字段不会被非 `high` 覆盖。
...
| 关注点 | 代码位置 |
|--------|----------|
| 本轮高置信识别 | [`src/memory/facts/high-confidence-facts.ts`](../../src/memory/facts/high-confidence-facts.ts) |
| fact wrapper 类型与 unwrap | [`src/memory/types/session-facts.types.ts`](../../src/memory/types/session-facts.types.ts) / [`src/memory/types/long-term.types.ts`](../../src/memory/types/long-term.types.ts) |
| 字段行渲染 | [`src/memory/formatters/fact-lines.formatter.ts`](../../src/memory/formatters/fact-lines.formatter.ts) |
| prompt 组装 | [`src/agent/context/context.service.ts`](../../src/agent/context/context.service.ts) |
| 本轮线索/待确认线索 | [`src/agent/context/sections/turn-hints.section.ts`](../../src/agent/context/sections/turn-hints.section.ts) |
| 查询硬约束/参考信息 | [`src/agent/context/sections/hard-constraints.section.ts`](../../src/agent/context/sections/hard-constraints.section.ts) |
| 工具上下文合并 | [`src/agent/agent-preparation.service.ts`](../../src/agent/agent-preparation.service.ts) |
...
会话事实 sessionFacts：慢一点、经过校验/清洗、Redis 持久到本 session
长期画像 profile_facts：从 booking / settlement / enrichment 写入 Supabase，字段级保留置信度和证据
```
