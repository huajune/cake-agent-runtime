...
> 状态：**v7（2026-07-21 观测可判定性修订）**——v6 三项改动部署首日实测，修补门禁分母的批次粒度缺陷、补齐误命中归因字段（§18.1）
> 前版：v6（2026-07-20 遗留项核实修订，`category_default` 裁定撤除、工种词护栏、门禁分母落库，遗留项汇总入 §18）；v5（2026-07-16 实施后修订，收录上线后 48h 内的实施偏离与事故修订，§17）；v4（2026-07-14 全量代码核查重评）；v3（2026-07-13 评审迭代）
...
> **遗留项一律登记 §18**（唯一权威清单，含代码核实状态）；§6.2/§13/§15.6/§16/§17 内的历史遗留描述以 §18 为准。
...
> ⚠️ **本条的「2% / 7 天」判据已于 2026-07-22 裁定作废，改按 §19 执行。** 上文保留为历史口径。作废理由见 §19.1——护栏类改动会让新旧**必然分歧**，比率度量的是护栏活跃度而非风险；且新路径自 v10.12.0 起就是生效路径，比率从来不是"切换开关"的风险指标。逐条归因要求**继续有效**。
...
1. **实施状态**：13 项验收条件全部达成；显式排除项（观测窗口/旧路径删除/生产数据操作）按约未做。旧匹配路径以 `legacyDetectBrandAliasHints` / `legacyNicknameBrandMatch` 形态保留为 shadow 对照组，随 §15.6 指标门下线（**该指标门已于 2026-07-22 作废，改按 §19 执行**）；
...
2. **品类偏好的跨轮记忆**：现状品类品牌经永久并集持久化、次轮起参与工具兜底；新设计品类不入状态，次轮起只能靠模型读对话历史自行延续。接受该退化，还是给品类偏好独立状态位（如 `currentCategory`）？（6.2）
   （v6 注记：`category_default` 方案已于 2026-07-20 **裁定撤除**，代码与 `category_default` 档位一并删除，品类词恢复全展开。
   v5 注记称该方案"尚未合入 develop"是失实的——它当天（07-16）已随 PR #576（标题为"加固事实护栏与人工接管链路"，与品类行为无关）合入，并随 v10.15.0 与"姐"热修同批上生产，裁定从未发生。
   生产代价：`category_expansion` 由 60/天跌至 0，8 天内 12 次品类命中全部收敛到 M Stand；其中 4 次是"咖啡师"工种词误触发（裸子串匹配无词边界），生产实例 6a5dbfa7 在候选人报名拉瓦萨途中把 `currentBrand` 改写成 M Stand。工种词护栏见 §7.3。
   流程教训：规格外方案不得搭无关 PR 合入；标注"待裁定"的方案在裁定发生前不应进入发版分支。）
3. **`excludedBrands` 的查询侧强制**：维持提示词软约束（Phase 4.4 后生效），还是在 `brandSource ∈ {session_state, none}` 的查询上追加确定性后过滤 + 结果披露？（8.1）

---

## 18. 未完成项汇总（v6，2026-07-20 逐项对代码核实）

本节是**唯一权威的遗留项清单**。此前遗留项散在 §6.2 / §13 / §15.6 / §16 / §17 五处、无一处汇总——v5 注记失实（§17 第 2 条）正是这种分散的直接后果。后续新增遗留项一律登记在此，不再散写。

**核实方法**：文档声称 → grep 代码实况，两者不符以代码为准。

| # | 项目 | 代码实况 | 状态 |
|---|---|---|---|
| 1 | 旧对照组下线（§15.6）| 两个 legacy 函数 + shadow 事件 + 计数器已物理删除 | ✅ 2026-07-22 完成（§19）|
| 6 | 品牌解析未剥引用块 | 剥离已收进 `detectBrandAliasHints` 入口 | ✅ 2026-07-22 完成（§19.2）|
| 2 | Prompt Section 迁移（Phase 4.4）| turn-hints / hard-constraints 已改读 `SessionBrandState` | ✅ 已完成 |
| 3 | `preferences.brands` 投影删除（Phase 4.4）| 投影已替换为读边界墓碑，全部读方迁 brand_state | ✅ 2026-07-22 完成（§19.6）|
| 4 | 遗留决策点（§16、§17）| 3 项中 2 项已裁定（`category_default` 撤除；§18.2 投影口径以"立即收口"落槌），1 项开放（`excludedBrands` 查询侧强制）| ⚠️ 1 项开放 |
| 7 | 懒迁移 `deriveLegacyLastBrand`（§9.4）| 已删除——迁移窗口经生产 Redis 实测数学耗尽 | ✅ 2026-07-22 完成（§19.6）|
| 5 | 受控模糊匹配决策（§6.2 line 252）| 所依赖的"线上未命中率"观测**从未采集** | ⚠️ 决策前提缺失 |

### 18.1 旧对照组下线：门禁状态（2026-07-20 生产数据；判据已于 07-22 作废，见 §19）

| 门禁条件 | 实测 | 达标 |
|---|---|---|
| 差异率 < 2% | 会话口径今日 3.6%、07-17 10.3%（07-15 1.3%、07-16 1.7% 达标）| ❌ |
| 且持续 7 天 | 有效观测仅 4 天（07-18/19 周末结构性停机，8 回合/天不构成观测）；07-17 超标已打断连续计数 | ❌ |
| 差异样本逐条归因 | 07-17 的 61 条未归因 | ❌ |

**分母可测性（v6 引入，v7 修补）**：一致计数原为 `SessionService` 内存字段、只打日志、重启归零，导致差异率的**正式分母根本算不出来**，门禁到期也无法判定。v6 起每满 100 次一致落一条 `brand_resolution_shadow_agreement` 事件，差异率 = diff 事件数 ÷ (diff 事件数 + Σ batchSize)，一条 SQL 可算。

v7 修补 v6 纯按批带来的三个缺陷（2026-07-21 首日观测实测暴露）：

| 缺陷 | 后果 | v7 修法 |
|---|---|---|
| 只按批落库 | 低流量日永远攒不满 100，当天分母恒为 0，差异率算出来是 NULL；周末（5–15 回合/天）结构性不可测 | 批满 **或** 距上次落库超 5 分钟，取先到者 |
| `batchSize` 写死常量 | 一旦允许不满批落库，分母直接虚报 | 落**实际计数** |
| `contact_name` 一致只 logger.log | 该 origin 的 diff 从来没有可配对的分母 | 同样走计数器落库 |

部署当日实测：分子覆盖全天、分母只覆盖部署后且只落了一批，事件级差异率虚高到 19.35%（会话级代理口径同日 4.14%）。**部署首日的事件级差异率不可直接采信**，需等分母覆盖完整自然日。

已知精度损失：进程重启丢失未满一批的计数（每实例 < 100）。v7 起 `onModuleDestroy` 会 drain 剩余计数，滚动发布不再吞当批；非正常退出仍会丢，只会让差异率显得偏高、不会漏报。

**归因字段（v7 新增）**：`brand_state_change.triggers` 补 `matchedText`（命中的品牌库词条）与 `sourceText`（用户原文片段，截断 80 字符）。此前事件只有 `matchType` + `canonicalName`，**脏别名塌缩与候选人真实简称在事件里长得一模一样**——2026-07-21 日检判定「成都你六姐」的 5 例 `alias_containment` 是真阳还是 §17 那类 P0，只能逐条回查 `chat_messages` 原文。这两个字段让误命中归因在事件表内自足。

**歧义档位留痕（v7 新增，`brand_resolution_ambiguous` 事件）**：ambiguous 结果（冲突别名如「小龙」→ 小龙坎/小龙翻大江）按设计不写状态，而 `brand_state_change` 仅状态变化时发射——**纯歧义轮此前整档零留痕**，「歧义发生了多少、候选人怎么说的」完全不可知（2026-07-21 发现）。新事件在 `applyTurnResolutions` / `applyLateImageResolutions` 入口无条件记录歧义现场（含 `matchedText` / `sourceText` / candidates），不依赖状态变化，过期丢弃路径也留痕。量级与冲突别名频率同阶（预计每天个位数）。

**注意归因优先于比率**：v6 的工种词护栏（§7.3）会让新旧在"咖啡师"类输入上必然分歧（新路径不认、旧路径误展开三个咖啡品牌），**差异率因此会上升**。这类样本归因为"新对旧错"，是旧路径**该下线的证据**而非阻塞理由。§15.6 原文即要求"不能只看比率达标"，按比率一刀切会得出与事实相反的结论。

### 18.2 `preferences.brands` 投影：口径冲突（已于 2026-07-22 裁定，执行记录见 §19.6）

§15.6 与 §17 第 4 条曾**互相矛盾**：

> §15.6：「收尾不执行完，本次改造不算完结，**不允许投影长期滞留**」
> §17 第 4 条：「删除时点待读方清点后定，**不设固定期限**，降格为跟踪项」

清点结果（3 个读方）：

| 读方 | 性质 |
|---|---|
| `hard-constraints.section.ts` `pref` 组装 | ⚠️ 同文件内新旧两套并存——品牌约束段已迁 `SessionBrandState`，`pref` 仍读旧投影 |
| `high-confidence-facts.ts`（highOnly / unwrap）| facts 序列化，机械迁移 |
| `brand-state.service.ts` 懒迁移 | **设计需要**——删投影前须先确认存量 Redis 已迁完（§9.4）|

即真实依赖只有第三条。前两条属可直接清理项。

### 18.3 其余开放项

- **差异率阈值与窗口**（2% / 7 天）：至今仍是"建议"，从未正式敲定；
- **品类偏好跨轮记忆**（`currentCategory`）：grep 无此字段，未实现。v6 撤除默认品牌后品类词走 `category_expansion`，按 §6.2 **不写会话主品牌**——跨轮完全不留痕迹、只能靠模型读对话历史。该退化已成既成事实，需补裁定；
- **`excludedBrands` 查询侧强制**：仍是提示词软约束。`brand-query.util.ts` 只在**品类展开**时减 `excludedBrands`，`brandSource ∈ {session_state, none}` 的无品牌查询无确定性后过滤；
- **受控模糊匹配**（§6.2）：`unmatched` 仅作为工具入参的 rejected reason 存在，无未命中率聚合观测。此项不是"待决策"，是**决策所依赖的数据从未采集**——要么补观测，要么承认无限期挂起。

---

## 19. 旧对照组下线裁定与执行计划（v8，2026-07-22）

### 19.1 裁定：按 §15.6「2% / 7 天」阻塞切换的判据作废

2026-07-22 日检数据：事件级差异率 `extraction_hints` **12.73%**（21/165）、`contact_name` 0.00%（0/14），会话级代理 4.17%。按 §15.6 字面判据，切换应被阻塞。**该判据经复核作废**，三条理由：

1. **比率已不度量风险，度量的是护栏活跃度。** 当日 21 条差异归因：9 条工种词护栏（`legacy=[10 个咖啡品牌] → next=[]`）、11 条新路径多识别（`legacy=[] → next=[品牌]`）、1 条加法差异（`[奥乐齐] → [奥乐齐,肯德基]`，同会话另有独立命中佐证）——**21/21 全为"新对旧错"**。护栏（§7.3 工种词、§7.3 时间语境数字）在设计上就要求新旧分歧，比率越高越说明旧路径该退休。§18.1 末段"归因优先于比率"已预告此结论，此处正式确立。
2. **切换不是行为翻转，行为增量为零。** `detectBrandAliasHintsWithShadow` 的契约（high-confidence-facts.ts 函数注释）写明"新路径 resolveBrands 是**生效路径**，旧匹配算法作为对照组在 shadow 运行"。新路径自 v10.12.0（07-15）起即驱动生产行为，旧路径只参与对比、不影响任何输出。所谓"切换"= 删除对照代码，**当日行为增量为零**，无回滚风险面。
3. **"7 个有效观测日"的目的已经达成。** 该条约束的实质目的是攒够新路径的行为证据；行为证据自 07-15 起持续积累（8 天全量生产流量），窗口重置影响的只是**分母埋点的年龄**（v6/v7 分别在 07-21 上线），不是新路径本身的曝光量。

**继续有效的部分**：差异样本逐条归因的要求不作废——本次切换前 27 条（近 24h 全量，无截断）已逐条归因完毕。

### 19.2 执行计划（2026-07-22 单批执行完毕）

初版计划把「观测 1 个完整工作日」作为删除前的闸门。复核后**取消该闸门**，四步合为一批：

- 剥引用块是确定性字符串处理，**回归用例的证据强于一天生产观测**；
- 该闸门的实际价值是"多留一天对照通道找未知分歧类"，而 8 天窗口只产出 1 个反例；
- 关键是 **Step 3 与 Step 2 同批发版**——排行口径放宽到全 `triggers` 后，引用型误匹配直接显形于日检（#619 已为 triggers 补 `sourceText`），检测能力在同一次发版内完成交接，**不存在盲窗**。

发版后的验证改为「发版后第一个完整工作日的日检确认无引用型命中」，不再阻塞删除。

| Step | 内容 | 状态 |
|---|---|---|
| 0 | 品牌解析剥引用块 | ✅ 已完成 |
| 1 | ~~观测 1 个完整工作日~~ → 降级为发版后验证 | 取消闸门 |
| 2 | 物理删除对照组（清单见 §19.3）| ✅ 已完成 |
| 3 | 监控口径调整（§19.4）| ✅ 已完成 |

**Step 0 —— 品牌解析未剥引用块（P1，切换前必修）**

`session.service.ts:693` 把**原始** `userMessages` 传给 `detectBrandAliasHintsWithShadow`，未过 `stripQuotedBlocks`；而该函数注释恰好点名此场景——「品牌解析同理：引用块里的品牌是经理的话」。同文件 line 356 的事实提取路径已剥，品牌路径漏了。

生产实例：chat `6a5f21efce406a6aee90a7f0`，2026-07-21 16:43。

```
用户: [引用 辛瑜琦：零售群是超市、便利店、门店导购这类]
      姐，这个群可以拉我一下嘛，我一块看看
→ legacy=[成都你六姐]  next=[7-11便利店, 成都你六姐]
```

「7-11便利店」来自**引用块中 Agent 自己说过的「便利店」**。Agent 高频列举品牌（"餐饮群主要是肯德基、必胜客、奶茶店这类"），候选人一引用回复，这些品牌即作为"候选人表达的兴趣"进入会话状态——**自污染回路**。与 2026-07-21「引用前缀经理名预填成候选人姓名」是同一结构类，一周内第二次。

**这不是 PR #619 修复的形态**：#619 的时间语境数字守卫（brand-matcher.ts `TEMPORAL_PREFIX_PATTERN` / `TEMPORAL_SUFFIX_PATTERN`）只管"晚上7-11点"→"711"的数字塌缩，与泛词包含匹配无关。07-22 日检初判"推定已修"系误判，已更正。

**实施口径**：剥离收在 `detectBrandAliasHints` **函数入口内**，而非修调用方。根因是"剥离契约只写在 `stripQuotedBlocks` 注释里、无结构约束"，改调用方只修了这一处、下一个调用方照样会漏。收进入口后调用方无法再漏；`stripQuotedBlocks` 幂等，`extractHighConfidenceFacts` 侧已剥过一次也无副作用。昵称入口（`contact_name`）已核实无需处理——昵称是备注名，不含引用块结构。

回归用例见 `tests/memory/high-confidence-facts.spec.ts`「引用块剥离」：引用块内品牌不产出线索、行首 `引用 XXX：` 形态同样剥离、正文品牌不受误伤（引用肯德基 + 正文瑞幸 → 只出瑞幸）、整条消息只有引用块时短路。

**这条为什么必须在切换前修**：该 bug 在**新路径**内，删不删对照组它都在。但暴露它的只有 `shadow_diff` 通道——每日观测第 1 项（包含匹配排行）按 `currentBrand` 取数，而本例中 7-11 未成为 `currentBrand`，**排行对它完全失明**。删掉对照组再修，等于先拆掉唯一的探针。另注：观测到 1 例只是**下限**——旧路径同样误匹配时不落 diff，该通道对这类是哑的。

### 19.3 删除清单（Step 2）

| 文件 | 删除内容 |
|---|---|
| `memory/facts/high-confidence-facts.ts` | `legacyDetectBrandAliasHints`、`detectBrandAliasHintsWithShadow`、`BrandAliasShadowDiff`；调用方改回 `detectBrandAliasHints` |
| `memory/services/session.service.ts` | `brandShadowAgreement` 字段、`emitBrandShadowAgreement`、`onModuleDestroy` drain、line 693–715 对比分支 |
| `memory/services/brand-state.service.ts` | `legacyNicknameBrandMatch`、`nicknameShadowAgreement`、`emitNicknameShadowAgreement`、`emitNicknameShadowDiff`、`onModuleDestroy` drain |
| `observability/shadow-agreement-counter.ts` | 整文件（仅此两处使用）+ 其 spec |
| `observability/observer.interface.ts` | `brand_resolution_shadow_diff`、`brand_resolution_shadow_agreement` 两个事件类型 |
| `observability/persisting-observer.ts` | 上述两个事件的持久化白名单条目 |
| `tests/memory/high-confidence-facts.spec.ts` | `detectBrandAliasHintsWithShadow` 相关用例 |

**不删**：`brand_state_change`、`brand_resolution_ambiguous` 是常设观测，与对照组无关。历史 `shadow_diff` / `shadow_agreement` 事件行保留在 `agent_execution_events` 内自然过期（30 天），不做数据清理。

### 19.4 切换后监控口径调整（Step 3）

`.claude/scheduled-tasks/brand-resolution-daily-watch` 需同步改造，否则每日巡检会持续报"门禁不达标、不可下线"，并继承 §19.2 指出的盲区：

1. **删除第 2、3 项**（差异事件取样、差异率门禁）——数据源已不存在；
2. **第 1 项口径放宽**：包含匹配排行从 `payload->'next'->'currentBrand'` 改为**展开全部 `triggers`** 统计。现口径只看最终生效品牌，对"品牌进了 triggers 但未成为 `currentBrand`"的误匹配失明——§19.2 那例正是如此。删掉 `shadow_diff` 后这是唯一探针，必须先补齐覆盖面；
3. **保留**第 1、4、5、6、7、8 项与生产库安全纪律；判定准则改引 §19.1。

### 19.5 遗留观测空白（不阻塞切换）

- **歧义档位零样本**：`brand_resolution_ambiguous`（v7 新增）上线首日 0 条。已核实埋点确已接通（`brand-state.service.ts:116` / `:173` 两处入口无条件调用，仅在无 ambiguous 结果时早退），0 即真无歧义输入。但该档位的下游行为在生产**从未被观测执行过**；旧路径无歧义档，故与切换无关，登记为独立跟踪项；
- **`contact_name` 分母薄**：14 次一致 / 0 差异，仅覆盖 #619 部署后不足一日。同样只是埋点年龄问题——该 origin 的新路径自 07-15 起即生效。

### 19.6 Phase 4.4 收尾：投影与懒迁移退役（2026-07-22 同日第二批）

§19.3 的对照组删除随 v10.24.0 上线（17:11 发布，17:16 起 shadow 事件归零、`brand_state_change` 正常）后，用户裁定**当日完成全链路迁移**——§18.2 的口径冲突以 §15.6「不允许投影长期滞留」落槌。

**前提核实（生产 Redis 实测，只读）**：`production:factsv2:*` 共 889 个会话——850 个已有 `brand_state`（95.6%）、37 个连 facts 都没有（懒迁移无从触发）、仅 2 个「有 facts 无 brand_state」；其中 1 个 `brands:[]` 无值可迁，**真正可迁的只剩 1 个且 TTL 剩 <17h**。sessionTtl=3 天且随写续期，brand_state 上线已 7 天——迁移窗口数学耗尽，懒迁移已是死代码。放弃该 1 例的品牌种子（必胜客，medium 置信）为已接受的损失。

**变更清单**：

| 项 | 处置 |
|---|---|
| 懒迁移 `deriveLegacyLastBrand` + reducer `legacyLastBrand` 入参 + 两 API 的 `facts` 入参 | 删除 |
| `applyBrandProjection` | 替换为**读边界墓碑** `retireBrandsField`：读出时恒置 `preferences.brands=null`。不是换个投影——deepMerge「null 不覆盖」会让收口前的旧值在长活跃会话（如 6a1e42a5）无限存续，逐读方防御不如一处截断 |
| hard-constraints `pref.brands` 组装 | 置 null（软提示行本就直读 `sessionBrandState`）|
| turn-hints 品牌 partition 分支 | 删除（收口后 `highPref.brands` 恒 null，partition 早退，实为死代码）|
| fact-lines 意向品牌行 | 改由 `options.currentBrandName` 显式注入；memory.service（主 prompt/复聊共用 recall）与 reply-repair 传 `brand_state.currentBrand`；提取 prompt 两处不传（品牌上下文对提取无用，brand_intents 是本轮信号）|
| settlement 长期意向品牌快照 | 源改 `brand_state.currentBrand`（经 `SettlementFactOrigin.brandState` 三层下传）；旧存储残留不再沉淀。长期记忆里既有 brands 数据不动（历史层自带"以本次为准"披露）|
| test-suite memory-fixture | 夹具 brands 按「末位≈最近」种成 `brand_state`（新增 `seedFixtureBrandState` 测试专用直写，生产写入仍只经 reducer）|
| 提取 prompt | 删 `brands` 输出指令（写通道已死，白耗 tokens）；顺带清除 position 条目里「咖啡默认 M Stand」过时事实（该默认档 07-20 已撤除，指令残留是真隐患）|
| schema `preferences.brands` 字段 | **保留**，仅作旧存储解析兼容；任何读写复活均为违规 |

**验证**：tsc 干净；全量 jest 365 套件 / 5532 用例全过；改动文件 lint/format 干净。
