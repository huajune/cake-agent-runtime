...
> 状态：Draft v4（2026-07-14 全量代码核查重评：品类展开赦免收紧 / 异步补写时序防护 / 过渡期旧兜底门控 / 写入点与守卫切换点清单补全 / 观测落点修正 / 现状口径校正），待复审  
> 前版：v3（2026-07-13 评审迭代：极性收敛 3 值 / 品牌逻辑单一居所 / LLM 触点与时序核实 / 图片双路径修正 / filterMode 两层拆分 / 兜底边界原则 / 昵称品牌 seed 化）  
...
轻量计数类观测（10.3 的“图片无描述”漏调率、“补写过期丢弃”数）不新增事件类型，走日志聚合或现有 step 统计。Logger 仅作辅助排障。日志与事件都不得把“模型传入的别名”与“工具实际应用的品牌”混为同一字段。
