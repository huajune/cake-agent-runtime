...
> 状态：**v6（2026-07-20 遗留项核实修订）**——`category_default` 裁定撤除、工种词护栏、门禁分母落库；遗留项逐项对代码核实后汇总入 **§18**
> 前版：v5（2026-07-16 实施后修订，收录上线后 48h 内的实施偏离与事故修订，§17）；v4（2026-07-14 全量代码核查重评）；v3（2026-07-13 评审迭代）
...
**分母可测性（v6 修复）**：一致计数原为 `SessionService` 内存字段、只打日志、重启归零，导致差异率的**正式分母根本算不出来**，门禁到期也无法判定。v6 起每满 100 次一致落一条 `brand_resolution_shadow_agreement` 事件，差异率 = diff 事件数 ÷ (diff 事件数 + Σ batchSize)，一条 SQL 可算。已知精度损失：进程重启丢失未满一批的计数（每实例 < 100），只会让差异率显得偏高、不会漏报。
