...
> 状态：Draft v3（2026-07-13 评审迭代：极性收敛 3 值 / 品牌逻辑单一居所 / LLM 触点与时序核实 / 图片双路径修正 / filterMode 两层拆分 / 兜底边界原则 / 昵称品牌 seed 化），待复审  
> 分支：`codex/brand-resolution`  
> 本文只描述方案，不代表相关代码已经实现。
...
当前企微链路中的 `contactName` 表示候选人的微信昵称。项目内没有独立的 `contact_remark` 字段（现存代码中的 `contact_remark` 命名是历史误会，Phase 2.4 修正）。
...
| `tools/duliday/job-list/search.util.ts` | `filterJobsToRequestedBrands` 私有包含匹配（`normalizeKeyword` + 双向 includes 策略） | 入口标准化（8.2）后过滤条件是品牌 ID/标准名，本地过滤退化为**等值比较**；私有包含匹配策略废弃，不迁移 |
| `tools/utils/sanitize-brand-name.util.ts` | 回复文本品牌错名修正 | 迁入 `sanitize-brand-name.ts`（逻辑不变，只换居所） |
...
└── sanitize-brand-name.ts         # 迁移：回复文本品牌错名修正（自 tools/utils/sanitize-brand-name.util.ts）
...
【turn-finalizer / memory.onTurnEnd】统一副作用出口    ← 锚点二
  ...
  复用回合上下文中的图片品牌解析结果    缺描述→异步补写（10.3）
  汇总本轮全部结果 → reducer 批量应用（先 positive 后 negative，
                     排在 extract_facts 之后，同一串行收尾序列）
...
- 作用范围：仅当轮查询扩展（多品牌 enforce 召回），不写入会话主品牌（见 9.3）；
- 生效来源：`user_text` 与 `image_description`；`contact_name` 不做品类展开（“咖啡爱好者”不是品牌意向）；
- 该能力承接现有 `detectBrandAliasHints` 内的品类兜底（迁移映射见 5.1），行为与线上一致，已上线的咖啡品类召回不回归。
...
1. **确定性规则**处理高置信模式，维护有限清单：“不要X”“除了X（都行/都可以）”“品牌不限/都行/随便”“换个品牌/换一家”等。规则命中即定极性；
...
- LLM 轨在 **onTurnEnd** 的串行收尾序列运行（`extract_facts` 步骤，带缓存/跳过/降级），其极性与指代链接产出正好赶在品牌状态写入之前——**reducer 的应用必须排在 extract_facts 步骤之后**（同一串行序列内追加一步即可）；
...
| 事实提取 LLM | memory `extractFacts` | onTurnEnd 收尾序列（回复后、品牌状态写入前；可跳过/可降级） | 极性判断 + 指代链接（6.3.1） | 不创造品牌名：输出必须过目录验证才进 reducer | 只剩规则轨，默认 positive |
...
**建模原则：模式只描述查询形态，品牌来源单独记录。** 早期草案曾设 `inherit` 模式表示“沿用会话品牌”，评审裁定废除——兜底后实际执行的就是一个 enforce 查询，“沿用”不是查询形态，而是**品牌来源**；把来源编码成模式是维度混淆。生产代码已有正确先例（`brandAliasSource: 'input' | 'contact_remark' | 'session_facts'`），本方案将其扶正为 queryMeta 的 `brandSource` 字段。
...
memory 侧的调用形状（onTurnEnd 收尾序列内、排在 extract_facts 之后追加一步，全程在已持有的租约锁内）：
...
现状 `preferences.brands` 有两个写入方：LLM 事实提取抽出的品牌、规则匹配（`detectBrandAliasHints` → `mergeDetectedBrands`）命中的品牌，都直接并入该数组。品牌真相迁到 `SessionBrandState` 后，若 LLM 链路继续写旧数组、规则链路写新状态，系统会同时存在两份内容不一致的品牌数据（Prompt 读旧、工具读新），必须杜绝。
...
- 多品牌表达（“肯德基和麦当劳都可以”）或品类展开 → 拆两半处理：**不立主品牌**（候选人没说更想去哪个，系统不替他挑，`currentBrand` 不动，当轮按多品牌查询——注意多品牌查询由主 Agent 看原文传参执行，不经过状态机：reducer 跑在 turn-finalizer，此时本轮查询早已发生，见 5.3 贯穿性规则 3）；**但解除排斥照常**（若肯德基此前在 `excludedBrands`，现在他说“都可以”，黑名单必须移除——否则下轮兜底跳过肯德基、排除过滤筛掉肯德基岗位，系统行为与他刚说的话矛盾）。原则：正向表达一律解除该品牌的排斥，无论是否成为主品牌。

单/多品牌表达的判别规则（reducer 的实现判据）：**同一来源的 positive 结果 ≥2 条，或任一结果 `matchType='category_expansion'`，即为多品牌表达**；同来源恰 1 条 positive 即为单品牌表达（“肯德基不要，麦当劳可以”的 user_text 正向只有麦当劳 1 条，正常替换）。跨来源不算多品牌——图片 positive(A) + 文字 positive(B) 按“图片先、文字后”逐来源各自应用替换，文字赢。实现上两个操作解耦：解除排斥对每个 positive 品牌**无条件**执行，立主品牌仅单品牌表达时执行。边界行为：debounce 把“肯德基呢”“麦当劳也看看”两条消息合并成一轮时，合并文本解析出 2 条 positive，按此规则不立主品牌——这是刻意的保守选择，与本节“同轮内先后顺序不可信”的论证一致。
...
- **初始化优先级**：旧 `preferences.brands` 末位品牌 > 已验证昵称品牌 seed > 空状态。旧数组末位是对话表达（时点晚于加好友的昵称），优先；两者皆无则空。旧数组其余品牌直接丢弃（无极性无时序，不值得继承）。依据：旧数组按 append 顺序构建（`mergeDetectedBrands` 为 `[...existing, ...detected]`），末位 ≈ 最近表达的品牌；若一个都不继承，存量候选人在模型忘传品牌的轮次会失去会话品牌兜底，badcase recvjFFKcZPsiC（想找大米先生被跨品牌推荐）将在存量会话整体复发；继承的品牌即使恰为过期/误判，替换式状态机也保证一轮内被真实表达纠正；
...
主路径的描述回写靠工具提示词驱动（“你必须调用”），模型可能忘调——该图品牌将永远进不了状态。兜底：turn-finalizer 检测本轮图片消息缺描述时，触发一次异步 Vision 补写（复用 `ImageDescriptionService`），补写完成后走同一条 `resolve` → reducer 链路；并对“图片无描述”落观测计数，持续监控模型漏调率。

### 10.4 实现约束

- **图片描述消息的识别必须结构化**：区分“图片描述”与“用户文字”依赖消息存储中的消息类型标识（storage message kind），不得靠 `[图片]` 前缀字符串猜；
...
**只新增一个事件：`brand_state_change`**——会话品牌状态迁移的前后快照 + 触发它的解析结果（含来源/匹配方式/极性）。仅在状态实际变化时写入，多数轮次零行。它是品牌链路上唯一**不可重放**的信息（状态迁移依赖前态），并承担 `historicalBrands` 删除后的历史回放职责。

刻意不落的两类（遵循事件表“只存不可再得的结构化事实、不当日志表”的既有纪律）：

- 解析结果不单独落事件：`resolve()` 是纯函数，误判归因时拿候选人原文 + 品牌目录**离线重放**即可精确复现，可重放的信息不在线记录；
...
Logger 仅作辅助排障。日志与事件都不得把“模型传入的别名”与“工具实际应用的品牌”混为同一字段。
...
1. 用户文字、微信昵称在回合准备阶段调用 BrandResolution；图片描述的解析与状态更新挂 turn-finalizer（多模态主路径下描述是回合内工具产物，准备阶段不存在，见 10.2），并实现描述缺失的异步 Vision 补写兜底（10.3）；
2. 引入新的会话品牌状态：迁移规则实现为 `brand-state.reducer.ts` 纯 reducer（见 5.2），memory 侧持锁读 facts hash 单字段 `brand_state` → 调 reducer → 写回（见 9.1）；
3. 停止 `preferences.brands` 永久并集，LLM 事实提取的品牌输出经状态机统一写入，旧字段改为只读投影（见 9.2）；
4. 修正当前代码中将 `contactName` 结果命名为 `contact_remark` 的错误；
...
3. 优先转成 `brandIdList`；
4. 记录 applied/rejected 品牌元数据；现有工具 details 里的 `brandAliasSource` 局部变量被 `queryMeta.brandSource` 吸收替代（`input→model_input`；`contact_remark` 档**废除**——昵称品牌经 seed 进入 `currentBrand`，查询侧统一表现为 `session_state`，2.4 的错误命名随档位一并消失；`session_facts→session_state`，读取点同步从旧并集数组换成 `SessionBrandState.currentBrand`）——注意不是纯改名：现状兜底无条件触发，新设计受 mode 门控（`clear`/`browse_all` 旁路）；
5. 删除工具内的昵称品牌兜底逻辑分叉（含未经品牌库验证的部分）——昵称品牌统一在首轮准备阶段经 BrandResolution 验证后 seed 进状态，工具侧不再存在昵称档；
...
8. 入口标准化落地后，`filterJobsToRequestedBrands` 的本地过滤从私有包含匹配退化为品牌 ID/标准名**等值比较**，废弃 `normalizeKeyword` + 双向 includes 策略。

### Phase 4：守卫和观测

1. 出站守卫改读标准化查询元数据（含 `fuzzySuggestions` 承接同音回指规则，见第 11 节切换映射）；守卫私有的 `normalizeClaimedBrand` 改 import `brand-normalize.ts`；
2. `brand_state_change` 事件落 `agent_execution_events`（唯一新增事件，见第 12 节；解析可离线重放、queryMeta 已随流水落库，均不落事件）；
3. 建立线上误判和漏判回归集；
...
2. 新旧结果并行记录一段时间，对比品牌命中差异；
3. 工具先记录标准化结果，再切换为以标准化结果执行查询；
4. 守卫最后切换数据源，避免工具尚未完整提供 queryMeta 时产生误拦截；
5. 线上重点观察未命中率、冲突率、昵称品牌采用率、品牌过滤后零结果率和跨品牌回复率；
6. **旧匹配路径下线以观测指标为准而非固定时间**：旧的局部品牌别名判断是新旧并行对比的对照组，须待新旧命中差异率低于阈值（建议 2%，待定）且持续 7 天后才物理删除，差异样本必须逐条归因（新对旧错 / 旧对新错 / 语义歧义），不能只看比率达标。注意该指标门**只管旧匹配路径的删除**：Prompt Section 迁移与投影删除（Phase 4.4）、`contact_remark` 命名修正（Phase 2.4）都在主线内直接执行，不等观测窗口。收尾不执行完，本次改造不算完结，不允许投影长期滞留。
