...
│   ├── conversation-test.service.ts # 回归验证服务
...
| **结果处理** | 回写飞书 | 前端展示 + 回写飞书 |
| **Supabase 表** | test_executions | conversation_sources + test_executions |
...
**用例测试流程**:
```
飞书 testSuite 表 → 导入 → 执行 Agent → 人工评审 → 回写飞书
```

**回归验证流程**:
```
飞书 validationSet 表 → 解析 → 拆分轮次 → 执行 Agent → LLM 评估 → 回写飞书
```
...
| **ConversationTestService** | 回归验证 | parseConversation, executeConversation | LlmEvaluationService |
