...
| 路径 | 规则 | 处理 |
|------|------|------|
| 代码直接修 | `brand_name_violation` 的平台名别名、`district_level_distance_claim` | transform 后复检，干净则直接投递；`brand_name_violation` 中岗位品牌错配无法 sanitize 时回落重写 |
...
- transform 是否生效：`reason_code='transformed'`。
