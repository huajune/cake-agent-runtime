...
> 状态：总设计（方向已定；HC 硬约束的实现级机制见子文档，落地进度见路线图）。架构评审 2026-06-24。
> 日期：2026-06-23
...
| `runner` | 编排一个回合（generator → guardrail → revise） | OpenAI `Runner` |
| `reengagement` | 主动触发调度（何时发起一个回合） | 业务/增长域术语（框架无对应，均为被动） |

复聊（二次触达）= `reengagement` 触发 → 复用 `runner` → 天然继承全部 guardrail，不另起发送系统。
...
```
processMessageCore (L190)
  → preAgentRiskIntercept.precheck            前置高危关键词同步暂停+告警（不短路）
  → ensureVisionDescriptionsReady             等图片 vision 描述回写
  → callAgent (L514) ── runner.invoke ──┐
  │                                     └─ AI SDK generate(tools, stopWhen)
  │                                          工具在 loop 内执行副作用（booking 同步调 sponge）
  │                                          stopWhen: skip_reply / request_handoff 短路
  │     ReplyFactGuard.check (L611)     确定性出站守卫 → blockedByGuard
  → [replay] 生成期间有新消息则丢弃重跑一次（deferTurnEnd 保护记忆）
  → isSkipped || blockedByGuard ? 跳过发送 : deliveryService.deliverReply (L415)
...
- **守卫已存在**：[ReplyFactGuardService](../../../src/agent/guardrail/output/hard-rules.service.ts) `check()`（L193+），现有规则 `discriminatory_screening_leak`(block)、`group_promise_without_invite`、`salary_fabrication`、`booking_form_field_mismatch`；确定性，非 LLM；调用点 reply-workflow L611。另有 `InputGuardService`（生成前输入守卫）。**——这两者本质就是 input/output guardrail。**
...
- `generator`：生成 agent。即现 `AgentRunnerService` 正名（OpenAI 称 `Agent`；它本就是"生成方/会动手的 worker"）。
- `guardrail`：OpenAI 的 input/tool/output guardrails。现 `InputGuardService` = input、`ReplyFactGuard` = output(rule)，本设计补 output(llm) + tool(booking)。
- `runner`：OpenAI 的 `Runner`——"manages turns, tools, guardrails, handoffs, sessions"，即"编排一个回合"。把 `runner` 一词从生成层释放给本层（生成层正名 `generator`）。
...
```
guardrail/
  ├─ input/    InputGuardrailService    生成前拦危险输入（现 InputGuardService 归入）
  ├─ tool/     BookingGuardrailService  副作用工具执行前的权威门禁（解 P1 自证）
  └─ output/   OutputGuardrailService   回复生成后审查
        ├─ rule  RuleGuardrail   确定性（现 ReplyFactGuard 归入；补"兼职提保险""回名字"规则）
        └─ llm   LlmReviewer     LLM 审查（ModelRole.Review / qwen3.7-plus，只读）
```

#### guardrail 收紧：detect / decide / act 分层 + 统一目录

现状：guard 类逻辑散落在 `agent/input-guard`、`wecom/reply-fact-guard`、`wecom/pre-agent-risk-intercept`、`tools/duliday/*`（booking-guards/age/sensitive-screening/hard-requirements）、`conversation-risk/*`、`biz/strategy` 等。收紧的前提是认清 **guardrail 是「决策层」，别把它周围的层一起吸进来**：

```
检测/信号(detect) ──→ 判定/否决(GUARDRAIL · decide) ──→ 执行/告警(act)
conversation-risk      agent/guardrail/ (input/tool/output)   intervention(pause托管)
strategy red_lines     ↑ 统一契约 + 统一目录(catalog)         notification(告警)
(config 数据)                                                  message-sender
```

收紧动作 = **统一 `Guardrail` 契约 + 统一目录(可审计) + 把散在 wecom 的 input/output guard 物理归并到 `agent/guardrail/`**。周围三类**不并入**：信号源（`conversation-risk` 检测器，作 guardrail 输入）、config（`strategy.red_lines`，guardrail 读它）、sink（`notification`/`intervention`，guardrail 触发）；渠道入站清洗（`filter.service`，留渠道，非业务 guardrail）。
...
| `wecom/reply-fact-guard` | output guardrail | `agent/guardrail/output/rule`（搬） |
| `wecom/pre-agent-risk-intercept` | input guardrail（消费风险信号） | `agent/guardrail/input/risk`（搬；调 intervention 用依赖倒置） |
| `tools/duliday/{booking-guards,age,sensitive-screening,hard-requirements}` | tool guardrail | **留 tools**，遵契约 + 登目录 |
| `conversation-risk/*` | 信号源(detect) | 留，作 guardrail 输入 |
...
**rule 层（`RuleGuardrail`，确定性，先跑）**——每条都对齐一个外生事实：
...
```ts
@Injectable() class TurnRunnerService {
...
```
**流程**：`input 守卫 → generator.invoke → output 守卫 →` `pass` 产出 reply / `revise` / `block`。

⚠️ **`block` 与 `handoff` 是两种不同终态，勿混**：
- **`blocked`（不发+观测）**：output guard 命中普通话术/口径问题（如兼职提保险、回名字），**本轮沉默不发、记观测告警**，**不暂停托管、不 @ 人**。
- **`handoff`（不发+转人工+pause+告警）**：高危/结构性（歧视外露、booking hard-reject、原则性红线），必须 `request_handoff` → 暂停托管 + @ 招募经理。
- output guard 的 `block` 决策须带 `severity`，由 runner 据此映射到 `blocked` 还是 `handoff`——避免普通话术 block 被误实现成人工介入，或高危 block 漏掉介入。

⚠️ **revise 必须区分本回合是否已提交副作用工具**（`invite_to_group`/`booking`）：
- **无副作用** → 允许全量重跑 generator（≤1 次，复用 `deferTurnEnd` 丢弃机制）。
- **已提交副作用** → **只能 no-tool 文本重写**（无工具改写文案）或直接 block/handoff，**禁止全量重跑**——否则会再次 booking/invite（prepareStep 的副作用屏蔽只在同一次 AI SDK loop 内生效，跨次重跑失效）。见 §12 HC-1。
...
@Processor('reengagement') class FollowUpTaskProcessor {
  constructor(private state: SessionService, private runner: TurnRunnerService, private delivery: ChannelDeliveryPort) {}
  async process(job) {
...
```ts
// agent/ports/channel-delivery.port.ts —— agent 定义接口
...
agent/
  generator/    generator.service.ts                 (← runner.service.ts 正名)
  guardrail/
    catalog.ts                                       (统一目录/注册表；逐条审计"新外生信号")
    input/      input-guardrail.service.ts           (← input-guard.service.ts)
                risk-intercept.service.ts            (← wecom/pre-agent-risk-intercept；调 intervention 用倒置)
    output/     output-guardrail.service.ts          (组合 rule→llm)
                rule-guardrail.service.ts            (← reply-fact-guard.service.ts；补 3 规则)
                llm-reviewer.service.ts              (新；ModelRole.Review)
    （tool guardrail 不在此 —— 因分层物理留 tools/，见下）
  runner/       turn-runner.service.ts               (新；编排)
  reengagement/ scenario-registry / trigger-evaluator / scheduler / task-processor  (新)
  ports/        channel-delivery.port.ts             (新接口)
...
llm/llm.types.ts  ModelRole.Review = 'review'        (新角色；+ .env AGENT_REVIEW_MODEL)

channels/wecom/message/
  reply-workflow.service.ts   被动：intake/merge/replay → runner.runTurn → 投递
  delivery.service.ts         implements ChannelDeliveryPort
  （pre-agent-risk-intercept、reply-fact-guard 迁出 → agent/guardrail/）
```

依赖方向（无环）：`reengagement → runner → {generator, guardrail.input/output} → {memory, llm}`；`agent/guardrail/catalog → tools`（tool guardrail，agent→tools 允许）；**guardrail 全员与 tool guardrail 都 import 中立 `types/guardrail.contract.ts`（破环关键，tools 不反向依赖 agent）**；`channels/wecom → runner` 并提供 `ChannelDeliveryPort` 实现。
...
- **状态并发**：写权威状态必须在处理锁内（现 turnEnd await 模式 L443），杜绝跨 job 覆盖。
...
| 阶段 | 内容 | 依赖 |
|---|---|---|
| **阶段 0a** 纯搬家（行为不变，纯结构迁移，易回滚） | ① `generator` 正名；② 抽 `runner`(渠道无关) + `ChannelDeliveryPort`，`reply-workflow` 改为调 `runner`（**保持原 turn-end/投递语义**）；③ 建 `guardrail` 模块 + 中立 `Guardrail` 契约 + `catalog`，迁入散落 guard（input 归位 / output rule 迁出 / risk 迁出，登目录，**行为等价**）；④ 搭 `AuthoritativeSessionState` 骨架 + 读写 API（**只读不改写时机**）。进展：`TurnRunnerService` 薄接缝、`ChannelDeliveryPort`、`AuthoritativeSessionState` 骨架已落；guardrail catalog 未落。 | — |
| **阶段 0b** 语义变更（行为变，需独立验证） | HC-4 记忆生命周期改造（"投递成功后才写'对用户说过'"）；`runner` revise 回路接缝（`toolMode`）。**与 0a 分开上，便于回滚/对比** | 阶段 0a |
| **阶段 1a** `guardrail.tool` BookingGuardrail（权威 `evaluatePrecheck` 重算 + 硬拒强制 handoff） | 抽 `evaluatePrecheck()`；权威状态**最小切片**：`collectedFields/hardConstraints` 确定性写入 + **`recalledJobIds:Set` 成员判定**（jobId provenance，非现有布尔 `hasRecalledJobs`）；候选人原文 parser 须在 gate 前运行 |
| **阶段 1b** `guardrail.output` LlmReviewer（强模型）+ rule 补 3 规则 + `runner` revise 回路 | 阶段 0 的 guardrail 模块；`ModelRole.Review` |
| **阶段 2** 岗位事实结构化渲染 + geocode 置信度 | 承接 §5.5 **B 类** prompt 规则（事实口径转渲染、从 prompt 删） |
| **阶段 3** 权威状态补全：`presentedStores/location/lastCandidateMessageAt` | 渗透展示/geocode 工具 |
| **阶段 4** `reengagement` shadow mode → 低风险场景开放 | 阶段 3 的状态停止条件 |
...
## 12. 未决硬约束（评审焦点 · 实现前必须落定）

> 来自架构评审（2026-06-24）。方向（拆 booking/precheck 自证链、output fact guard 从告警升级为裁决）是对的，但**在 HC-1/2/3 落定前不进入实现**——否则只是把事故路径包装得更漂亮，核心问题仍在。
> **HC-1/2/3 的可实现 runtime 机制详见**：[agent-reliability-hc-runtime-mechanisms.md](./agent-reliability-hc-runtime-mechanisms.md)。

**HC-1（P0）副作用后的 revise 只能"无工具文本重写"，不能全量重跑 generator。**
prepareStep 的副作用屏蔽只在同一次 AI SDK loop 内生效（[runner.service.ts:306](../../../src/agent/runner/agent-runner.service.ts#L306)）；现有代码已对命中 `invite_to_group`/`booking` 的回合跳过 replay（[reply-workflow.service.ts:303](../../../src/channels/wecom/message/application/reply-workflow.service.ts#L303)）。
→ `runner` 在 revise 前判本回合是否有**已提交副作用工具**：有则降级为 no-tool 文本重写或 block/handoff；仅无副作用回合允许全量重跑。
...
→ `collectedFields` 写入准入 = ① 候选人原文确定性解析 / ② 成功 booking 回写 / ③ 其他确定性来源；模型参数仅作"待确认草稿"，BookingGuard 不据此放行。
...
→ booking hard-reject 返回统一 `shortCircuited` gate result（携 `gateRejected + reasonCode`）→ `runner.stopWhen` 识别停 loop → outcome 处理层对 booking gate 路径执行 handoff（pause+告警），gate 只判定。完整 `TurnOutcome.kind='handoff'` 类型抽象仍待后续收敛。

**HC-4（P1）记忆写入边界二分。**
lifecycle 现接收被采纳 assistantText（[runner.service.ts:397](../../../src/agent/runner/agent-runner.service.ts#L397)），但 revise 前草稿/被拦草稿/未投递成功的回复都不应写入"已对用户说过"。
→ (a)「对用户说过」记忆 ← 仅 pass+投递成功的最终文本；(b)「已发生副作用」状态 ← 已提交工具效果，无论回复是否被拦/投递都落。

**HC-5（P2）渠道无关核心抽离范围被低估：tool 侧渠道能力也要端口化。**
`AgentModule` import `CustomerModule`（[agent.module.ts:10](../../../src/agent/agent.module.ts#L10)）、`ToolRegistryService` 依赖 `RoomService`/`MessageSenderService`（[tool-registry.service.ts:37](../../../src/tools/tool-registry.service.ts#L37)）。
...
**HC-6（P3，已修正）** `REPLAY_BLOCKING_TOOL_NAMES` 只含 `invite_to_group` + `duliday_interview_booking`，`advance_stage` 不属于（[reply-workflow.service.ts:51](../../../src/channels/wecom/message/application/reply-workflow.service.ts#L51)）。§1 已更正。
...
- [reply-workflow.service.ts](../../../src/channels/wecom/message/application/reply-workflow.service.ts)：`processMessageCore`(L190)、`callAgent`(L514)、ReplyFactGuard 调用(L611)、投递(L415)、replay 丢弃(L327)
- [reply-fact-guard.service.ts](../../../src/agent/guardrail/output/hard-rules.service.ts)：`check`(L193+)、规则(L358-396)、歧视正则(L131-155) —— 归入 `guardrail/output/rule`
- [runner.service.ts](../../../src/agent/runner/agent-runner.service.ts)：`invoke`(L112)、`stopWhen`(L138-142)、副作用工具屏蔽(L306-310) —— 正名 `generator`
- `src/tools/duliday-interview-precheck.tool.ts` / `duliday-interview-booking.tool.ts`：precheck 校验 + booking `prechecked` 契约（待改：抽 `precheck-core` + 接 BookingGuardrail）
