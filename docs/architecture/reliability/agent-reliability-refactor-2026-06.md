...
> - [agent-reliability-implementation-roadmap.md](../../archive/reliability/agent-reliability-implementation-roadmap.md) = 拆成 PR-A…G 的施工计划 + 进度
...
- `generator`：生成 agent。现为 [GeneratorAgent](../../../src/agent/generator/generator.agent.ts)（OpenAI 称 `Agent`；它本就是"生成方/会动手的 worker"）。
...
prepareStep 的副作用屏蔽只在同一次 AI SDK loop 内生效（[generator.agent.ts](../../../src/agent/generator/generator.agent.ts)）；现有代码已对命中 `invite_to_group`/`booking` 的回合跳过 replay（[reply-workflow.service.ts](../../../src/channels/wecom/message/application/reply-workflow.service.ts#L323)）。
...
**HC-4（P1）记忆写入边界二分。**
lifecycle 现接收被采纳 assistantText（[generator.agent.ts](../../../src/agent/generator/generator.agent.ts)），但 revise 前草稿/被拦草稿/未投递成功的回复都不应写入"已对用户说过"。
...
- [generator.agent.ts](../../../src/agent/generator/generator.agent.ts)：`invoke`、`stopWhen`、副作用工具屏蔽、`deferTurnEnd` lifecycle —— 原 `runner.service.ts` 正名为 `generator`
