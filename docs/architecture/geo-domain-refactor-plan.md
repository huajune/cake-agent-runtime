...
4. 业务足迹内县级市补录进映射（如 `余姚市/慈溪市 → 宁波`，9.2），这是延吉类隐患在在营城市的直接闭环，**不等 Phase 4**；
   ...
   - 分组与可行性（2026-07-30 真实海绵只读查询实测更正）：**苏州系**（太仓/常熟/张家港）原判"可凭昆山实证优先补录"**已不成立**——苏州全城仅 1 个在库岗位（在苏州工业园区），`regionNameList` 传昆山市/太仓市/常熟市/张家港市 一律 `total=0`（含 `onlySignableJobs=false`），昆山自身也已查不到岗位，"按昆山同法确认"无 live 数据可比对；**宁波系**（余姚/慈溪）同样 0 在库岗位。两系现在是同一个阻塞：等岗位回库。**其余**（青岛系胶州/莱西/平度、宜昌系宜都/当阳/枝江、荆州系松滋/洪湖/石首/监利、黄冈系麻城/武穴、襄阳系枣阳/宜城/老河口、赣州系瑞金/龙南、沈阳新民）均未做海绵抽样验证。
   - 结论：**"补录前先验证"原则下，当前一条都不能补**。触发式补录（候选人真实提到某县级市时再针对性验证）比批量抽样更可行，故每日观测把 `DEFERRED_COUNTY_BACKFILL` 命中列为上报项。已补录的昆山市映射在 `COUNTY_LEVEL_CITY_TO_PREFECTURE` 中不受 0 库存影响，无需回滚。
