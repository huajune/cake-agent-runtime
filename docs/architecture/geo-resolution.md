...
4. ~~**守卫规则保留为后盾**：拦截量趋零本身就是验收指标。~~ ⚠️ **2026-08-14 更正：该后盾并不存在。** `district_level_distance_claim` 的规则实现早在 **2026-07-10 的 #499 守卫硬规则批量下线**（location 全族）中就已删除，比本方案 07-22 上线还早 12 天；#628 提交信息里「守卫规则保留为后盾，不下线」在写下时即不成立，src 内如今只剩注释引用。**因此「拦截量趋零」不是验收指标**——规则不存在，命中必然为 0，观测三周的 0 命中不构成任何证据。**渲染层是这条链路的唯一防线**，验收只能看第 2 条：生产 3 周 764 次 area_level 渲染、缺「估算」0 次（2026-08-14 实测）。若要恢复双保险，须重新实现规则，而非"保留"。
