...
├── administrative-division.overrides.ts    # 人工维护：供应商差异 + 业务偏置 + 待补录登记
...
- 父子关系环；县级市缺失父级；
- 标准名称尾缀和行政级别不一致；
- **显式城市表 × 县级市映射交叉一致性**——全国显式表中属于业务城市辖下的县级市，必须在县级市映射表有父级条目（即「余姚防线」，见 §9.1）；
- 业务裸地名别名表中不得混入省份；
- 生成数据数量相对上一版本异常增减。
...
`administrative-division.overrides.ts` 的 `DEFERRED_COUNTY_BACKFILL` 现登记 **23 条**（余姚 / 慈溪为原始 2 条，其余为 `geo:validate` 按余姚防线扫出的同构隐患 + 沈阳补录连带登记）。

**「补录前先验证」原则下，当前一条都不能补**（2026-07-30 真实海绵只读查询实测）：

| 分组 | 状态 |
| --- | --- |
| 苏州系（太仓 / 常熟 / 张家港） | 苏州全城仅 1 个在库岗位，`regionNameList` 传任一县级市均 `total=0`，昆山自身也已查不到岗位——**无 live 数据可比对** |
| 宁波系（余姚 / 慈溪） | 同样 0 在库岗位。与苏州系同一阻塞：**等岗位回库** |
| 其余（青岛 / 宜昌 / 荆州 / 黄冈 / 襄阳 / 赣州 / 沈阳系） | 均**未做**海绵抽样验证 |

**处置**：触发式补录（候选人真实提到某县级市时再针对性验证）比批量抽样更可行，故每日观测把 `DEFERRED_COUNTY_BACKFILL` 命中列为上报项。已补录的昆山市映射不受 0 库存影响，无需回滚。校验对已登记条目降级为提示，**补录完成后须从表内移除**，否则校验报冗余。
...
- **24 个由 `unresolved` 变为父级命中**——23 条 `DEFERRED_COUNTY_BACKFILL` 待验证条目 + 全国表补充样本义乌市；
...
### 9.3 地理信号冲突检测：enforce 判 no-go

`detectGeoSignalConflict` 在多信号指向不同城市时返回 `ambiguous` + `candidates`，目前**仅 shadow 运行，enforce 未开启**。

shadow 窗口累计**样本 1 条**，该唯一样本判定为**噪音**：
...
| anchor | `source=geocode, precision=poi, coordsProvenance=turn_geocode` —— 该轮实际处理无误 |

**根因：同形地名跨层级。** 长阳既是宜昌下辖县（`UNIQUE_SUBDIVISION_TO_CITY` 收录为 `长阳: '宜昌'`），也是北京房山下辖镇。收录不变式「多个城市共享的区名必须排除」只按**区/县级**排查过，**跨层级（县 vs 镇/街道）同形是漏网类别**。

**为何 no-go**：按 enforce 语义（冲突非 null → 跳过城市回填、留 `city=null` 让 Agent 反问），本例会让 Agent 对一条**已携带精确坐标**的位置分享反问「你在哪个城市」——净误伤，且误伤面正是信息最充分的输入类型。

**开 enforce 的前置条件（任一）**：

1. **同城包含消解**：候选 A 的 matchedText 若是候选 B 城市的下辖地名，判为同一地点、不计冲突；
2. **精确坐标豁免**：本轮已有 geocode / 位置分享坐标时不做冲突拦截（坐标本身是比文本更强的证据）；
3. **同形地名清表**：对 `UNIQUE_SUBDIVISION_TO_CITY` 全表做镇/街道级同形扫描（已 vendored 的 `china-division` 生成表可直接比对），把长阳类条目剔除或降权。

**观测缺口**：`detectGeoSignalConflict` 的两个调用点（`session.service.ts` 的 `backfillCityFromWhitelist`、`duliday-job-list.tool.ts` 的 `queryMeta`）都只传 `district` / `location`，**不传已确立的会话 city**。上例文本第一轮扫描就显式命中了「北京市」，冲突检测却对此失明。把已知 city 传入并用于候选消解，是条件 1 的最小实现路径。
