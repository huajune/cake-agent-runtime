...
> 状态：草案 v3（方向已定，**含 6 项未决硬约束，未达可实现状态** —— 见 §12）。架构评审 2026-06-24。
> 日期：2026-06-23
...
- **守卫已存在**：[ReplyFactGuardService](../../src/channels/wecom/message/application/reply-fact-guard.service.ts) `check()`（L193+），现有规则 `discriminatory_screening_leak`(block)、`group_promise_without_invite`、`salary_fabrication`、`booking_form_field_mismatch`；确定性，非 LLM；调用点 reply-workflow L611。另有 `InputGuardService`（生成前输入守卫）。**——这两者本质就是 input/output guardrail。**
...
prepareStep 的副作用屏蔽只在同一次 AI SDK loop 内生效（[runner.service.ts:306](../../src/agent/runner.service.ts#L306)）；现有代码已对命中 `invite_to_group`/`booking` 的回合跳过 replay（[reply-workflow.service.ts:303](../../src/channels/wecom/message/application/reply-workflow.service.ts#L303)）。
...
**HC-4（P1）记忆写入边界二分。**
lifecycle 现接收被采纳 assistantText（[runner.service.ts:397](../../src/agent/runner.service.ts#L397)），但 revise 前草稿/被拦草稿/未投递成功的回复都不应写入"已对用户说过"。
...
- [reply-fact-guard.service.ts](../../src/channels/wecom/message/application/reply-fact-guard.service.ts)：`check`(L193+)、规则(L358-396)、歧视正则(L131-155) —— 归入 `guardrail/output/rule`
- [runner.service.ts](../../src/agent/runner.service.ts)：`invoke`(L112)、`stopWhen`(L138-142)、副作用工具屏蔽(L306-310) —— 正名 `generator`
