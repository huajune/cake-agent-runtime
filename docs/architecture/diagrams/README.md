...
> `scripts/diagrams/build-agent-quality-excalidraw.mjs` and re-run it, then run
> `scripts/diagrams/render-agent-quality-preview.mjs` to refresh the SVG. The other
> diagrams can be edited directly from their `.excalidraw` source files.
