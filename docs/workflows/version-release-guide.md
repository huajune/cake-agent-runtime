...
语义化版本，基于 commit 历史计算：`BREAKING CHANGE`/`type!:` → major+1；`feat:` → major+1（项目口径：业务新能力直接进入下一大版本）；`perf:`/`refactor:` → minor+1；其他有效提交 → patch+1。注意版本号按 commit 算，但 CHANGELOG 文案不照抄 commit，优先取 PR 模板中文说明。
