...
- [测试套件表](#测试套件表)
  ...
  - [test_conversation_snapshots](#12-test_conversation_snapshots---对话测试数据源)
- [RPC 函数](#rpc-函数)
- [数据生命周期](#数据生命周期)
...
| 12 | `test_conversation_snapshots` | 测试 | insert / update | select | 待定 |
...
| `feishu_table_id` | varchar(100) | - | 飞书表格ID（回写目标 sheet） |
...
| `conversation_snapshot_id` | uuid | - | 关联 test_conversation_snapshots |
...
- `idx_test_executions_conversation_snapshot` - conversation_snapshot_id
- `idx_test_executions_conv_turn` - conversation_snapshot_id, turn_number（部分索引）

---

### 12. test_conversation_snapshots - 对话测试数据源

**用途**：存储对话回归测试的源数据，从飞书导入完整对话内容

**代码位置**：

- Repository: `src/biz/test-suite/repositories/conversation-snapshot.repository.ts`
- Entity: `src/biz/test-suite/entities/conversation-snapshot.entity.ts`
...
**触发器**：`trigger_conversation_snapshots_updated_at` — UPDATE 时自动更新 updated_at

**索引**：

- `idx_conversation_snapshots_batch_id` - batch_id
- `idx_conversation_snapshots_status` - status
- `idx_conversation_snapshots_batch_status` - batch_id, status
