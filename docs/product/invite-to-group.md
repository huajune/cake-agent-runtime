...
群归属于"琪琪"账号（群主），其他托管账号通常也在群内。拉人使用企业级接口（`/hub-api/api/v1/instantReply/addFromRoom`）：优先用当前接客 bot 邀请；若接口返回 `room not found`（接客 bot 不在该群或看不到该群），先用群列表中记录的群所属 bot 把当前接客 bot 拉进群，再用当前接客 bot 重试邀请候选人。
