...
## 8. 人工反馈池分层处置与 T1 复测（2026-07-23 收口）

### 池分层终态（159 条待分析 → 三层归位）

| 层 | 数量 | 处置结果 |
|---|---|---|
| A 层（已有修复覆盖） | 73 | 已批量置"处理中"挂修复出处（#684/#685/v10.25.0/守卫在产），T1 复测通过后转已解决 |
| C 层（判定过时） | 9 | 已按"已过时并入已解决"约定收口 |
| B 层（新根因专项） | 77 | 保持待分析，归属 8 个专项（见 §8.2） |

池终态核账：待分析 77 / 处理中 111 / 待验证 26 / 已解决 3,544。

### 8.1 T1 复测批（batch `56d30741`，本地 worktree 合并态 = develop+#684+#685，TEST 库真实链路）

16 条策展 scenarioCase 已入飞书正式测试集（PII 已脱敏、动态事实以本轮工具结果为准）；14 条执行（2 条需真实工单 fixture 未跑）：**通过 10 / 失败 3 / 跳过 1**，Dashboard `pass_rate 71.43%`，剔除 skipped 后可评估通过率 76.9%。批次已同步生产 Dashboard（warnings 空、executionCount=14 对账一致）。

**修复实证全部生效**：姓名解锁三路径、记忆污染零外显、二档无岗承接、全等复读改写、转人工静默、电话面试口径（booking 轮）、区级城市映射、phone 溯源。附加收获：jobId provenance 闸门在测试中按设计拦截了模型臆造 jobId。

**T1 暴露的两个真实残留（入 B 层）**：
1. 对话层同题限问无护栏——模型不调 booking 时（无 jobId 上下文），"索名 2 次转 handoff"闸门不触发，仍会第三次索名；
2. 面试形式跨轮不粘——booking 轮的"先电话沟通"指令管不到后续追问轮，需把面试形式写入 session facts 并持续 hint。

**归类修正**：1ptrzpwk（"麦当劳早班14点下班"被当求职品牌）复测确认不在已修范围，从 A 层移回 B 层。

### 8.2 B 层 8 个新根因专项（77 条的归属，按优先级）

| 专项 | 规模 | 优先级 | 要点 |
|---|---|---|---|
| B-5 报名失败静默 | 6 | P0 性质 | booking 终态播报硬约束：成功必播报、失败必告警/handoff，禁静默；补改约同天/重报路径 |
| B-1 岗位事实口径 | 12 | P1 | 班次/结算周期/阶梯薪资/工时下限结构化输出照念，缺字段明示"以门店为准"禁推断 |
| B-2 健康证状态机 | 6 | P1 | "有证约"岗位 precheck 硬校验；四态口径；记忆健康证提交前本会话重确认 |
| B-3 硬性条件漏筛 | 13 | P1 | 经验/班次/工期/身份筛选条件推荐前先筛（customerLabel 覆盖面扩展） |
| B-4 多人/多家报名 | 8 | P1（需产品裁定） | 至少做到识别意图→明确边界+handoff，杜绝"说报上了实际没报" |
| B-6 时间锚定 | 4+ | P1 | 面试时间从候选人确认到提交的一致性校验；+T1 残留②面试形式粘性 |
| B-7 话术微调批 | 12 | P2 | 一个 prompt PR 收拢；+T1 残留①对话层限问护栏 |
| B-8 产品口径裁定批 | 5 | 待裁定 | 残障候选人话术红线/合同询问转人工/撤回消息感知/办证期留岗承诺 |

### 8.3 待办交接

- 2 条策展 case（取消已完成面试/带外披露）需真实工单 fixture 能力后补测（test-suite memorySetup 当前不支持注入工单）
- v10.27.0 部署后生产冒烟：input 层 human_handoff_request 短路路径（T1 走的是模型 handoff 等价路径）、电话面试岗真实 booking 话术抽查
- A 层 73 条在生产复测通过后由 T1 后续批推进"已解决"
