...
| 项 | 执行结果 |
|---|---|
| PR-1（A3/B4/B5/B8 + 电话面试修复） | [PR #684](https://github.com/huajune/cake-agent-runtime/pull/684) 待 review；全量测试 0 失败 |
| PR-2（B7/C9） | [PR #685](https://github.com/huajune/cake-agent-runtime/pull/685) 待 review；文件与 #684 零交集 |
| 门面收口（盘点顺产） | [PR #686](https://github.com/huajune/cake-agent-runtime/pull/686)：三个无主 re-export 门面退役；geo 门面按 Phase 5 条款 ≈7/28 收口 |
| S2 / D9 批量归档 | **3,115/3,115 成功，0 失败**（151 条分层抽样复核后执行；78 条薪资真阳单独标注）。飞书表复核：已解决 419→3,535，待分析 3,266→159 |
| S1 / A2 存量污染清洗 | 生产 907 个活跃 factsv2 扫描 → 44 会话确认污染（候选人原文二次校验 0 假阳）→ 88 业务字段 + 5 个 reasoning meta 字段清除 → **复扫归零（837 key / 0 命中）**；审计与回滚材料存档 |
| Supabase 长期档案污染 | 1 行（user 7881301002133137）name/phone 已带守卫单行 UPDATE 清除并验证 |
| D14 Aron 案补录 | 已入池（P1 / 11-情绪/话术） |
| D11 测试占位收尾 | y3dzymr3 已置已解决 |

### 收口时新发现（需运营处理）

**工单 452333**（jobId 528698，chat `6a60806dce406a6aee613ff5`，2026-07-22 09:47 提交）：清洗长期档案时发现该工单以编造身份"张三/13800138000"**报名成功**（source=booking 实证）——门店登记的姓名与手机号均为假，联系不上候选人。需运营手工核实真实身份后改约或取消。PR #684 的 phone 溯源闸门上线后可阻断此类提交。

### 仍在途

- PR #684/#685/#686 review 合入 + 发版（修复上线前生产仍跑旧逻辑）
- T1：C10 策展 ~10 条入正式测试集 + D10 收口 4–5 月 64 条高优存量（依赖 PR-1 发版）
- O1 / D13：冷区接客榜 + 引流-供给对账（需运营提供 Boss 投放城市清单）
- 海绵侧需求：面试方式结构化值下发 `firstInterviewWay`（已核实为 null，见根因 6）
- Step 10 源样本池状态回写（依赖 T1 验证批次结果）
