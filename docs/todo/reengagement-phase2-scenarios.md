...
- 话术模板：需求稿 A–D 为空白。**0820 用户裁定：无需运营固定话术**——复聊文案由模型按 generationPolicy 与当次已核验事实生成（与一期"示例不是固定模板"口径一致），不构成真发 blocker。
...
- 话术：**无需运营固定模板（0820 裁定）**，generationPolicy 即最终口径：确认是否已顺利入职 / 有没有遇到问题 / 需要协助可以说；不得断言候选人已入职或未入职；不施压、不催报到。
...
依赖关系：PR-A 独立先行；PR-B 是 PR-C 的前置；PR-D 独立。
...
- [ ] 本档 generationPolicy（0820 裁定即最终口径，无需运营话术校准）：确认是否已顺利入职 / 有没有遇到问题 / 需要协助可以说；不得断言已入职或未入职；不施压不催报到。
...
- [ ] 真发前置：§7 #1（聊天轨）#2（是否暂停）#3（复核口径）按建议默认已实现；话术 blocker 已由 0820 裁定解除。
