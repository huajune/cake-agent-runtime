...
**⚠️ 学生筛不得随本批退役**：岗位侧的学生硬筛（`screeningCriteria.isStudent` →
`student_rejected`）是运营最在意的硬筛之一，而契约只有 60% 的岗位带身份语义族标签。
照原计划退役，那 40% 无身份标签的岗位会**静默停止筛学生**——不报错、不告警，
只是学生开始被放进不收学生的岗位。故：学生维度的岗位侧筛选**保留**，
待后端补齐身份标签覆盖率后单独立项退役，退役前需重跑本探针确认 100%。
