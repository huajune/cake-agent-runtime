...
> 办结即终审。
>
> **⚡ 0818 瘦身裁定（v3-lean，实施形态以蓝图为准）**：用户裁定原设计过重。本节
> 机制描述中以下细节已被蓝图 v3-lean 取代——确认态砍除（**针对性问答即采信**，
...
- **契约缺口（对照核对清单）**：required 标志无（按"返回即须收"处理）、年龄边界未结构化
  （687 是无值域 TEXT）、interviewTime 窗口语义未进契约、TEXT 值域校验无、披露级别无、
  缓存失效信号无——已消解（0818 裁定标签零缓存实时查询）。labelInstructions 仅 22 处使用。
  **0818 升级约定（收资判决单源化）**：已与后端约定 minAge/maxAge、required 等判决要素
...
岗位聚焦（job_list / focusJob 确立）
  ...
  → precheck 收资模型：
      身份核标签（769/770/687/771）——既有闸门（姓名真名闸/手机号出处闸/年龄边界）降为槽位写守卫
        （年龄等值域判据读标签契约，不读岗位详情——0818 判决单源约定）
