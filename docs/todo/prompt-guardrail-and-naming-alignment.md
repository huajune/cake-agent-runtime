...
## Backlog（挂触发器，不排期——2026-08-13 "这层怎么这么浅"评审记账）

> 第四层当前刻意薄（发牌制：薄到证据要求它长为止；主力是 220 行 CI 扫描 + 六处负代码清洗）。
> 以下两项是评审确认的真欠账，**只挂触发器不排期**，触发即立项。

| # | 项 | 内容 | 升深触发器 |
|---|---|---|---|
| BL1 | CI 形状扫描的门店形空洞 | 人名形检测只认 2-4 字 CJK，「人民广场店」类 5 字+门店名形不被抓；registry 有 `store_name` kind 却无对应检测器。补法：「X 店/X 广场/X 中心」后缀启发式 + 注册表白名单 | 下一次普查复查发现 CI 漏掉的红区值 |
| BL2 | **语料分域（示教原则 4）的结构化执法**——本层唯一值得变厚的方向 | 现状：分域只在抽取审计一处以手工排除清单存在。成年形态：prompt 组装时给每个 block 打结构化标签（teaching / evidence / tool_result，StruQ 思想的 scaffold 层版本），出处公证与回声审计消费标签而非各自维护排除清单 | ①任一例"教学文本穿过公证被当证据"的事故；或②出现第三个需要手工排除语料的消费点（两处是惯例，三处该抽机制） |
