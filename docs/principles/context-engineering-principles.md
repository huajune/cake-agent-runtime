...
与 Claude Code 设计哲学同构："信任模型的局部判断，把执行关进高度确定性、最小侵入的操作 harness"——本库 P11（模型作证、代码公证、本人终审）即此形状。判断替代型 harness 折旧的历史学费单（守卫 30→10、语义审查 enforce 从未够格、repair 破产等）见 [bitter-lessons.md](./bitter-lessons.md)。
