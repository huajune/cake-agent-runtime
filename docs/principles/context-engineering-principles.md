...
> 配套现状：[knowledge-base/05-Prompt-Section动态组装体系](../knowledge-base/05-Prompt-Section动态组装体系.md)、
> [Prompt 规则台账](../prompt-rule-ledger.md)；上下文治理三期已收官，执行过程见 Git 历史。
...
| 守卫硬规则历史批次 30 → 10 条（2026-07-10 大下线）                | 同一单向棘轮病在守卫层的表现                                                                         | 只有加法算子的系统必然腐烂     |
...
与 Claude Code 设计哲学同构："信任模型的局部判断，把执行关进高度确定性、最小侵入的操作 harness"——本库 P11（模型作证、代码公证、本人终审）即此形状。判断替代型 harness 折旧的历史学费单（守卫大规模退役、语义审查 enforce 从未够格、replan 破产等）见 [bitter-lessons.md](./bitter-lessons.md)。
...
| 两层存储 + CoALA 内容类型映射              | 参考 Letta/MemGPT 的分层思想，但不照搬其物理存储层；本库由确定性 pipeline 管理状态 | MemGPT / CoALA 论文系   |
| consolidation 空闲摘要                     | 与 Anthropic compaction 的触发思想对照；本库摘要 append-only、水位独立             | Anthropic 工程博客      |
