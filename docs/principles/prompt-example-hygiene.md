...
3. **格式教学交给渲染，不交给描述**：与其在 describe 里写"表单长这样：姓名：X…"，
   ...
   这是 P2"按构造约束"用在教学材料上。
4. **教学语料与证据语料分域**：教学文本（system/describe）与证据文本（候选人消息/工具结果）
   永不混装；一切出处核验（引文公证、回声审计）只在证据语料内进行。
   抽取审计的 system-prompt 排除已实现此原则，应为通例。
...
| **出站侧（示例值漏给候选人）** | **示例词表出站扫描**：回复含注册示例值（常允丽/13800138000/人民广场店…）→ 拦截。纯字符串比对，P3 合法封闭形态，业界 canary 守卫的同款 | ❌ **唯一缺口，建议补**（本文唯一新增防线建议） |
| 注意力稀释 | "最小高信号 token 集"——普查红黄区清洗即是减法 | ◐ 随清洗推进 |
...
出站 canary 扫描规则——见 [prompt-example-census.md §五](../todo/prompt-example-census.md)。
