...
| [ElizaOS（v2）](https://github.com/elizaOS/eliza) | 开源仓库源码 | 共识 #1/#2：provider 三通道、`cacheStable` 声明与确定性断点（~~B5 先例~~ **2026-08-25 降级为实现观察，非决策依据**——dynamic 入 user 消息的做法信任拓扑与本库不匹配，见三期文档批二节）；facts 检索故意零向量+置信度×时间衰减（无向量裁定再证）；反面：全局裸整数 position 排序 |
...
| [ZeroClaw](https://github.com/zeroclaw-labs/zeroclaw) | 开源仓库源码 | 共识 #2/#3：记忆召回前缀拼最后一条 user 消息（~~B5 先例~~ **2026-08-25 降级为实现观察，非决策依据**——个人助手 user 通道=主人，与本库客户对话拓扑不匹配，见三期文档批二节）；溢出拒绝摘要、整回合裁剪+breadcrumb"丢失永不静默"；注入策略按回合来源键控；防记忆投毒 skip set |
