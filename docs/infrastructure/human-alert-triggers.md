...
飞书任务开启后，每次有效提交人工介入都独立创建一条任务，记录本次候选人、会话、原因、负责人及跟进时间；同一会话、同一岗位、同一分钟再次命中也各建一条，不合并到旧任务或追加评论（2026-09-24 裁定）。每次提交使用独立创建标识，仅该次请求的网络重试复用标识，避免响应丢失后重复建单。原有介入提交条件及测试会话过滤仍适用。
