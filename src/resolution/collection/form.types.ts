...
export interface FormSlot {
  ...
  /**
   * 已经烧过配额的**作答内容指纹**（同槽最多留 MAX_REJECTED_ATTEMPTS_PER_SLOT 条）。
   *
   * `lastRejectionCountedTurnId` 只挡得住**同一回合内**的重复入账；跨回合的同一句话
   * 挡不住——候选人贴回的那张模板会留在证据窗里，**每一轮都被 form_line 重新解析、
   * 重新拒收**，于是候选人只答过一次，两轮就把配额烧光并静默转人工。
   *
   * 生产实证（2026-08-31，chat 6a94e92d… labelId 3 籍贯）：候选人 03:21:03 贴回模板
   * 写「籍贯：南京」（契约要省级），03:21:55 与 03:25:20 两轮各记一次拒收 → 熔断。
   * 而 Agent 的正确引导话术「改成省级，写江苏省就行」03:25:41 才发出——**比熔断晚 21 秒**。
   * 模型已经会自救，状态机却先一步判死。
   *
   * 按内容指纹去重后，「两次」恢复它本该有的语义：**两次不同的作答**。同一句话答几遍
   * 都只算一次，候选人换了说法才是第二次机会用尽。
   */
