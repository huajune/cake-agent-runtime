/**
 * 不合格原因的**披露分级**（总纲 §2.8 构造性质④补充）。
 *
 * 铁律：**判定入账永远如实**（账本落 labelId + 命中项 + 证据），委婉只在渲染层。
 * 本文件只回答一个问题：这条不合格原因能不能对候选人明说。
 *
 * 三档：
 * - `open` 可明说族：年龄/性别/学历/健康证/身高体重等岗位硬性条件。运营裁定性别与年龄
 *   属岗位公开信息（岗位卡本就写「18-40岁，仅限男」），直说要求 + 转岗合规；
 ...
 *
 * 契约侧已对专业/籍贯/婚育族返 RESTRICTED；本红线保留为与契约标记**并行的第二道闸**
 * ——防运营新建敏感标签漏标。收词纪律与共用词表一致：只收**属性名本身**，组合形态归共用词表。
 */
const SENSITIVE_TITLE_FAMILIES =
  /^(?:专业|所学专业|专业背景|婚育|婚姻|婚姻状况|生育|生育情况|民族|籍贯|户籍|户口|户籍省份|政治面貌|宗教信仰|(?:是否有|有无)?(?:纹身|文身|刺青))/u;

/**
 * 禁明说档统一婉拒话术——**唯一居所**。收资拒绝渲染（tools/collection/rejection-renderer）与
 * 无岗位字段的默认门槛（纹身，final-check `tattoo_self_report_soft_decline`）都引用这一句，
 * 保证候选人听到的永远是同一个中性口径：不点名字段、不给可推理的因果，接换岗/拉群。
 */
export const RESTRICTED_REJECTION_MESSAGE =
  '这家的岗位跟你这边暂时没太对上，我再帮你找找其他合适的，有匹配的第一时间告诉你';

/** 禁明说档的配套禁令，与话术同居所。 */
export const RESTRICTED_REJECTION_FORBIDDEN: readonly string[] = [
