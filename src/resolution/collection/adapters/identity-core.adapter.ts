 ...
 *
 * 槽位识别按映射层产出的 `systemField` 语义标记（环境级 labelId 锚点 + 标题核验，
 * 见 contract-mapping.ts），**代码不认 labelId 字面量**（D4，0818 用户裁定）：
 * 769/770/687/771 是生产实测值，只进环境配置与测试核对基准，不进代码。
 *
 * 本适配器只做「原话 → 值 + 出处」的翻译，四道身份闸门（真名闸 / 手机号出处闸 /
 * 值形状 / 年龄值域）全部在 `applyFieldValueProposal` 的公证链里执行——闸门只设一处，
