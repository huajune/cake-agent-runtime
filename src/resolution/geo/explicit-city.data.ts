/**
 * 全国显式"XX市"城市名表（自 memory/facts/geo-mappings.ts 行为等价迁移，Phase 1）。
 *
 * 口径：基于县以上行政区划数据中名称以"市"结尾的地级市 + 县级市。这里不把
 * "苏州/昆山"这类裸名称加入扫描字典，避免在任意长句里把品牌名、门店名、
 * 普通词误当城市；只有用户原话出现"苏州市/昆山市"这种显式城市后缀时才命中。
 *
 * 生成来源：lcn data/cities.json + data/areas.json（民政部县以上行政区划数据的
 * 开源整理），取 name 以"市"结尾的唯一值，按中文排序（§9.3：来源注明保留并强化；
 * 本表不在 Phase 4 生成化范围内，长期只接受小步人工补录，受 geo:validate 交叉校验）。
 */
