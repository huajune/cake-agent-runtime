...
export const HIGH_CONFIDENCE_BARE_LOCATION_ALIASES = [
  ...
  '赣州',
  '江西',
  // 证据化族 badcase 城市补录：两城均有在售岗位（辽吉必胜客/佛山零售），
...
export const UNIQUE_SUBDIVISION_TO_CITY: Record<string, string> = {
  ...
  来凤: '恩施',
  鹤峰: '恩施',
  // 青岛（业务足迹补录）：候选人只报区名时 LLM 留 null city，白名单缺市则确定性兜底也补
  // 不出城市，invite 城市门会反问"你在哪个城市"。
  ...
  市北区: '青岛',
  平度市: '青岛',
  // —— 以下按"全国无重名且日常语料不易误命中"口径收录：区名唯一映射在查询路径
  // （geo-text-scan → 事实提取 → 岗位查询）与拉群城市门同时生效，补录只改这一处 ——
  // 上海（镇级高频地名，候选人常只说"川沙"）
  川沙: '上海',
  // 广州（候选人报"黄埔区"被当上海"黄浦区"推了一页上海岗，
  ...
  天河区: '广州',
  南沙区: '广州',
  // 沈阳（候选人报"浑南区/奥体中心"，geocode 已 unique
  ...
  大东区: '沈阳',
  沈河区: '沈阳',
  // 佛山（城市误拒高发）。南海/高明/三水均为常用词（南海=海域、高明=聪明、三水≈山水），必须带"区"后缀；
