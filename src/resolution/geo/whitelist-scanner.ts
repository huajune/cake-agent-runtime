 ...
 * 专名部分，而不是行政区本身，必须拒绝。
 *
 * 生产实证（2026-07-29 shadow，4 天 6 条冲突样本 6/6 同此根因）：
 *   - 「宝山区新顾村大家园(C区)（宝山区**宝安公路**）」→ "宝安" 命中深圳宝安区，
 *     一个上海地址被拆成 上海+深圳，且污染 sessionFacts 连挂 3 轮；
 *   - 「海淀区田村阜石路93号院（海淀区**宝山中街**）」→ "宝山" 命中上海宝山区，
 *     一个北京地址被拆成 北京+上海；
 *   - 深圳罗湖的 **洪湖**公园/洪湖路 → "洪湖" 命中荆州洪湖。
 * 这些轮次 4/6 还带着精确经纬度，靠"先命中先赢"侥幸选对城市——顺序反过来就是
 * 静默定错城市（比反问更隐蔽的事故），故在匹配层收口而非逐个清表：宝安/宝山/
 * 海淀/罗湖都是合法区名，清表清不掉。
 *
