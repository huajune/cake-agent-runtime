 ...
 *
 * 判据刻意留窄：只拦"城市同名别名 + 同城已知区县"这一形态，不把任意汉字延续
...
/**
 * 岗位卡片「发布方」字段值不是候选人的求职意向品牌。
 *
 * 候选人转发的招聘平台截图里，`发布方：XX·人事招聘主管` 是发布 / 代理主体，与他想去的
 * 雇主品牌是两个不同字段。该字段值命中品牌库即写状态时，会把候选人上一轮真实说过的
 * 品牌顶掉，岗位召回随即跑偏。
 *
...
function matchClause(
  ...
  for (const candidate of index.candidates) {
    ...
    // 履历语境：老东家自述不是新意向，标记后由 reducer 限制其状态写入。
