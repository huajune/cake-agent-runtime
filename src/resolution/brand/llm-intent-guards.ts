/**
 * LLM 品牌意图轨的确定性输入闸（2026-07-27 全天审计新增）。
 *
 ...
 *
 * 1. **助手话术回声**：Agent 自己的找店话术（"我看下有家塔可贝尔（中骏广场店）
 *    离你大概9公里"）被 LLM 当作候选人意向输出——2026-07-24 生产实例 chat
 *    6a633590，null → 塔可贝尔 凭空立主品牌。
 * 2. **系统文本回流**：守卫 repair 反馈（"- [requested_brand_mismatch] 问题：
 *    工具实际应用品牌为…"）被当候选人原话解析，形成"守卫抱怨品牌 → 品牌被重新
 *    种进状态"的自我强化回路（2026-07-23 观测实测 8 天 3 条 / 2 会话）。
 *
 * 两个判定都是纯函数：调用方（session.service validateBrandIntents）持有对话
 * 上下文，命中即整条丢弃并记日志。裸品牌名/短指代不在拦截范围——指代链接
