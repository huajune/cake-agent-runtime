/**
 * 极性确定性规则轨（§5.2）—— 模式清单集中在此，一处维护。
 *
 * 覆盖的高置信模式（有限清单）：
 * - "品牌不限 / 什么品牌都行 / 品牌随便"        → browse_all
 * - "换个品牌 / 换一家 / 换个牌子"              → 品牌为空的 negative（指向当前主品牌，§7.3）
...
/** 品牌为空的 negative："换个品牌"语义上就是排斥当前主品牌（§3.3 设计取舍）。 */
const SWITCH_BRAND_PATTERNS: RegExp[] = [/换个品牌|换一家|换个牌子|换别的品牌|换家品牌/];

/**
 * 指示代词排斥（§5.2 规则轨新增）：输出品牌为空的 negative。
 * 按 §7.3 执行顺序，同轮图片 positive 先立主品牌、空品牌 negative 再把它移入排斥，
...
const HISTORY_PRECEDING_TAIL =
  /(?:(?:刚|才|新)?从|(?:之前|以前|原来|先前|过去|曾经?)(?:在|是|做)?|(?:老东家|上一?家|前东家|上一?份工作?)(?:是|就是|在)?|(?:现在|目前|正在|眼下|当前|如今|这会儿|一直)(?:是)?(?:在|做|干)(?:做|干)?)$/;
...
const HISTORY_FOLLOWING_HEADS: RegExp[] = [
  ...
  // 现职体："在肯德基上班呢""麦当劳做着呢"——正在做的不是求职意向（badcase 1ptrzpwk）
  /^(?:的话)?(?:也|就|都|还)?(?:上着班|做着|干着|在职|工作着|上班(?:呢|了|的))/,
...
/**
 * 判断归一化子句中的品牌片段是否处于**履历/过往就职语境**（§5.2 延伸）。
 *
 * 背景：Agent 收资固定会问"之前在哪家公司做过什么岗位"，候选人如实回答老东家
 * （"优衣库的话有做三个月""刚从盒马鲜生做分拣离职"）后，老东家可能经包含匹配
 * 被当作新求职意向、覆盖已确立的 currentBrand。履历提及是记忆性陈述，不是意向切换。
 *
