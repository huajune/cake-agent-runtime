...
export const BRAND_GENERIC_ALIAS_BLOCKLIST = new Set([
  ...
  // （实锤）。降级为仅全等，单独打 "BS" 仍可命中。
  'bs',
  // 业态泛词直接是品牌标准名（brandId 10338）：候选人描述工作履历"生鲜超市做过理货
  // 和补货"经句中包含误判成意向品牌（，顶掉在位的奥乐齐）。
  // 降级为仅全等——候选人单独打"生鲜超市"指名该品牌时仍可命中。
  '生鲜超市',
]);
 ...
 * 同时在海绵品牌目录里占了一条（brandId 10024）。候选人转发的岗位卡片截图里
 * 「发布方：跃橙云服·人事招聘主管」命中品牌库后会直接写状态，把候选人上一轮真实说过的
 * 品牌顶掉，岗位召回随即圈到我方派遣主体上。
 *
 ...
 *
 * 单字符别名在中文对话里是纯噪音源，即使只做全等 token 匹配，分句后的单字也会
 * 高频误命中。品牌标准名本身不受此限（单字品牌如"匠"
 * 仍可被整句全等命中）。
 */
 ...
 * 服装品牌，顶掉上一轮真实说过的品牌。
 * 命中该集合的别名在"地名延续"语境（后紧跟同城已知区县）下按地名拒绝，见
