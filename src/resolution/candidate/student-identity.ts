 ...
 *
 * 身份判定贯穿事实抽取、precheck 证据闸门、booking 复核和出站守卫。
 * 识别前必须剥离消息装饰；否则锚定匹配会漏判，并将错误证据传给造假教唆守卫。
 *
...
const STUDENT_CLAUSE_RE =
  /^(?:我|本人)?(?:现在|目前)?(?:确定|确认|肯定)?(?:还)?(?:就)?(?:是)?(?:学生|大学生|在校生|在读学生|在校学生|高中生|在读|在校|在上学|还在上学|大一|大二|大三|大四)(?:学生|在读)?(?:的|哈|哦|呀|啊)?$/u;
...
const PENDING_EDUCATION_TRANSITION_RE =
  /(?:等|等待|准备|马上|即将)[^。！？\n]{0,12}(?:大学|录取|通知书|开学|入学)/u;
// 表单行的括号说明允许任意文本，以兼容 Agent 生成的选项说明；值交给宽松分类器。
...
export function matchIdentityEvidence(rawText: string): IdentityEvidence | null {
  ...
  for (const clause of statementText.split(CLAUSE_SPLIT_RE)) {
    ...
    if (
      ...
      !PENDING_EDUCATION_TRANSITION_RE.test(statementText)
    ) {
      return { identity: '社会人士', source: 'direct', evidence: text };
    }
  }
...
export function matchIdentityStatement(rawText: string): CandidateIdentity | null {
  const direct = matchIdentityEvidence(rawText)?.identity;
  if (direct) return direct;
  const text = stripMessageDecorations(rawText);
  if (
    /[？?]/u.test(text) ||
    /(?:吗|么|呢)(?:[啊呀嘛])?(?:[！。])?$/u.test(text) ||
    /(?:是否|是不是|招|要求|限).{0,8}(?:学生|社会人士)/u.test(text)
  ) {
    return null;
  }
  if (
    /不是学生|已经?毕业|毕业了|上班族|已经工作|工作过|在职|待岗|失业|退休|全职妈妈|在家带娃/u.test(
      text,
    ) &&
    !PENDING_EDUCATION_TRANSITION_RE.test(text)
  ) {
    return '社会人士';
  }
  return null;
