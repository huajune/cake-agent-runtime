...
export function parseHouseholdProvince(text: string): CandidateParseResult<string> | null {
  const match =
  ...
  for (const label of labels) {
    ...
    if (tail.startsWith(label)) return { value: label, excerpt: match[0].trim() };
    if (tail.startsWith(normalized)) return { value: normalized, excerpt: match[0].trim() };
  }
  return null;
}
