...
// ==================== gender_source 拆除批 A：过渡兼容读 ====================

interface FactEnvelopeLike {
  value: unknown;
  source?: unknown;
}

export type GenderProvenance = {
  source: 'candidate' | 'system';
  fromLegacySibling: boolean;
} | null;

function asFactEnvelope(value: unknown): FactEnvelopeLike | null {
  return value !== null && typeof value === 'object' && 'value' in value
    ? (value as FactEnvelopeLike)
    : null;
}

/**
 * 先读 gender 信封，信封尚无新语义时才回退旧 sibling。
 * 批 B 随 sibling 一起删除本函数。
 */
export function readGenderProvenance(info: {
  gender?: unknown;
  gender_source?: unknown;
}): GenderProvenance {
  const gender = asFactEnvelope(info.gender);
  if (gender?.source === 'candidate_quote') {
    return { source: 'candidate', fromLegacySibling: false };
  }

  const legacyEnvelope = asFactEnvelope(info.gender_source);
  const legacySource = legacyEnvelope?.value ?? info.gender_source;
  if (legacySource === 'candidate' || legacySource === 'system') {
    return { source: legacySource, fromLegacySibling: true };
  }

  return gender?.source === 'system' ? { source: 'system', fromLegacySibling: false } : null;
}
