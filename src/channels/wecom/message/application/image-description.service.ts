...
export class ImageDescriptionService {
  ...
  private readonly SYSTEM_PROMPT = [
    ...
    '\n- 招聘海报 / 招聘传单 / 含二维码的招聘截图：必须明确指出含二维码，并按图片上的标注说明其面试、报名或进群用途；用途无法确认时只说明二维码存在。同时提取品牌、门店、岗位、薪资、地址等关键信息。即使二维码本身无法解码，也不得否认图片含二维码。',
    ...
    '\n- 地图/位置截图：提取地点名称和位置信息',
    '\n- 聊天截图：提取关键对话内容',
    '\n- 表情包/表情贴图：只输出表情传达的情绪或动作，控制在 4-12 个字；不要描述角色外观、颜色、姿势细节，也不要猜测台词或意图。',
    '\n不要添加评价、建议或主观判断，只如实提取图片上看得见的事实。',
  ].join('');
  ...
  private readonly STRUCTURED_SUFFIX = [
    ...
    '聊天截图里分不清归谁就 unknown。岗位页中明确标注为设备用户自身位置的地址归候选人，key 用 candidate_address；不要与岗位地址混淆，也不得仅凭距离文字推断地址归属。',
    '身份证号/证件号不要写进 fields。',
  ].join('');
  ...
  /**
   * 读时懒补写：回合开始时补写本会话仍是裸 `[图片消息]` 占位的历史图片描述。
   *
   * 人工接管、非托管或超时期间同步进历史的图片可能没有经过描述链路；托管恢复后
   * 必须在 Agent 读取消息窗口时补齐。
   *
