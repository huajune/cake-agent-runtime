...
- 🔴 **前置状态闸门（一票否决，2026-08-06 同日两次踩坑后固化）**：**`chatHistory` 只是消息回放，不会重建任何跨轮状态。** 回放历史文本不会重跑抽取、不会重跑品牌解析、不会生成工单、不会写记忆。判据依赖以下任一状态的 badcase，**必须**用 `memorySetup` 显式注入，或改成多轮 `conversationCase` 让产生状态的那一轮真正被处理；否则判据不可评估，**不得记为"复测未通过"**：

  | 依赖的前置状态 | 典型 badcase 形态 | 只用 chatHistory 会怎样 |
  |---|---|---|
  | 已确认的预约工单 | 爽约/改约/取消边界 | 模型臆造工单号，工具报错，回复看似正常 |
  | `brand_state`（已确立品牌） | 品牌延续、品牌参与地名消歧 | 模型按"未指定品牌"走，判据落空 |
  | `sessionFacts`（已抽取事实） | 已给信息不得重复询问、硬门槛延续 | 事实为空，模型重新询问，误判为回归 |
  | precheck 拒绝态 | 拒绝不得翻案、敏感拒绝粘性 | 无拒绝态，模型按首次流程走 |
  | 已展示岗位池 / 当前焦点岗位 | 已拒岗位不得复推、岗位事实延续 | 池为空，模型重新召回 |

  **自检**：写完 case 后问一句"这条的判据，是否依赖某个由**更早某一轮的副作用**建立起来的状态？"答是 → 必须 `memorySetup` 或 `conversationCase`。
  **踩坑实证**：`au5gy9hy`（缺预约工单 → 模型臆造工单号 `WO_2100366180047430400`，超出安全整数范围致工具报错，回复看似正常，差点被判"修复未生效"）；`qvsj97jr`（缺 `brand_state` → 候选人开场白里的品牌从未进入解析，模型按"未指定品牌"查岗，差点被判"复测未通过 + 与另一 badcase 诉求冲突"）。两条都是**判据不可评估**，不是缺陷。

- **品牌输入轨要分清**（同族误判高发）：`resolveBrands` 只对**当前轮 `user_text`** 与 **`contact_name`（微信昵称/备注）** 生效。判断一条 badcase 属于哪条轨，看品牌字面出现在哪里——出现在候选人自己打的消息里是 `user_text` 轨（应采信），出现在昵称/备注里是 `contact_name` 轨（需谨慎，历史上有脏别名误命中）。**两轨的产品口径相反，混淆会得出"运营诉求互相冲突"这种假结论。**
...
BadCase 状态只保留 5 个运营可判断状态：

- 刚收集、尚未分析：`待分析`
- 已确认要修、正在修代码 / prompt / 工具、或修完待测：`处理中`
- 测试已跑但还需人工确认，或场景测试已通过但回归验证失败：`待验证`
- 定向测试/回归验证已通过，或已确认无需继续处理：`已解决`
- 现状无法修（依赖外部数据/系统能力缺口，如岗位数据无承载字段、平台侧无接口），与运营对齐后搁置：`暂搁置`（2026-08-04 用户与运营裁定新增，首例 oo83ez03/recvpYfDLkx4Fz）

`暂搁置` 使用规则：

- 只用于「归因已清楚、但当前系统条件下无法根治」的问题；纯粹还没分析、或等产品拍板的问题不算，仍分别停在 `待分析` / `处理中`
- 写入时必须在 `修复说明` 追加：搁置原因（缺什么能力/字段）、已落的兜底（如有）、解除条件（什么变化后重开）
- 巡检和积压盘点时 `暂搁置` 不计入未解决积压，但每次全量盘点要单独列出清单，防止永久遗忘；解除条件满足时改回 `处理中` 重新走修复链路
