...
export class WeworkPlanTurnToolService implements ToolFactory {
  ...
  private async planTurn(opts: {
    ...
    try {
      const classifyModel = this.modelService.resolve('classify');

      const result = await this.agentRunner.generateObject({
        model: classifyModel,
      ...
      const plan = result.object as TurnPlan;
