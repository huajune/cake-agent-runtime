...
export class SettlementService {
  ...
  async detectAndSettle(
    ...
    try {
      ...
      // 冷启动：用 epoch 查全量历史，之后初始化基准或正常检测断层
      ...
      // 无未沉淀消息：不推进边界（DB 暂时不可用返回空时不会跳过历史）
      ...
      // 取第一个断层：每轮只沉淀一段旧会话，多段旧会话在后续轮次逐步消化
      ...
      // 无闭合会话段：不推进边界，首段消息留待后续轮次（出现断层时）再沉淀
  ...
  private async generateAndSaveSummary(
    ...
    try {
      ...
      // 沉淀时将提取的身份信息写入 Profile（中置信度兜底，未 booking 的候选人也能保留数据）
      if (facts) {
