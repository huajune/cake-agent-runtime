...
- **规则线索供参考**：[规则模式匹配线索] 中列出的字段是通过正则/白名单初步提取的结果，通常置信度较高，但不具备语义理解能力（无法识别否定、纠正、指代等语境）。请将其作为参考依据，结合对话上下文做最终判断——若上下文显示用户纠正了某个信息，以用户最新表述为准
 ...
 */
function formatRuleFactsSection(ruleFacts: EntityExtractionResult | null): string {
  if (!ruleFacts) return '无';

  const lines: string[] = [];
  const info = ruleFacts.interview_info;
  const prefs = ruleFacts.preferences;

  if (info.name) lines.push(`- 姓名：${info.name}（来源：结构化表单键值对）`);
  ...
  if (prefs.city) lines.push(`- 城市：${prefs.city.value}`);
  if (prefs.district?.length) lines.push(`- 区域：${prefs.district.join('、')}`);
  if (prefs.position?.length) lines.push(`- 岗位：${prefs.position.join('、')}`);
  if (prefs.labor_form) lines.push(`- 用工形式：${prefs.labor_form}`);
  if (prefs.salary) lines.push(`- 薪资：${prefs.salary}`);
  if (prefs.schedule) lines.push(`- 班次：${prefs.schedule}`);
  if (prefs.brands?.length) lines.push(`- 品牌：${prefs.brands.join('、')}`);
...
export function buildSessionExtractionPrompt(
  ...
  ruleFacts: EntityExtractionResult | null = null,
): string {
  const brandInfo =
    brandData.length > 0
      ? brandData
