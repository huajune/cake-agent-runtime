...
- **来源声明（explicit_provenance）**：对 interview_info 中**候选人明确提供**的字段，在 explicit_provenance 数组声明 { field, quote }。判定标准（满足任一）：①结构化表单回填（「年龄：37」「健康证：有」等键值对）；②候选人直接自陈（"我有健康证""我今年37岁"）。凡候选人原话直接支持的 interview_info 字段都应列入，不要漏报。quote 必须是**候选人消息中的逐字连续片段**：禁止改写、翻译、概括或拼接多处原文；系统会按原文校验，对不上则声明无效。仅由上下文/常识推断的字段、助手提及后候选人仅以"嗯/好的"附和的字段、转发文案里的信息一律不列；解释和推理只写入 reasoning。name 字段不需要声明（有独立校验通道）
...
- is_student: 是否是学生（true/false）。触发：
  ...
  - **升学过渡仍按学生处理**："高中毕业，在等大学通知书/准备上大学/待入学"、"本科毕业，等研究生入学"同时包含“毕业”和明确升学语境时，必须提取为 is_student=true；不得只抓“毕业”改判为社会人士
- experience: 过往工作经历。候选人被问"之前做过哪些公司、什么岗位、做了多久/年限"或按收资模板回填"过往公司+岗位+年限"时提取，尽量合并为"公司/门店 + 岗位 + 时长"的短句。如果公司、岗位、时长分散在连续几条消息里，结合上下文合并；不要把岗位招聘要求里的经验要求当成候选人经历。
- height: 身高，去掉单位只保留数字（cm）。仅当候选人主动给出或按收资模板回填「身高：xxx」时提取
- weight: 体重，去掉单位只保留数字（kg）。仅当候选人主动给出或按收资模板回填「体重：xxx」时提取
- household_register_province: 户籍/籍贯所在省份（省份名，保留原话）。**敏感字段**：只在候选人**主动透露**或按收资模板回填「户籍：xxx」「籍贯：xxx」时提取，不要据"常驻城市/现居地"推断户籍
...
- labor_form: 用工形式，仅允许以下合法值之一："全职"、"兼职"、"小时工"、"寒假工"、"暑假工"。
  ...
  - "暑假工短期的兼职"里的“兼职”是对暑假工的上位类目描述，labor_form 必须保留为"暑假工"；"开学后有空也做"只描述未来可用时间，不代表当前主动放弃暑假工或接受普通兼职
...
- salary: 意向薪资（保留原话，时薪值或月薪区间）
- position: 意向岗位（工种名）
  注意区分**品类/行业词**与**工种**：候选人说"咖啡""奶茶""火锅"等是**品类/行业**，**不要据此推断具体工种**。"咖啡兼职"不要提取为 position "咖啡师"，除非候选人明确说"我想做咖啡师/想当咖啡师"
