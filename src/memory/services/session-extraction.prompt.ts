...
- name: 真实姓名（如："张三"）
  ...
  禁止来源：引用块前缀 —— 候选人消息以 [引用 XXX：...] 或 引用 XXX：... 开头时，XXX 是被引用方（通常是招募经理）的名字，不是候选人自己。引用块本身的内容也是别人发的，不可作为候选人的姓名/电话/年龄等个人信息来源。historical badcase m5lpfwi0：候选人引用招募经理"李涵婷"的发言，引擎把"李涵婷"抽成 interview_info.name，预填进了 booking 模板里发给候选人。
  触发提取的明确语境（满足任一即可）：
    1) 候选人被明确询问"真实姓名/叫什么名字"后答复
    2) 自我介绍出现"我叫 xxx"、"我的全名是 xxx"等明确句式
    3) **结构化收资模板回填**：候选人按助手发的资料 checklist 回填，行内出现「姓名：xxx」「姓名:xxx」「姓名 xxx」等键值对（key 为"姓名/名字"，value 为合法真名）。这是 booking 表单回填语境，可信度高于打招呼语。historical badcase ci7iigv4 / 362ketwp：候选人按模板填了"姓名：赵堤"，但 prompt 未声明该触发场景导致 LLM 不抽 name，Agent 反复反问"是不是身份证上的真名"。
...
- is_student: 是否是学生（true/false）。触发：
  - true：候选人原话出现"我是学生/在读/读大几/读高中/还在上学/上学的/校园"等在校学习语境，或被问"学生还是社会人士"时答"学生"
  - false：候选人原话出现"社会人士/上班族/已经工作/工作过/我在 X 公司/原本/原职/已就业/已毕业/在职/待岗/失业/全职妈妈/带娃/退休"等已离开校园的语境，或被问"学生还是社会人士"时答"社会人士/工作的/上班的/不是学生"。historical badcase v9mxbgiv：候选人明确回"社会人士，目前待岗状态"，prompt 没有反向触发规则导致 is_student 始终为 null，Agent 反复追问"学生还是社会人士"
