...
三个对外口径为 **7d / 3d / 3d**：消息回看窗口 7 天、会话状态 TTL 3 天、闲置沉淀间隙 3 天。⚠️ 沉淀间隙受 `MEMORY_SETTLEMENT_GAP_DAYS` 环境变量控制，且**不得配成小于 sessionTtl**：gap < sessionTtl 时会话状态仍存活期间即会发生沉淀，同一会话会读回自己刚沉淀的长期记忆（2026-08 生产曾配 1 天，引发蒋强 case 的自沉淀回流，08-31 已改回 3 天）。消费端（prompt 裁决器）已按跨层去重兜底，任何依赖"长期记忆必来自上一段会话"的假设仍不成立。`factsv2:` 的实际 TTL 额外加 12 小时，只是保证 delayed job 先读取再过期的安全余量，不构成新的业务生命周期。
