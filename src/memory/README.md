...
`MessageWindowService` 从 `chat_messages` 读取最近 N 条（硬上限 300），在内存套滚动 7 天窗口（锚点 = 本批之前候选人最后一次开口），再按本批上界与字符数裁剪。窗口比会话状态存活更久是设计意图：状态过期后，回访仍可利用原文重建上下文。

「滚动 7 天」是**相对锚点**而非相对当前时间，实际跨度可能远大于 7 天：连续未获回复的候选人消息（skip_reply、守卫静默、投递失败都不落 assistant 行）会并进同一批，把锚点一路往前推；首次咨询找不到更早的开口时整段不裁。真正的物理封顶是 300 条 + 字符预算。
