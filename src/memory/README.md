...
- 给大模型的 `[用户档案]` 保留字段值、置信度、来源、证据和更新时间
...
1. 取最后一条 user 消息
2. 读取旧的 `sessionState`
3. 用旧 `sessionState.facts` 启动 settlement 检测
...
4. 重新拉品牌表
5. 基于 user 文本重新做品牌 alias hints 和规则高置信识别
6. 构造 extraction prompt
7. 调 extract 模型输出结构化对象
8. 用 `mergeDetectedBrands()` 做品牌 alias 兜底合并
9. 用规则高置信结果补位/标记字段 metadata
10. `saveFacts()` 深度合并回 Redis
...
1. 判断这段旧会话是否已经沉淀过
2. 读取 `lastSettledMessageAt` 之后到旧会话断点之间的消息片段
3. 使用当前 Redis `sessionFacts` 作为已校验/清洗过的结构化事实参考
4. 调 LLM 生成一条摘要
5. 追加到长期 `summary.recent`
6. 如有溢出，压缩进 `archive`
7. 从 `sessionFacts.interview_info` 抽身份字段写入长期 `profile_facts`
8. 更新 `lastSettledMessageAt`
