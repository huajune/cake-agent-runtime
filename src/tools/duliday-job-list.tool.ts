...
function renderWorkTimeSection(wt: any): string {
  ...
  if (hasFullWeekOrRigidSchedule(lines)) {
    lines.push(
      '- **排班硬约束提示**: "每天/做六休一/周一至周日/固定排班"表示工作日也要配合；候选人只做周末或只做晚班时，不能把该岗位说成"周末能排"或"晚班能排"。',
    );
  }
...
- 候选人说"只周末"、"平时下班后"、"只能晚班"、"周四最早 19:30"这类班次限制时，必须把工作时间当硬约束；岗位结果里的"每天"、"周一至周日"、"做六休一"、"每周四/六/日都要给班"、"早开晚结全天时段/05:00-23:00"表示强排班要求，不能解释成任选一天、任选晚班或可只做周末
- "只周末/纯周末"是比"每天"更窄的约束；除非岗位明确写着"只周末/仅周末/可只排周末"，否则看到"每天/周一至周日"必须视为不匹配，不得回复"周末能排"
