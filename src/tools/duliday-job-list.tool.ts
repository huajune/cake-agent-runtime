...
function stripCityPrefixFromStoreName(
...
/**
 * 单条同品牌门店渲染条目。`displayLine` 是固定结构话术，强制要求 LLM 在转述时
 * 把"门店名/距离/薪资"全部带上，避免出现"有肯德基，17-27.5 元、肯德基，
 * 17-27.5 元"这种无法区分的转述（badcase laybqxn4）。
 */
 ...
 */
function buildBrandNearestStoreSummary(
...
- **福利信息主动展示**：本轮要做具体岗位推荐时，必须开 includeWelfare=true；工具返回的福利字段（员工餐/包吃住/餐补/补贴/转正机会/节假日加薪等）若非空，必须在岗位介绍中按工具原文展示。候选人没问也要给——这些是候选人决策的重要因素，藏着等候选人问才答的"偷懒式介绍"会显著降低报名率。historical badcase tt17hs51：候选人问"有员工餐吗"才答出来"备注里写了…可能提供"，岗位介绍那一轮 Agent 主动没带。福利字段为空时按"空头承诺禁忌"如实说"这个我再确认"
...
- **工具未返回的业务事实禁止用训练知识/通识补充**：候选人追问"日结具体哪天到账 / 这家面试是线上还是线下 / 同品牌能不能跨店 / 全职岗还是兼职岗 / 排班是固定还是灵活 / 试用期多久 / 经验要求"等业务规则时，若本轮工具结果没明示该字段，必须说"这个我再帮你确认下"或按 request_handoff 转人工，**严禁**用"一般日结当天结 / 同品牌跨店没问题 / 应该是全职"等通识/经验性回答。historical badcases ro3qwel8（上海肯德基日结其实是次日发，Agent 说"做完当天结"）、3a5ehskt（北京必胜客实际不允许员工跨店，Agent 说"同品牌跨店没问题"）、wq8ctuuu（奥乐齐门店有兼职选项但 Agent 说"固定全职岗"）
- **同会话同字段多次查询结果不一致时相信最新一次**：本会话先后多次调用本工具，若同一门店/品牌/区域的"是否在招/班次/薪资/年龄要求"等字段前后返回不同，必须以**最新一次**结果为准，并自洽地回复；不得既承认上轮的"在招"又承认本轮的"无空缺"造成人格分裂。同时必须用一句衔接（"刚再核了一下，这家目前看下来确实没空缺了"），不要让候选人在前后矛盾间困惑。historical badcases 1q016p80（六姐"置业梦享家店"先说没→后又"刚搜到了，有的"）、qx3vehv5（浦江生活广场店一句话之内"有→暂时没空缺了"）`;
