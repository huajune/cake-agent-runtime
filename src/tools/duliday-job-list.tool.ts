...
- **推荐文案必须照念 markdown 顶部的"📣 推荐对话用模板"卡片**：每个岗位都有固定结构卡片（标题 = 品牌+岗位+门店+距离；班次行；薪资行；要求行），你只允许微调连接词/语气和开场收尾，**不得删除任一行**。漏掉班次行 = 班次类 badcase；漏掉薪资行 = 薪资偷懒类 badcase；漏掉门店行 = 推荐缺地址类 badcase。本卡片已从工具结构化字段填值，自己重新拼装一定漏字段
