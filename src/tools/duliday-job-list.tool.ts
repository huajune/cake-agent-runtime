...
function buildBrandRejectedResult(params: {
  ...
  if (fuzzyConfidence === 'high' && topMatch) {
    ...
    replyInstruction =
      ...
      '回复时用一句轻确认带过让候选人自然听到正确品牌名；不要单独反问品牌指代，' +
    ...
    replyInstruction =
      ...
      '（见 queryMeta.brand.fuzzySuggestions），无法判定指代哪一个。**列出实际候选品牌，用一句话请候选人选择**；' +
...
const inputSchema = z.object({
  ...
  jobCategoryList: z
    ...
    .default([])
    .describe(
      '候选人明确点名的意向工种关键词。**仅用于把 岗位名称/岗位类型/工作内容 匹配的岗位排到结果前面并标注，不做过滤、不会减少召回结果**——召回范围始终由城市/区域/坐标 + 品牌决定。只有候选人**明确点名具体工种**时才填；没点名就留空。\n不要填品类/行业词，应通过品牌参数表达；不要填用工形式词，岗位 laborForm 属性已按会话事实自动硬过滤。',
    ),
  brandIdList: z
    .array(z.number().int())
    .optional()
  ...
  settlementPeriodList: z
    ...
    .default([])
    .describe(
      '结算周期筛选，取 salary_period 字典名称：日结算、周结算、月结算、半月结；其他值以实际字典为准。仅当候选人**明确点名要某种结算周期**时填；平时留空。注意结算周期是薪资属性，不是岗位工种，不要塞进 jobCategoryList。',
    ),
  ...
  requireAccommodation: z
    ...
    .optional()
    .describe(
      '候选人明确要求岗位包住、提供住宿或有宿舍时传 true。传入后工具自动：①解除距离上限改全城召回（通勤距离对包住岗位不成立，坐标只用于显示距离）；②只保留福利里公司提供住宿或有住宿补贴的岗位，其余剔除并在 queryMeta.accommodationFilter 说明。候选人仅询问是否提供住宿时不算硬需求，不要传。',
    ),
  candidateScheduleConstraint: z
    .object({
      ...
      availableWindow: z
        ...
        .optional()
        .describe(
          '候选人可上班的具体时段，start 与 end 均使用 HH:MM 格式；结束时间允许 24:00，end 不晚于 start 时表示跨午夜至次日。传入后班次必须整段落在该时段内才保留（组合班次需全部落入；窗口式排班按重叠时长判）。候选人给了具体钟点区间时必须传，不要降级成 onlyEvenings。',
        ),
    })
    .optional()
    .describe(
      '候选人班次硬约束。传入后，工具会按岗位 workTime 语义判定是否兼容；不兼容岗位会从结果中移除并在 queryMeta.scheduleFilter 里说明剔除数量。候选人明确限制可上班的日期、时段或每周天数时必须传。注意方向：解释某班次无法出勤属于**排除**该班次，不得据此传 onlyEvenings/onlyMornings。onlyWeekends/onlyEvenings/onlyMornings 断言的是**排他性**，必须有候选人只能在该时段出勤的原话依据；明确寻求周六或周末工作的求职意向也按 onlyWeekends=true 处理。工具会按原话校验，缺依据直接拒绝本次查询。候选人仅表示某时段可以出勤或接受排班属于可用性，不构成排他性约束，不得据此传这三个字段。班次约束跨轮累积：既有约束未被候选人取消或纠正时必须继续携带，不得用新增约束替换。',
    ),
});
...
function buildJobIdentitySummary(
  ...
  if (counts.unspecified > 0) {
    lines.push(
      `- ⚠️ **${counts.unspecified} 个岗位的社会身份要求字段为空**（本轮部分岗位：${missingDataJobs
        .map((job) => job.label)
...
function buildJobAgeScreeningSummary(
  ...
  if (counts.hard_reject > 0) {
    lines.push(
      `- **hard_reject 岗位默认不得推荐**：不得把年龄硬拦截的岗位当作可选项展示给候选人——选中后会被 precheck 打回，浪费候选人意向。仅当候选人主动点名该品牌/门店时可提及，且必须同时明确说明年龄不符、无法约面。存在 pass/boundary 岗位时只推荐 pass/boundary 的。`,
    );
  }

  if (boundaryExamples.length > 0) {
    lines.push(
      `- 本轮 boundary 岗位（仍可推进，不得按严格年龄视同无岗）：${boundaryExamples
...
| 查询某区域岗位 / 候选人提供所在区 | **默认按就近处理**：区/县名先 geocode 拿坐标走 location 距离召回；城市没把握就 city 留空让 geocode 三态判定，不要先反问候选人。区内硬约束的精确过滤见下方 regionNameList 条 |
| 查询附近岗位 / 提供商圈或地标 | 先 geocode 拿坐标 → 传 location 半径；若结果 ≤ 1 条**必须**去掉 location 重查全市 |
| 用户接受了某门店但要换条件 | **先在 [会话记忆] 里查这门店所在的 region**，用 regionNameList 重查；不要直接拿口语门店名传 storeNameList |
| 候选人要求查看其他品牌 | **不带 brandIdList 重查**当前区域，对比之前已展示的 brand 集合，告诉用户除了已推过的还有什么 |
...
- **1 条** 且候选人要求其他选择：反常信号，**必须再放宽 1 个维度重查**（去掉 location / 扩大半径 / 去掉某个 brand/category filter），不得用 1 条结果直接答"暂时没空缺"
- **≥ 2 条**：可以基于结果回复，无需扩面
...
- 候选人明确限制出勤日期、班次、每周天数、休息安排或起止时刻时，必须把工作时间当硬约束。岗位要求全周出勤、固定多日出勤或覆盖全天开闭店时段时，除非岗位明确允许候选人对应的窄时间窗，否则视为不匹配；不得将强排班要求解释成自由任选出勤日或班次，也不得擅自承诺可协调。
...
- **regionNameList = 区级行政区名的精确过滤，不是就近召回**（后端对库里的区级 storeRegionName 做精确字符串匹配）。三条推论：
  - 候选人提供所在区时，默认视为**就近信号**：geocode 成坐标走 location，避免精确过滤漏掉邻区更近的门店。regionNameList 仅用于候选人明确要求仅在某区内的硬约束，或从已知门店扩展回其所在区重查。
  - **只接受区/县级行政区全称**：乡镇/街道/片区名与商圈/地标/详细地址精确匹配必然 0 条、且绝不代表该片区没岗，必须先 geocode 解析成"区级 district + 坐标"再查
  - 区名简称也先 geocode 拿规范全称，避免对不上后端区级实名
- **未确认城市禁默认**：[本轮解析线索] 与 [会话记忆] 都未给出城市时，禁止默认任何城市做查岗或品牌承诺；候选人明确品牌但未给城市时，必须先简短确认求职城市
...
- **薪资/班次的补充纪律**：任何场景引用阶梯薪资必须保留"基础 + 阶梯规则"原文，禁止省略阶梯规则或简化为估算金额；薪资数字必须带单位（元/小时、元/月，多岗混排逐岗标注）；确认候选人能否接受班次前必须告知班次时间；工作时间字段缺失时如实说明班次尚需门店确认，不得编造，也不得把"面试时间"当"上班班次"
- **福利信息主动展示**：普通福利字段（员工餐/包吃住/餐补/补贴/转正机会等，**不含保险/社保**——敏感政策按全局规则不主动提及）非空必须按工具原文展示，候选人没问也要给；为空时按"空头承诺禁忌"如实说明该福利尚未确认
- **挑选式开场禁忌**：直接展示 1~2 个最匹配岗位的完整详情，不要只列岗位名称就等待候选人挑选
- **岗位卡片必须紧凑**：单岗信息集中在 1-2 段内（行内可用顿号/逗号分隔），不要用段间空行拆成多段——后置消息切分器按空行拆成独立微信消息，候选人会连收 6-8 条碎片
- **工作内容**：回答具体工作内容前必须读本轮岗位详情的工作内容字段，未返回时按缺字段规则补查；出现行业短语时用一句口语化解释展开，不要原样复读
...
- **品牌/区域分布判断必须基于本工具结果**：候选人说出品牌时，不得未查询就对在招情况作确认，需先在当前已知范围验证在招；各区域有无在招的分布结论也必须先查。未查前只能说明准备查询
- **具体岗位/门店推荐必须带位置**：候选人给了商圈/地标/街道/详细地址/位置分享/经纬度等具体位置线索、且本轮要输出具体岗位或门店推荐时，必须先 geocode 或使用位置分享经纬度再调用本工具；不要因对方未明确要求就近推荐而跳过。学校、校区、学院、小学部等地点名只代表位置，不代表学历
...
- **距离数字只能引用本轮工具结果，严禁编造**：距离只能来自本轮结果的距离字段（推荐卡片 / brandNearestStores 的 km 数）；本轮没传 location、结果无距离数据时**禁止**凭地名印象估算公里数——已有候选人坐标就带 location 重查，拿不到就不提供距离数字，并如实说明尚未确认
- **location 坐标禁止凭记忆自编**：坐标只能逐位复制自本轮/历史 geocode 结果或候选人位置分享；改半径复查时原样复用上次完整坐标（小数位不得截断改写），拿不准就重新 geocode——自编坐标会把搜索圈画到错误圆心上
...
- **候选人明确要包住/住宿 → 传 requireAccommodation=true**：通勤距离对包住岗位不成立，工具会自动解除距离上限改全城召回、只留提供住宿或有房补的岗位，距离照常显示由候选人决定；**不得**按附近范围的空结果判断全城没有住宿岗位。全城仍为空才如实说没有包住岗，并问是否接受不包住再走常规召回
- **点名品牌豁免距离上限——0 条先放宽复查再下结论**：候选人点名的品牌不受 max_recommend_distance_km（约 10km）约束（阈值只管 Agent 主动推荐）。距离内查得 0 条时必须放宽再查一次：location.range 放大到 30000 并保留品牌 + 城市/坐标（**不要只去掉 range**——保留坐标缺省 range 会被阈值兜底拉回；全城查则去掉整个 location）。放宽后查到较远门店 → 如实告知距离让候选人决定，**严禁**把"超距离"说成"没有/暂无在招"；整城仍 0 条才说"该品牌你所在城市暂无在招"，再按无岗动作链收口
...
- **禁止推断品牌地理分布**：本工具只确认查询范围内有无在招，不掌握品牌全国门店分布；0 条只能说明查询范围内暂未查到该品牌在招岗位，**严禁**据此推断品牌主要在哪些区域开店
- **Agent 自推岗位不适用品牌锁死**：如果候选人并未主动指定品牌，而是你上一轮先推荐了某品牌/门店，候选人只是接受推荐或补收资资料，则该品牌不是硬性 brand intent。后续发现该岗位年龄/性别/班次/学历等条件不匹配时，必须先去掉 jobIdList / brandIdList / brandAliasList，保留候选人的位置、年龄、身份、时间窗等硬约束重查，并基于新结果推荐可匹配岗位；不要直接 request_handoff，也不要用"明确品牌意向"规则阻止换岗自救
- **工时长度反查**：候选人表达偏好较长工时时，必须基于返回的工作时间字段重新筛选；若结果集仍以短班为主，先如实说明附近岗位以短班为主，再询问是否扩大区域，不得将短班描述为满足长工时偏好
- **首次推荐必须把关键要求 + 工作班次时间随岗位信息一起告知**，让候选人自行判断；严禁推完岗位再逐个追问个人条件去做比对
- **推荐文案只输出工具结果顶部的实际岗位卡正文，严禁自行添加“模板/示例/内部说明”等元标题**：每个岗位四行（标题 = 品牌（门店）- 岗位，距离；班次行；薪资行；要求行），**不得删除或合并任一行**。薪资行已由卡片按岗位类型给全关键项（兼职阶梯岗：基础时薪 + 各档门槛 + 计算口径 + 结算；兼职无阶梯岗：时薪 + 法定节假日/加班 + 结算；全职岗：综合薪资 + 法定节假日/加班 + 奖金项 + 结算），推荐时照卡片说，不要再把详情块里的其他薪资字段塞进薪资行；候选人追问薪资细节时再按详情块展开。班次行、要求行的取值结合该岗位下方详情和备注组织完整。漏掉班次行、薪资行或门店行都属于不合格推荐
...
- **无岗与推荐不满意分流**（互斥顺序收口）：
   ...
   6. **结果非空但全部与硬约束冲突** = 视同 0 条有效，先放宽一维重查；仍无匹配才按真实无岗收口。年龄判断必须沿用 precheck 弹性口径：处于年龄上下限弹性边界时，用 duliday_interview_precheck 复核，不得直接判无岗
   7. 新搜索无匹配时回看 [会话记忆]「上轮候选岗位池」，有潜在匹配用 jobIdList 精查后再推荐
- **包餐/餐补硬偏好**：候选人明确要求包餐或餐补时，将其视为硬约束；不得用门店周边餐饮便利替代，也不得继续收资。继续推荐须本轮核对福利字段，无匹配就如实说明并结束本轮，不因真实无岗直接拉群。
- **面试相关字段**：推进面试时优先读工具结果中的「约面重点」；工具没明确时间不得编造；相对当前时间已过期的日期限制视为历史备注，不得当作当前规则输出

## 空头承诺禁忌
- 工具未返回某福利字段时不得说"有 / 没有该福利"；候选人需要该答案才能决定时，当轮按 request_handoff（reasonCode="salary_admin_inquiry"）转人工——不得承诺找人确认却不转人工。**唯二例外是员工餐和住宿**：福利速览已按运营口径把这两项的"未配置"标成"❌ 无"，直接照速览答、不得为此转人工；交通补贴/晋升/保险等其它福利未配置仍按"未明确"处理
- **阶梯薪资只能复述工具原文的门槛与单价，两个口径缺口禁止自行填空**：① **累计周期**——工时累计**按月结算、每个自然月清零重新累计**（平台口径），候选人询问累计周期时，必须明确每月重新累计、次月从基础档起算，不得声称档位跨月延续或把确定的平台规则推给门店；② **计算基数**——升档后是全部工时按新档还是仅超出部分按新档，**以阶梯明细每档的括注原文为准**（"超出后所有工时按照新的薪资标准计算" = 升档后全部工时按新档；"超出部分按照新的薪资标准计算" = 仅超出部分按新档），照括注原文答，**严禁**脱离括注按个税式分段模型自答或断言"全部按最高档"；括注确实为空时才当轮按 request_handoff（reasonCode="salary_admin_inquiry"）转人工确认。两类说错都会导致候选人结算时工资对不上
- **健康证口径必须两段一起给**：岗位分「面试前须持证」与「入职前办妥」两档；属后者时，必须同时说明面试前无需持证、入职前仍须办妥。时点以本轮健康证字段为准，不得增加岗位数据未明确的环节或持证要求。
- 历史助手回复说过的门店事实不能当本轮事实复述；本轮要给候选人新的具体推荐时，必须以本轮工具结果为准；只有 [当前焦点岗位] 等记忆字段是稳定的，可以直接承接
- **工具未返回的业务事实禁止用通识补充**：候选人追问的主题在岗位字段中确实没有答案时，当轮按 request_handoff（reasonCode="salary_admin_inquiry"）转人工，**严禁**凭经验作答。**先查字段再转**：排班固定或灵活看「工作时间」段的排班周期/排班类型；发薪日期与工资归属月看「结算周期」；面试线上或线下看「面试方式」；是否有试用期看对应字段。字段已有答案时按本轮结果回答，不得转人工；试用期时长未返回时仍按缺字段处理。
- **学生安排只服从岗位数据，且资格通过≠预约已通过**：工具明确写不接受学生就不得推荐；写接受/学生优先则继续；岗位社会身份要求字段为空时**不得按"不限身份"放行**：该字段在岗位后台是必填项，为空属于数据缺失，不得对候选人说"这个岗不限身份/学生也可以"。候选人身份未知时可继续校验其余条件；候选人已明确是在校学生、且本轮只剩这类字段为空的岗位可推时，按 request_handoff（reasonCode="identity_age_exception"）转人工，reason 必须写明"岗位社会身份要求字段为空（疑似岗位数据缺失），无法确认是否接受在校学生"，并如实告诉候选人需要跟门店确认、稍后答复。除此之外不得凭空说"需要跟店里确认"、不得因此 request_handoff 或声称已联系门店，年龄/学历/常识不能替代岗位数据。资格通过只代表可以继续：约面阶段必须保持候选人原话身份（历史明确填学生就传 candidateIsStudent=true）走 duliday_interview_precheck，只有 booking 返回 success=true 才能说已提交/已预约，严禁只调本工具就声称正在或即将提交预约。
- **门店运营状态禁编造**：本工具只确认是否有在招岗位，**不掌握**营业/装修/关店/搬迁/招满等状态；结果为空只能说明当前查不到在招岗位，**严禁**推测关店或搬迁；候选人坚持要实际状态时按 request_handoff 转人工。
- **同字段多次查询不一致时以最新一次为准**：前后返回不同时按最新结果自洽回复，简短说明信息已经重新核对并更新，不得同时保留相互矛盾的结论`;
          ...
          if (jobs.length === 0) {
            ...
            if (fuzzyConfidence === 'high' && topMatch) {
              ...
              replyInstruction =
                ...
                topMatch.brandName +
                '** 高度同音回指（见 queryMeta.brand.fuzzySuggestions[0]）。**直接按该品牌继续推进**，' +
                '回复时用一句轻确认带过让候选人自然听到正确品牌名；' +
                '**不要单独反问品牌指代，不要照念 noMatchScript，不要调 invite_to_group。' +
              ...
              replyInstruction =
                ...
                '分数差过小无法判定指代哪一个。**列出实际候选品牌，用一句话请候选人选择**；' +
                '不要直接答"没查到"，不要照念 noMatchScript，不要调 invite_to_group。';
            } else {
              outcome = '未找到符合条件的岗位';
              replyInstruction =
          ...
          if (
            ...
            return buildToolError({
              ...
              replyInstruction:
                ...
                '，不得主动要求候选人放宽时间限制；只有候选人后续主动改口放宽，才按新时段重新查岗。' +
            ...
            if (laborFormFilterResult.excluded.length > 0 && jobs.length === 0) {
              ...
              return buildToolError({
                ...
                replyInstruction:
                  ...
                  '不得将其他用工形式的岗位描述为符合候选人的用工形式要求，也不得凭通识承诺有岗。' +
                  noMatchFollowUp,
                details: {
                  queryMeta: {
                    laborFormFilter: {
