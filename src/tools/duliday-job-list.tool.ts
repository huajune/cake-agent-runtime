...
- **明确品牌意向时不静默换品牌**：候选人明确说出"找成都你六姐 / 我想去肯德基"时，brand 必须进 brandIdList；该品牌在范围内 0 条岗位时，先告知"暂时没有 X 品牌的岗位"，按下面"无岗时的动作链"收口拉群，**不得**主动反问"看看其他品牌吗"，更不得默默换成其他品牌推荐。本工具会在 brandAliasList 非空时硬过滤结果到该品牌；如果你想跨品牌推就别把 brandAliasList 填上去——但候选人明确品牌意向时禁止省略该字段
...
- **推荐文案必须严格按"📣 推荐对话用模板"卡片的固定格式输出**：每个岗位四行（标题 = 品牌（门店）- 岗位，距离；班次行；薪资行；要求行），**不得删除或合并任一行**。每行的具体取值须结合该岗位下方详情和备注组织完整信息——例如薪资行须包含备注中的阶梯/节假日等补充薪资，而非只写结构化字段的基础时薪。漏掉班次行 = 班次类 badcase；漏掉薪资行 = 薪资偷懒类 badcase；漏掉门店行 = 推荐缺地址类 badcase
- **无岗时的动作链**：候选人范围内 0 条结果时，按以下顺序收口：
   ...
   6. **结果非空但全部不匹配**：返回的岗位全部与候选人当前硬约束冲突（如候选人要白天班但结果全是夜班、候选人年龄超出所有结果的年龄要求），视同"0 条有效结果"，必须至少放宽一个维度（优先清空 jobCategoryList）重查一次；仅在放宽后仍无有效匹配时，才走上面的兜底路径
