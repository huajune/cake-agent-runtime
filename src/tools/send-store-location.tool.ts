...
export function buildSendStoreLocationTool(
  ...
  return (context) =>
    tool({
      description: `向候选人发送当前门店的企微位置消息。

使用场景：
1. 已经明确具体岗位或门店后，候选人追问“地址在哪”“发个定位”“怎么过去”“导航怎么走”。
2. 必须确认目标门店唯一明确后再调用；如果当前在聊多个岗位，先确认具体是哪家店。

执行效果：
- 工具会直接向当前企微会话发送位置消息（不是文本）。
- 成功后通常只需要再补一句简短确认。若返回 _fixedReply，必须原样输出。`,
