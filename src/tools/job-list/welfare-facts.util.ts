...
export function renderWelfareFactsBanner(facts: WelfareFacts): string {
  ...
  lines.push(
    '> ⚠️ 本速览即员工餐/住宿的**最终事实**：候选人问包不包吃住/有没有员工餐时按上面各项直接答（含"❌ 无"），**不得**将这两项描述为尚未明确或仍需确认，也**不得**为此调 `request_handoff`。「未配置即为无」**只适用于员工餐和住宿这两项**；交通补贴/晋升/其它福利/保险未配置时仍是"未明确"，不得替公司断言"没有"。',
  );
