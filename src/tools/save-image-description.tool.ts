...
- 图片：像结构化摘录一样保存图片上的关键事实，不要只写泛泛概括；招聘平台截图/岗位卡片必须尽量保留岗位/品牌/门店/地点、薪资及阶梯规则、班次时间、工作内容、工作要求、福利、联系人/来源等可见信息。信息多时用短句或分号组织，允许 3-6 句，优先完整准确而不是过度压缩。若是 Boss直聘岗位页/岗位卡片，岗位标题中方括号内的纯数字是品牌ID；必须保留完整标题，并在描述中用 "品牌ID：" 加原始标题中的实际数字记录，不要当成岗位ID或普通编号。
...
- 表情：只写情绪或动作短语，控制在 4-12 个字；不要描述角色外观、颜色、姿势细节，也不要猜测台词或意图。
只提取事实信息，不要添加评价或建议。
...
- fields：图上的关键值逐个列出。岗位截图上的电话/年龄要求/薪资是发布方的（ownership=publisher）；候选人自己的简历/证件上的信息是 candidate；聊天截图里分不清归谁就 unknown。岗位页中明确标注为设备用户自身位置的地址归候选人，key 用 candidate_address；不要与岗位地址混淆。身份证号/证件号不要写进 fields。`;
  ...
  return (context) => {
    return tool({
      ...
      execute: async ({ messageId, description, kind, fields }) => {
        ...
        // 证件号脱敏（红标 2，）：sheet 的 rawDescription 由 finalize 内部
