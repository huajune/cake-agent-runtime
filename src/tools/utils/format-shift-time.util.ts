 ...
 * 2) 选择关系（可选其一/必须全做等）— 由数据形态推断
 * 3) 工时长度（约 X 小时 / 短班）— 由 endTime - startTime 计算（处理跨午夜）
 * （海绵2.0 新结构不再下发星期分配，星期维度已随旧结构一并退役）
 *
 * 约定：
 * - 无具体时段且非灵活排班（仅有 weekAndMonthWorkTime 等聚合数据）→ 返回 null，
 *   调用方按 null 选择"不显示该字段"，不补 fallback 文案
 * - 灵活排班且无可用时段 → 返回弹性概要（"弹性排班，按门店实际安排…"），非 null
 */
 ...
 * 全部要出勤），Agent 却答"班次三选一"，候选人接受其一后才发现口径不符。根因：现网
 * 只下发短标签，判据若只匹配整句形态（`/所有/`、`/其中一个/`）必然脱靶——2026-07-30
 * 14:00-17:00 生产窗口实测 138 个岗位块：`固定排班` 127、`组合排班制` 8、`灵活排班` 3，
 * 整句形态 0 条，此时 all_required 与 pick_one 两条判据双双恒不命中，只有 `灵活排班`
 * 因为短标签自带"灵活"二字侥幸命中。
 *
...
function formatDurationHint(start: string, end: string): string {
  ...
  // 按连续工时输出"全天班约19小时"会让候选人以为要连上一整天而流失
