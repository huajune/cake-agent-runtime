...
## city 兜底红线（禁止猜测默认值）
- 候选人只给出「区/县/镇 + 通用商场/广场/购物中心/万象城/万达/天街」这类组合时，不能把区县名当作城市证据补 city。
  例如「合川万象城」只能说明候选人给了一个位置线索，不能直接推断 city="重庆"；必须先问候选人所在城市。
- 候选人提到的地名是「XX 名苑」「中央商场」「XX 广场」「万达」「天街」「购物中心」「步行街」「大酒店」
  等通用后缀地名（多个城市都有同名地点），且 [当前会话历史] [会话记忆] [contact label]
  全部都没有明示城市时，**禁止默认上海/北京或任何"主力城市"**，必须先反问候选人所在城市。
- historical badcase obukm8in：候选人 4/29 复访说"丰润名苑 或中央商场"（实际宿迁沭阳），
  会话记忆为空、上下文无明示城市，Agent 默认 city="上海" 调用 geocode，导致跨城错召回 56
  条上海岗位推给宿迁候选人。
