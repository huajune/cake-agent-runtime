import { z } from 'zod';
import { CANDIDATE_FACT_OPERATIONS } from '@resolution/evidence/claim.types';
...
export const FormAnswerInputSchema = z
  .object({
    ...
    operation: z
      .enum(CANDIDATE_FACT_OPERATIONS)
      .optional()
...
/** 兼容模型偶尔把数组整体 JSON 字符串化；公开结构仍只有 FormAnswerInput[]。 */
export const FormAnswersInputSchema = z.preprocess((value) => {
...
}, z.array(FormAnswerInputSchema).max(20));
