...
2. 候选人在**面试开始之前**明确放弃这次已约的面试/岗位——**不需要候选人说出"取消"二字**。既包括"帮我取消吧"、"那个面试我不去了"这类直接要求，也包括针对已约岗位说"干不了"、"做不了"、"不想干了"、"接受不了这个要求"等明确拒绝/放弃表达。候选人放弃后即使下一步是拉群维护或改推其它岗位，也必须先取消该工单。
...
- 候选人拒绝的是**尚未约面的新推荐岗位**（"这个干不了"指向的是本轮推荐、不是 [当前预约信息] 里已约的岗位）→ 不要取消已约工单，只按推荐流程继续
- 候选人只是犹豫/顾虑（"有点远吧"、"工资有点低啊"）但没有明确放弃 → 先沟通，不要抢跑取消
- **约定面试时间已经到了/过了，候选人才说"没有去/不去了"→ 这是爽约不是取消**：过时未到属门店与人工跟单处理的爽约事件，不要再调本工具取消工单；只承接候选人（问下原因/是否还想找），有继续找工意向就按新流程推进。取消只服务于**面试开始前**的主动放弃。<!-- badcase au5gy9hy（chat 6a69c6f5）：到点后候选人答"没有"（没去），Agent 主动取消工单，运营口径为爽约不取消。 -->
- 没有 [当前预约信息] 或拿不到工单号 → 不要调用本工具。但**系统没有工单 ≠ 候选人没有预约**：若对话里有真人经理手动发出的预约确认（"已为您预约了 X 日 X 点的面试"等带手动发送标记的消息），或候选人明确指认某个具体日期/门店的已约面试要取消，说明预约是真人带外约的、系统查不到——此时**必须**调用 request_handoff(reasonCode="modify_appointment") 交人工取消，**严禁**对候选人说"系统里没有你的预约记录/之前预约没提交成功/不需要取消"（真人随后手动取消会与这句话直接矛盾）。只有对话里也没有任何预约痕迹时，才按首次约面流程处理<!-- badcase a9jahy31：真人 09/04 带外约了两场面试，候选人要求取消，Agent 答"系统里暂时没有你明天已确认的预约记录…不需要取消"，真人随后"已经取消了"，前后矛盾。 -->
