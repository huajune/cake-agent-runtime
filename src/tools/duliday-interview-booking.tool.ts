...
import { validateInterviewTimeAgainstSchedule } from '@tools/duliday/interview-booking-time.validator';
...
- 返回 errorType="booking.screening_mismatch" 时：候选人的回答命中了岗位硬筛选的不合格信号（例如在"专业（非新媒、食品）"里明确答了"我是食品专业/学食品"，在"周四六日都能上班吗"里答了"不一定"）。**严禁**对这位候选人重试本工具或换字段重填；按 _replyInstruction 的指引婉拒并调用 invite_to_group 维护，或 request_handoff 转人工。注意："食品类健康证/食品健康证/餐饮健康证"是健康证类型，不是专业答案`;
      ...
      execute: async ({
        ...
        if (!isLikelyRealChineseName(name)) {
          return markBookingFailed(
            context,
            buildToolError({
              errorType: TOOL_ERROR_TYPES.BOOKING_INVALID_LEGAL_NAME,
              outcome: '预约未提交（姓名校验未通过）',
              replyInstruction:
                '不要直接告诉候选人"这个名字不行"或暴露校验细节。用招募者口吻说"我帮你登记一下，能告诉我你的真实姓名吗？我们门店那边登记需要本名"，等候选人提供合法姓名后再调用本工具。',
              details: {
                providedName: name,
                detailedReason: `name="${name}" 看起来不像真实姓名（要求 2-4 个汉字、不含拉丁字母/数字/emoji/装饰符号）。微信昵称、emoji 名、自我介绍式昵称都不行。`,
              },
            }),
          );
        }
        ...
        if (screeningFailure) {
          ...
          return markBookingFailed(
            context,
            buildToolError({
              errorType: TOOL_ERROR_TYPES.BOOKING_SCREENING_MISMATCH,
              outcome: '岗位筛选未通过',
              replyInstruction:
              ...
              details: {
                ...
                detailedReason: `候选人对"${screeningFailure.label}"的回答"${screeningFailure.answer}"未通过岗位筛选（命中不合格信号"${screeningFailure.matched}"）。严禁继续提交预约。`,
              },
            }),
          );
        }
        ...
        try {
          ...
          if (interviewTimeValidation) {
            return markBookingFailed(context, interviewTimeValidation);
          }
