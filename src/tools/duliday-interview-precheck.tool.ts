...
// 子句级锚定匹配：候选人常把多个字段挤进一条消息（badcase 6a448d09"男 已经工作了"），
// 整句锚定会被无关前缀击穿；按空白/标点切子句后逐段匹配，并容忍"是/我是/还是"等口语前缀。
// 疑问尾词（吗/呢/吧）刻意排除在后缀外，"已经工作了吗"这类反问不构成自认。
const STUDENT_CLAUSE_RE =
  /^(?:我|本人)?(?:现在|目前)?(?:还)?(?:是)?(?:学生|大学生|在校生|高中生|在读|在上学|还在上学)(?:的|哈|哦|呀|啊)?$/u;
const SOCIAL_CLAUSE_RE =
  /^(?:我|本人)?(?:现在|目前)?(?:是)?(?:社会人士|社会人|不是学生|非学生|已经?(?:毕业|工作|上班)(?:了|啦)?|上班族)(?:的|哈|哦|呀|啊)?$/u;

function matchIdentityStatement(text: string): '学生' | '社会人士' | null {
  const compact = normalizePolicyText(text);
  if (
    /身份(?:[（(]学生\s*[/／]\s*社会人士[）)])?\s*[：:]\s*学生|我(?:现在)?是学生|(?:本科|大专|高中|硕士|研究生|博士)在读|我还在读|我在上学/u.test(
      text,
    ) ||
    STUDENT_CLAUSE_RE.test(compact)
  ) {
    return '学生';
  }
  if (
 ...
 * 对其回答"是的"语义不定，必须返回 null。badcase 6a448d09：候选人对确认问句
 * 回答"是的"，白名单不认，导致同一问题被追问 3 次。
 */
function detectIdentityConfirmQuestion(text: string): '学生' | '社会人士' | null {
...
function findLatestExplicitIdentity(messages: unknown[]): '学生' | '社会人士' | null {
  ...
  // 悬挂的单值确认问句：下一条候选人纯肯定应答（"是的"）即构成身份自认。
  // 任何新的 assistant 消息都会重置它，肯定应答只会绑定到紧邻的确认问句。
  let pendingConfirm: '学生' | '社会人士' | null = null;
  for (const message of messages) {
    if (!message || typeof message !== 'object') continue;
    ...
    if (candidateMessage.role === 'assistant') {
      pendingConfirm = detectIdentityConfirmQuestion(text);
      continue;
    }
    if (candidateMessage.role !== 'user') continue;
    const stated = matchIdentityStatement(text);
    if (stated) {
      latest = stated;
      pendingConfirm = null;
      continue;
 ...
 *
 * badcase 6a448d09：白名单不认候选人回答时，模型在 4 个 turn 里把同一个身份问题
 * 问了 4 遍（"再回一句社会人士"）。该统计供 identityFieldGuard 升级判断：
 * 追问 ≥2 次且候选人已作答仍无法核验 → 停止追问、强制转人工。
 */
function summarizeIdentityAskRounds(messages: unknown[]): {
  askCount: number;
  ...
  for (const message of messages) {
    ...
    if (record.role === 'user' && askCount > 0) {
              ...
              nextAction === 'ready_to_book'
                ...
                : nextAction === 'collect_fields'
                  ? identityAskEscalated
                    ? '预检卡在"身份"字段：已向候选人追问过 2 次且其已作答，系统仍无法核验。禁止再次追问身份、禁止重复调用本工具；本轮必须调用 request_handoff（reasonCode="system_blocked"，reason 说明身份字段无法核验需人工登记），并告知候选人资料已记录、人工会尽快完成登记。'
              ...
              studentIdentityMustBeExplicit && checklist.missingFields.includes('身份')
                ? identityAskEscalated
                  ? {
                      ...
                      mustHandoff: true,
                      reason:
                        '已就"学生还是社会人士"追问过 2 次且候选人已作答，系统仍无法核验身份。禁止再重复追问；本轮直接调用 request_handoff(reasonCode="system_blocked") 转人工登记。',
                    }
                  : {
