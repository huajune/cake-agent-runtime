...
- candidateLaborForm：候选人本轮明确表达的用工形式或对"是否暑假工"的回答，如"暑假工"、"不是暑假工"、"长期兼职"。**只能转述候选人原话，禁止代答**。暑假工状态默认是"否"，年龄、学生身份、在暑假期间求职均不得触发追问或改成暑假工；只有候选人原话主动明确说自己要找/要做/是暑假工时才切为"是"。**暑假工身份有粘性**：候选人一旦说过自己是暑假工，系统会按其原话与会话事实持续判定，省略本字段、说"长期也可以"或反向传值都不会解除拦截；只有候选人自己明确改口"不是暑假工"才会解除。工具会用候选人消息原文核验本字段，模型代填无效。**严禁**为通过校验把暑假工候选人按"非暑假工/长期"登记或建议候选人隐瞒暑假工身份——如实告知暂无暑假工岗位并维护候选人
...
- **identityFieldGuard.verifyIdentityFlip=true 时**（候选人自认学生、被告知不要学生后策略性改口社会人士）：首次改口不采信，必须先核实一次——告知"如实填写不影响推荐其它岗位、还在读也没关系"，候选人再次明确确认已毕业/社会人士后才继续；**严禁**暗示"改成社会人士就能报"，严禁未经确认按社会人士登记。若候选人已明确说明先前是"填错了/选错了/填顺手了/说错了"，随后清晰说明"已毕业/社会人士"，这是纠错而非策略性改口，应直接按最新身份继续，不得重复追问
...
- **暑假工临时口径（2026 暑期临时规则）**：暑假工状态默认是"否"，不得根据年龄或学生身份主动追问。只有候选人原话主动明确要暑假工时，才切换为暑假工；此时只有岗位结构化字段明确为暑假工（basicInfo.partTimeJobType=暑假工，或历史数据 basicInfo.laborForm=暑假工）才可继续约面，岗位未明确为暑假工时默认不招暑假工。候选人说"长期也可以"不等于"不是暑假工"，只有其明确原话"不是暑假工"才能解除暑假工粘性。该规则后续暑期结束可删除。
...
const SUMMER_SELF_DENY_PATTERN = /不是暑假工|非暑假工|不做暑假工|不当暑假工|不是暑期工|非暑期工/;
 ...
 *
 * 这是身份粘性的事实来源之一：badcase 6a50827c 里模型在下一轮 precheck 把
 * candidateLaborForm 字段删掉，守卫就忘了候选人一分钟前刚说过"暑假工的"。
 * 原话信号不依赖模型入参、不受提取延迟影响，按消息顺序取最后一个决定性信号。
 */
...
/**
 * 解析候选人暑假工身份（**非对称粘性**，防模型洗身份）。
 *
 * 优先级与方向约束：
 * 1. 候选人原话的确定性信号（最新一条）双向可信——明确改口"不是暑假工"才能清除身份；
 * 2. 记忆（高置信/会话事实）已确认暑假工 → 粘住，**模型显式入参无权反向覆盖**
 *    （badcase 6a50827c：模型为过审删参数、并计划按"非暑假工"登记）；
 * 3. 没有候选人原话或可信记忆信号时默认不是暑假工；模型显式入参不能单独改变身份。
 */
function resolveSummerWorkerIntent(params: {
  highConfidenceLaborForm: unknown;
  sessionLaborForm: unknown;
  messages: unknown[];
}): SummerWorkerIntent {
  const conversational = detectSummerWorkerSignalFromMessages(params.messages);
  if (conversational === 'not_summer_worker') return 'not_summer_worker';

  const memory =
    normalizeSummerWorkerIntent(readHighConfidenceValue(params.highConfidenceLaborForm)) ??
    normalizeSummerWorkerIntent(params.sessionLaborForm);

  if (conversational === 'summer_worker' || memory === 'summer_worker') {
    return 'summer_worker';
  }
  return 'not_summer_worker';
}
...
// 身份原话识别 / 追问统计 / 拒后改口核实统一走 @tools/shared/identity-statement.util，
...
function hasExplicitIdentityEvidence(identity: string, messages: unknown[]): boolean {
  return findLatestExplicitIdentityEvidence(messages)?.identity === identity;
}
 ...
 *
 * badcase 6a50827c：模型为推进约面凭空传身份入参。完全"无出处即丢弃"会误伤
 * 消息窗截断等合法转述场景（引发反复追问），所以口径收敛为：记忆里已有明确
 * 身份、显式入参要反着改、且候选人本轮原话找不到改口证据 → 判为模型代答，丢弃。
 * 候选人真实改口时原话里必然有对应词（"其实我是学生"/"已经毕业了"），不受影响。
 */
function isStudentClaimUnsupportedFlip(params: {
  claimed: string;
  messages: unknown[];
  sessionIsStudent: unknown;
  highConfidenceIsStudent: unknown;
}): boolean {
  const claimedStudent = params.claimed === '学生';
  const sessionRaw = params.sessionIsStudent;
  const sessionValue =
    sessionRaw !== null && typeof sessionRaw === 'object' && 'value' in sessionRaw
      ? (sessionRaw as { value?: unknown }).value
      : sessionRaw;
  const hcValue = readHighConfidenceValue(params.highConfidenceIsStudent);
  const memoryValue =
  ...
  // findLatestExplicitIdentityEvidence 当成候选人改口。
  const latestExplicitIdentity = findLatestExplicitIdentityEvidence(params.messages);
  if (latestExplicitIdentity !== null) return latestExplicitIdentity.identity !== params.claimed;
...
function applyCandidateFieldOverride(
  ...
  // 当前轮确定性抽取是候选人原话的权威视图，必须压过模型工具入参和历史记忆。
  ...
  // 模型显式入参只有能在当前候选人消息中找到证据时才允许覆盖。否则它很可能只是把
  // system prompt 里的 medium/llm 旧记忆重新包装成 candidateName/candidatePhone。
  const normalizedExplicit = normalize(explicitValue);
  if (
    normalizedExplicit &&
    (currentUserMessage === undefined ||
      normalizeEvidenceText(currentUserMessage).includes(normalizeEvidenceText(normalizedExplicit)))
  ) {
    ...
    tool({
      ...
      execute: async ({
        ...
        try {
          ...
          const highConfidencePreferences = context.highConfidenceFacts?.preferences ?? null;
          ...
          // 身份入参溯源：与记忆矛盾且原话无佐证的身份翻转判为模型代答，丢弃
          // （badcase 6a50827c：模型凭空传 candidateIsStudent），其余照旧采信。
          const normalizedIsStudentInput = normalizeCandidateIsStudentInput(candidateIsStudent);
          const isStudentFlipUnsupported =
            normalizedIsStudentInput !== null &&
            isStudentClaimUnsupportedFlip({
              claimed: normalizedIsStudentInput,
              messages: context.messages,
              sessionIsStudent: context.sessionFacts?.interview_info?.is_student,
              highConfidenceIsStudent: highConfidenceInfo?.is_student,
            });
          const vettedCandidateIsStudent = isStudentFlipUnsupported
            ? undefined
            : candidateIsStudent;
          if (isStudentFlipUnsupported) {
            logger.warn(
              `candidateIsStudent 与记忆矛盾且原话无佐证，判为模型代答已丢弃: claimed=${normalizedIsStudentInput}, jobId=${jobId}`,
            );
          }
          applyCandidateFieldOverride(
            knownFieldMap,
            '身份',
            vettedCandidateIsStudent,
            highConfidenceInfo?.is_student,
            normalizeCandidateIsStudentInput,
            context.currentUserMessage,
          );
          // 拒后改口核实（产品裁定 2026-07-15）：候选人自认学生、被告知岗位不要学生后
          // 改口"社会人士/已毕业"，首次改口不直接采信——须由 Agent 核实一次、候选人在
          // 核实问句后再次确认才生效。只防"学生→社会"造假方向。
          const identityFlip = resolveIdentityFlipAfterRejection(context.messages);
          const latestExplicitIdentityEvidence = findLatestExplicitIdentityEvidence(
            context.messages,
          );
          const latestExplicitIdentityRaw = latestExplicitIdentityEvidence?.identity ?? null;
          const flipVerificationPending =
            identityFlip.flipPendingVerification && latestExplicitIdentityRaw === '社会人士';
          const latestExplicitIdentity = flipVerificationPending ? null : latestExplicitIdentityRaw;
          if (latestExplicitIdentity) {
            knownFieldMap['身份'] = latestExplicitIdentity;
          }
          const normalizedKnownIdentity = knownFieldMap['身份'] ?? null;
          if (
            (studentIdentityMustBeExplicit || flipVerificationPending) &&
            (!normalizedKnownIdentity ||
              !hasExplicitIdentityEvidence(normalizedKnownIdentity, context.messages) ||
              flipVerificationPending)
          ) {
            delete knownFieldMap['身份'];
          }
          if (flipVerificationPending) {
            logger.warn(
              `学生被拒后改口社会人士，待核实不采信: jobId=${jobId}, chatId=${context.sessionId}`,
            );
          }
          applyCandidateFieldOverride(
            knownFieldMap,
          ...
          const summerWorkerIntent = resolveSummerWorkerIntent({
            highConfidenceLaborForm: highConfidencePreferences?.labor_form,
            sessionLaborForm: context.sessionFacts?.preferences?.labor_form,
            messages: context.messages,
          });
          ...
          const identityAskRounds =
            studentIdentityMustBeExplicit && checklist.missingFields.includes('身份')
              ...
              : null;
          // 拒后改口核实本身需要一次额外追问，升级阈值放宽到 3 次，避免核实问句
          // 刚发出就被转人工；其余场景维持 2 次上限。
          const identityAskEscalated =
            identityAskRounds !== null &&
            checklist.missingFields.length === 1 &&
            identityAskRounds.askCount >= (flipVerificationPending ? 3 : 2) &&
          ...
          return stripNullish({
              ...
              nextAction === 'ready_to_book'
                ...
                : nextAction === 'collect_fields'
                  ? identityAskEscalated
                    ...
                    : flipVerificationPending
                      ? '候选人在学生身份被拒后改口为社会人士，首次改口不能直接采信。请向候选人核实一次（示例："跟你确认下哈，你目前已经毕业了对吧？如果还在读也没关系，如实说就行，我帮你看接受学生的岗位"）；候选人再次明确确认后重新调用本工具。严禁暗示"改成社会人士就能报名"，严禁未经确认直接登记。'
            ...
            identityFieldGuard:
              (studentIdentityMustBeExplicit || flipVerificationPending) &&
              checklist.missingFields.includes('身份')
                ? identityAskEscalated
                  ? {
                  ...
                  : flipVerificationPending
                    ? {
                        ...
                        verifyIdentityFlip: true,
                        reason:
                          '候选人自认学生、被告知岗位不要学生后改口为社会人士。首次改口不采信：请核实一次（告知如实填写不影响推荐其它岗位），候选人再次明确确认后才继续。严禁暗示改身份能通过报名。',
                      }
                    : {
                        mustAskCandidate: true,
                        reason:
                          '本岗位把学生身份作为筛选题，候选人尚未明确回答。学历、年龄、“刚毕业”和历史推断均不得代填身份。',
                      }
                : undefined,
