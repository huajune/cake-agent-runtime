...
const TEMP_SUMMER_WORKER_FIELD = '是否暑假工';
const SUMMER_WORKER_SUSPECT_MIN_AGE = 18;
const SUMMER_WORKER_SUSPECT_MAX_AGE = 22;
...
- candidateLaborForm：候选人本轮明确表达的用工形式或对"是否暑假工"的回答，如"暑假工"、"不是暑假工"、"长期兼职"。**只能转述候选人原话，禁止代答**。暑期临时口径：18-22 岁且未明确不是暑假工时，非暑假工岗位必须先确认；候选人明确是暑假工时，只有岗位结构化字段明确为暑假工（兼职类型 partTimeJobType=暑假工）才可继续约面。**暑假工身份有粘性**：候选人一旦说过自己是暑假工，系统会按其原话与会话事实持续判定，省略本字段或反向传值都不会解除拦截；只有候选人自己明确改口"不是暑假工"才会解除。**严禁**为通过校验把暑假工候选人按"非暑假工/长期"登记或建议候选人隐瞒暑假工身份——如实告知暂无暑假工岗位并维护候选人
...
- sensitiveScreeningNotice：返回该字段时，说明本岗位筛选条件（含 remark/screeningChecks 原文）内嵌户籍/籍贯/民族/专业等敏感信息，必须严格按该提示执行——条件本身 🔒 仅供内部筛选，严禁透露给候选人
...
- **暑假工临时口径（2026 暑期临时规则）**：候选人明确要暑假工时，只有岗位结构化字段明确为暑假工（basicInfo.partTimeJobType=暑假工，或历史数据 basicInfo.laborForm=暑假工）才可继续约面；岗位未明确为暑假工时，默认不招暑假工。候选人年龄 18-22 岁且尚未明确是否暑假工时，先追问"是不是暑假工/能否长期做"，不得直接进入 booking。该规则后续暑期结束可删除。
...
const inputSchema = z.object({
  ...
  candidateLaborForm: z
    ...
    .optional()
    .describe(
      '候选人明确表达的用工形式，或对"是否暑假工"的回答。如 "暑假工"、"不是暑假工"、"长期兼职"、"全职"。用于暑期临时口径：18-22 岁候选人需先确认是否暑假工。',
    ),
...
const SUMMER_SELF_AFFIRM_PATTERN =
  /^(?:我?是)?暑假工(?:的|呀|啊|哦)?$|我是暑假工|(?:做|找|要|想做|想找)暑假工|(?:做|找|要|想做|想找)暑期工|^暑期工(?:的)?$/;
 ...
 * 3. 显式入参自证暑假工始终采信（与模型推进转化的动机相逆，可信）；
 * 4. 都无证据时才用显式入参/记忆的其余取值兜底。
 */
function resolveSummerWorkerIntent(params: {
  explicitLaborForm: unknown;
  highConfidenceLaborForm: unknown;
  sessionLaborForm: unknown;
  messages: unknown[];
}): SummerWorkerIntent | null {
  ...
  const explicit = normalizeSummerWorkerIntent(params.explicitLaborForm);
  const memory =
  ...
  if (
    conversational === 'summer_worker' ||
    memory === 'summer_worker' ||
    explicit === 'summer_worker'
  ) {
    return 'summer_worker';
  }
...
function buildTemporarySummerWorkerGuard(params: {
  ...
  summerWorkerIntent: SummerWorkerIntent | null;
}):
  | {
      status: 'needs_confirmation';
      field: typeof TEMP_SUMMER_WORKER_FIELD;
      reason: string;
    }
  | {
  ...
  const age = params.candidateAge;
  if (
    age !== null &&
    age >= SUMMER_WORKER_SUSPECT_MIN_AGE &&
    age <= SUMMER_WORKER_SUSPECT_MAX_AGE &&
    params.summerWorkerIntent !== 'not_summer_worker'
  ) {
    return {
      status: 'needs_confirmation',
      field: TEMP_SUMMER_WORKER_FIELD,
      reason: `候选人 ${age} 岁，落在疑似暑假工年龄段 ${SUMMER_WORKER_SUSPECT_MIN_AGE}-${SUMMER_WORKER_SUSPECT_MAX_AGE} 岁；当前岗位未明确标注为"暑假工"，需先确认候选人是否暑假工/能否长期做。`,
    };
  }
    ...
    tool({
      ...
      execute: async ({
        ...
        try {
          ...
          const candidateAgeForSummerGuard = parseCandidateAge(knownFieldMap['年龄'] ?? null);
          const summerWorkerIntent = resolveSummerWorkerIntent({
            explicitLaborForm: candidateLaborForm,
          ...
          const temporarySummerWorkerGuard = buildTemporarySummerWorkerGuard({
            candidateAge: candidateAgeForSummerGuard,
          ...
          const requiredFields = [
            ...
            ...collectLabelNames,
            ...(temporarySummerWorkerGuard?.status === 'needs_confirmation'
              ? [TEMP_SUMMER_WORKER_FIELD]
              : []),
          ];
