...
- **ageBoundary 字段存在时**：候选人年龄距岗位门槛在"差一点点"边界内（下限 ≥23 且 < 岗位下限，或 ≤ 岗位上限+2 岁）。**禁止**用年龄硬门槛直接劝退候选人；必须调 request_handoff(reasonCode="other", reason="年龄边界候选人需人工判断") 转人工，由招募经理决定是否申请破格登记。本字段 reason 已给出具体差值说明，可在 handoff 入参里直接复述
...
export const AGE_BOUNDARY_UPPER_TOLERANCE_YEARS = 2;
