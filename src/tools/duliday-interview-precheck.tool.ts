...
import { sanitizeLaborFormForDisplay } from '@memory/facts/labor-form';
import {
...
- candidateLaborForm：候选人本轮明确表达的用工形式或对"是否暑假工"的回答，如"暑假工"、"不是暑假工"、"长期兼职"。暑期临时口径：18-22 岁且未明确不是暑假工时，非暑假工岗位必须先确认；候选人明确是暑假工时，只有岗位 laborForm 明确为"暑假工"才可继续约面
...
- **暑假工临时口径（2026 暑期临时规则）**：候选人明确要暑假工时，只有岗位 basicInfo.laborForm 明确为"暑假工"才可继续约面；岗位未明确为暑假工时，默认不招暑假工。候选人年龄 18-22 岁且尚未明确是否暑假工时，先追问"是不是暑假工/能否长期做"，不得直接进入 booking。该规则后续暑期结束可删除。
...
function resolveSummerWorkerIntent(params: {
  ...
  return (
    normalizeSummerWorkerIntent(params.explicitLaborForm) ??
...
function buildTemporarySummerWorkerGuard(params: {
  ...
  | null {
  const jobLaborForm = sanitizeLaborFormForDisplay(
    typeof params.jobLaborForm === 'string' ? params.jobLaborForm : null,
  );
  const jobExplicitlySummerWorker = jobLaborForm === '暑假工';
  ...
  if (params.summerWorkerIntent === 'summer_worker') {
    return {
      status: 'blocked_non_summer_job',
      reason:
        '候选人已明确是暑假工，但当前岗位 laborForm 未明确标注为"暑假工"；按 2026 暑期临时口径，默认不招暑假工，禁止继续约面。',
      recommendedAction:
