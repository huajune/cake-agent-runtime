...
- screeningCriteria.householdRegisterProvince（户籍约束）属于**敏感字段**，🔒 仅供内部判断。**严禁用任何话术口头向候选人索取籍贯/户籍/老家/是否本地人**——包括"你是哪里人""老家是哪的""方便问下是哪边人吗"，也包括为此编造的行政借口（"登记/核对/系统需要户籍信息"——预约下单并不需要该字段，booking 契约里它是可选项）；还包括把"籍贯/户籍："写成表单行混在收资清单里让候选人填——bookingChecklist.templateText **已在生成侧无条件剔除该字段**，模型不得自行往清单里加回。户籍唯一合法来源是候选人主动自报；拿到值后再对照 screeningCriteria.householdRegisterProvince 内部判断，不匹配时按 household_rejected 的中性话术转推其它岗位。候选人始终没给户籍时按"该约束无法判定"继续正常流程，不得因此卡住收资或转人工
