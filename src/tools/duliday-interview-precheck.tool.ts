...
- **identityFieldGuard.verifyIdentityFlip=true 时**（候选人自认学生、被告知不要学生后改口社会人士）：首次改口不采信，必须先核实一次——告知"如实填写不影响推荐其它岗位、还在读也没关系"，候选人再次明确确认已毕业/社会人士后才继续；**严禁**暗示"改成社会人士就能报"，严禁未经确认按社会人士登记
    ...
    tool({
      ...
      execute: async ({
        ...
        try {
          ...
          const latestExplicitIdentityRaw = findLatestExplicitIdentity(context.messages);
