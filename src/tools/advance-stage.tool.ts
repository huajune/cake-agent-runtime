...
- reason：必须写清实际触发信号，说明当前阶段完成了哪些目标或候选人诉求为何需要切换阶段
...
const inputSchema = z.object({
  ...
  reason: z
    .string()
    .describe(
      '推进原因。必须写清实际触发信号，说明当前阶段完成了哪些目标或候选人诉求为何需要切换阶段',
    ),
});
 ...
 * - 阶段变迁的审计链在 logger 行与 agent_execution_events，不落进阶段状态
  ...
  return (context) => {
    return tool({
      ...
      execute: async ({ nextStage, reason }) => {
        ...
        // 阶段状态只落 currentStage：from/at/reason 三个字段曾一起落库号称
