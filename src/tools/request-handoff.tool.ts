...
9. system_blocked：precheck / booking 等工具返回结构性错误导致无法自助推进（如 precheck 持续 missingFields 卡住 booking、booking 返回 BOOKING_REJECTED 且非报名人数已满），属于系统/工具卡死而非候选人原因，需人工补录或修复。**报名人数超上限用 booking_capacity_full、拉群接口失败用 group_invite_failed，不要归入本码**
...
12. salary_admin_inquiry：候选人咨询针对个人的薪资/行政事务且岗位工具/字段没有答案——如工时未计入、几号发几月工资、考勤核算、开工作证明/收入证明、三方协议、合同/协议条款、签约主体、社保、试用期、银行卡异常不能本人收薪、税务/发薪主体等。第一次可先说一次"帮你确认下"；候选人再次追问同一主题或继续追问细节时，必须按本码调用本工具。不要第二次复读兜底，不要继续收资/预约，也不要凭常识编造合同或薪资口径
...
15. other：明显需人工介入、且确实不属于以上十四类的面试/入职阶段阻塞（真正的兜底，能归类就不要用 other）
