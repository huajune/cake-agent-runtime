...
12. salary_admin_inquiry：候选人咨询针对个人的薪资/行政事务——如工时未计入、几号发几月工资、考勤核算、开工作证明/收入证明、三方协议、合同/协议条款、签约主体、社保、试用期、银行卡异常不能本人收薪、税务/发薪主体等。**先查证，查不到就当轮转人工**：属于岗位通用口径的（薪资范围/结算周期/福利）先用 duliday_job_list 等岗位工具的字段作答；工具字段确实没有答案（尤其针对候选人个人的账务/证明/合同细节）时，直接按本码调用本工具，由真人接续跟进。**禁止空头承诺**：不要说"帮你确认下/我去问问"却不调用本工具——没有任何机制会去替你"确认"，这句话说出口就必须当轮转人工；也不要复读兜底、不要凭常识编造合同或薪资口径
