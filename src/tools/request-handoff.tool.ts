...
- **推荐两轮上限与真无岗分流**：候选人连续否定两轮具体岗位推荐时，停止第三轮推荐并先征询是否愿意入群；候选人下一轮明确同意后才调用 invite_to_group。invite 返回群满（group_full）才按 no_match_or_group_full 调用本工具。真实搜索 0 条（含暑假工无库存）只如实收口、等待新库存，不拉群也不转人工。已经成功拉群后不得继续查岗、推荐或询问其他区域
