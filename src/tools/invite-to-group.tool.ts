...
- [兼职群资源] 段已注明该城市无可用群时
- **候选人正在推进某个已匹配岗位的收资/约面/确认面试时** — 候选人已接受某岗位、正在回填资料、询问具体面试时间或报名方式时，说明当前有匹配在走报名流程，**禁止**此时拉群打断。本工具只承接"无岗维护"（场景 1/2）；"有岗推进"的拉群时点是报名成功那一刻，由系统自动完成，不需要你操作。应继续把这单约面收尾；只有确认该岗位无法继续（如失败/不符）且满足场景 1/2 时才考虑拉群
...
- industry（强烈建议传）：候选人的求职意向行业
  - 调用 invite_to_group 时，若候选人求职意向行业明确，必须传对应 industry 参数。
...
- inviteDelivery：拉群投递方式
  - "direct_add"（群<40人，已直接拉入）→ 告知候选人已加入，并带实际群名
  - "invite_card"（群>=40人，企微已自动发送入群邀请卡片）→ 告知邀请已发送，点击卡片即可入群
...
- **征询入群意愿**只在连续两轮推荐均不满意后使用：先承接候选人意向，**本轮不调本工具**；真实搜索 0 条不得提群
- 候选人对拉群提议回复"好/可以/嗯"等同意词后，**下一轮必须实调本工具**（场景 2）；提了拉群却一直不调 = 空头承诺，候选人看到没动静会立刻流失
- **宣称已加入或已发邀请****必须**本轮实调本工具且返回 success: true，或 booking 回执 groupInvite.success=true，否则严禁使用
...
function buildCityGateError(params: {
  ...
  return buildToolError({
    ...
    replyInstruction:
      '该城市在会话记忆和候选人原文里都找不到依据，不能据此拉群。请先中性询问候选人所在城市，得到明确回复后再调用本工具；本轮不要提群相关内容，也不要调用 request_handoff。' +
      unsavedImageHint,
...
function buildInviteTimingGateError(params: {
  ...
  return buildToolError({
    ...
    replyInstruction:
      ...
      '不要再次发起邀请、不要将邀请已发送说成候选人已入群。候选人主动问群相关问题时，告知邀请此前已发送，点击卡片即可入群；' +
...
function buildAlreadyInGroupResult(result: GroupInviteResult, city: string, industry?: string) {
  ...
  return {
    ...
    _replyInstruction:
      ...
      '候选人主动问群相关问题时，告知已在该群并带实际群名，其余情况不主动提及群。' +
  ...
  if (result.success) {
    ...
    return {
      ...
      _replyInstruction: isDirectAdd
        ? `候选人已被直接加入兼职岗位信息群"${groupName}"。回复时必须带实际群名，告知已加入，并说明群用于查看兼职岗位信息；这是兼职群，不是面试群，不得把腾讯会议链接或面试通知关联到这个群；不要输出任何群链接或二维码。${UNDELIVERED_PRELUDE_REMINDER}`
        : `企微已向候选人发送兼职岗位信息群"${groupName}"的邀请卡片。回复时必须带实际群名，告知邀请已发送、点击卡片即可入群，并说明群用于查看兼职岗位信息；这是兼职群，不是面试群，不得把腾讯会议链接或面试通知关联到这个群；禁止输出、编造或粘贴任何 work.weixin.qq.com 群链接 / URL。${UNDELIVERED_PRELUDE_REMINDER}`,
    };
  }
