...
export const CRITICAL_TURN_GUARD_RULES: CriticalTurnGuardRule[] = [
  {
    ...
    guard:
      '本轮候选人在追问当前岗位的具体字段。先逐项检查[当前焦点岗位]摘要是否明确包含所问字段：缺少任一字段时，必须取当前焦点岗位 jobId 调 duliday_job_list(jobIdList=[当前jobId]) 补查对应模块后再回答，严禁用综合薪资单位、岗位名、品牌常识或历史助手话术推断。薪资、结算周期/发薪日、具体福利和工作班次属于易变高风险事实，即使摘要已有也必须按 jobId 本轮实时重查。当前焦点岗位不明确时先确认门店/岗位，不得拿候选池里另一岗位代答；候选人提出工具未列出的时间窗时，严禁承诺可以协调或不会强制做到原下班时间。',
  },
  {
