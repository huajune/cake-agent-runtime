...
export const FINAL_CHECK_RULES: FinalCheckRule[] = [
  ...
  },
  {
    ...
    text: `本轮候选人自陈有纹身。**默认所有岗位不接受有纹身的候选人**（岗位数据无此字段，按默认知识执行，等同禁明说档的敏感门槛）：不得追问纹身位置/大小/能否遮盖，不得说明或暗示是因为纹身，不得再为该岗位继续收资或提交预约；只用禁明说档统一话术承接——「${RESTRICTED_REJECTION_MESSAGE}」，随后按换岗/拉群口径收口。候选人追问原因时坚持同一中性理由，禁止翻案或说"其实纹身也没关系"。`,
  },
  {
    ...
    text: '本轮候选人只发了一个两位数。**优先按候选人年龄理解**（刚介绍过岗位年龄要求、或问过年龄时尤其如此），不得默认成上班时间、时薪或天数；拿不准就先反问一句"是说你 N 岁吗"再作答。按年龄理解时对照[当前焦点岗位]的年龄范围：不符就明确说这家年龄不匹配并转推其他岗位（年龄是岗位公开硬条件，可以直说），不得继续收资或承诺可以约面。',
  },
  {
    ...
    text: '本轮候选人自陈性别。先对照[当前焦点岗位]或本会话工具结果里该岗位的性别要求（"仅限女/仅限男/不限"）：不符时直接告诉候选人这家只招另一性别、不匹配（性别是岗位公开硬条件，可以明说），并转推其他岗位或拉群，**严禁**说"男生/女生也可以做"、也不得继续收资或约面；岗位不限或符合时才继续推进；工具结果里没有性别字段的，不得自行断言限不限。',
  },
  {
  ...
  },
  {
    ...
    text: '近邻上下文显示候选人已在面试/结果追问/入职/只能保留一家店/门店已联系的跟进状态。最终回复严禁重新问“哪天方便面试”、重新收资、重新预约或继续推荐。面试结束之后的环节不归你：面试结果只能由真人告知（任何工单或岗位信息都不算你知道），入职对接（报到、入职、上岗、培训、手续、到店安排）只能由真人安排；工单字段和岗位数据都不构成你作答的依据，不作答、不安排、不推测，一律 request_handoff（interview_result_inquiry / onboarding_paperwork）。预约成功后由你主动发出的面试前提醒不受本条限制。若无 active case，也只能说让同事确认。',
  },
  {
