...
function formatContactNamePreferenceHint(
  ...
  return (
    ...
    `- 例外（以候选人为准）：候选人本轮主动指定了别的品牌、明确拒绝这个品牌、或要求扩大品牌范围时跟随候选人；带该品牌召回为空时再放宽到不限品牌。\n` +
    `- 品牌名本身含城市词不代表候选人所在城市，不要仅凭品牌名推断城市。\n` +
...
function formatRealtimeGroups(groups: RealtimeGroupStatus[]): string {
  ...
  return (
    ...
    `也不要承诺再次拉群；候选人问群相关问题时，直接按实际群名说明已在群内。_\n` +
    lines.join('\n')
  );
}
...
const BOOKING_SYNCING_BLOCK = [
  ...
  '候选人询问该预约、要求改约或取消时，只能自然说明正在确认最新预约信息、请其稍等；不得提及工单、海绵、缓存或系统同步。',
].join('\n');
...
function formatProfile(profile: UserProfileFacts | null): string {
  ...
  return (
    ...
    `- 预填报名表/提交预约前，必须说明信息来自此前登记并逐项请候选人确认，不得不加说明地当作候选人本次刚提供的信息直接使用；\n` +
    `- 向候选人复述这些信息时，说明是此前提供的内容并确认现在是否仍然适用，不得说成候选人本次已确认；\n` +
...
function formatSessionFacts(
  ...
  if (state.invitedGroups?.length) {
    ...
    sections.push(
      `## 本会话已邀入的兼职群（禁止重复拉群）\n${groupLines.join('\n')}\n\n_命中以上任一群时，禁止再次调用 invite_to_group；候选人本轮再次同意入群/暗示想进群时，按实际群名说明此前已邀请，并提醒查看手机微信即可。_`,
    );
  }
