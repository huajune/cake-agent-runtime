    ...
    if (interview.is_student !== null && interview.is_student !== undefined) {
      lines.push(
        interview.is_student
          ? '- 是否学生: 是（学生能否安排只看岗位数据；建议开 includeHiringRequirement 或 duliday_interview_precheck 核对。明确"不接受学生"则停止，明确接受/学生优先则继续；未写学生限制或未返回学生筛选项时按没有额外学生硬限制继续其余校验，不得凭空增加门店确认或人工介入）'
