...
export const FOLLOW_UP_SCENARIOS: readonly FollowUpScenario[] = [
  ...
  },
  {
    ...
    // 发送前的语义停止条件（已提醒 / 已问结果 / 聊天时间与工单不一致等）只住
    // reengagement.agent.ts 的场景 prompt（台账第八节 RE1–RE4），这里只写生成目标与形式要求。
    generationPolicy:
      '必须按状态摘要里的面试形式生成：AI 面试说明无需到店，提醒按面试通知的入口和要求在线完成，不提门店、到店或携带证件；线下面试才提醒时间、地点和已有证据中的证件；仅写“线上面试”但未明确 AI 时，不得说成 AI 面试。提醒里的具体钟点一律用工单面试时间。若状态摘要明确写着“工单未提供，不得猜测”，只能中性提醒候选人按面试通知中的时间和要求参加，不得提线上、线下、到店、地址、入口、材料或证件。使用中性表达，不索取新资料',
    relevantFactLabels: [],
    defaultRolloutEnabled: true,
  ...
  },
  {
    ...
    // 发送前的语义停止条件（已问结果 / 未开始 / 聊天时间与工单不一致等）只住
    // reengagement.agent.ts 的场景 prompt（台账第八节 RE1–RE4），这里只写生成目标与形式要求。
    generationPolicy:
