...
export const FOLLOW_UP_SCENARIOS: readonly FollowUpScenario[] = [
  ...
  },
  {
    ...
    generationPolicy:
      '仅在当前工单仍进行中，且近期对话没有取消、不参加、已有面试结果、面试时间之后已询问面试结果，或招募经理未在面试当天另行发过本次面试提醒时生成（改时间、通知面试时间、问 AI 面试做完没、发二维码、发地址都不算已提醒）。必须按状态摘要里的面试形式生成：AI 面试说明无需到店，提醒按面试通知的入口和要求在线完成，不提门店、到店或携带证件；线下面试才提醒时间、地点和已有证据中的证件；仅写“线上面试”但未明确 AI 时，不得说成 AI 面试。提醒里的具体钟点一律用工单面试时间；近期对话中明确约定了与工单不同的面试时间时不生成，交运营修正工单。若状态摘要明确写着“工单未提供，不得猜测”，只能中性提醒候选人按面试通知中的时间和要求参加，不得提线上、线下、到店、地址、入口、材料或证件。使用中性表达，不索取新资料',
    relevantFactLabels: [],
    defaultRolloutEnabled: true,
  ...
  },
  {
    ...
    generationPolicy:
      '仅在当前工单仍进行中，且近期对话没有取消、不参加、已有面试结果，或招募经理在面试时间之后已经询问过本次面试结果时生成；此前只发过面试提醒不影响回访，面试时间之前的“做完了吗”“到店了吗”不算已询问结果。面试是否已开始按工单面试时间判断；近期对话中明确约定了与工单不同的面试时间时不生成，交运营修正工单；对话说还没开始但没给新时间的也不生成。按状态摘要里的面试形式询问：AI 面试询问是否已经完成、是否遇到问题；其他面试询问是否顺利、体验如何、是否需要协助。不要直接断言候选人已完成面试，不施压入职',
    relevantFactLabels: [],
    defaultRolloutEnabled: false,
  },
  {
