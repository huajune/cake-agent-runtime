...
export class ReengagementAnchorService {
  ...
  private async loadState(context: AnchorContext): Promise<ReengagementState> {
    const session = this.session as SessionSemanticService & {
      getReengagementState?: SessionSemanticService['getReengagementState'];
    };
  ...
  private asksForLocation(reply: string): boolean {
    // 必须按句切分后逐句匹配：整段匹配时 `发.{0,12}定位` 会把“刚发错了，确实是四海城。
    // 面试定位我发你了”跨句拼接成“索要定位”，给面试流程中的候选人错误排缺定位触达。
    return reply.split(/[。！？!?；;\n]+/).some((sentence) => {
      // “定位/位置…发你（了）”或 Agent 自述“刚发错了”是给候选人发送位置，不是索要。
      // 不能用裸 `发错` 排除整句，否则“别发错了，把定位发我”这类真实索要也会漏排程。
      if (
        /(?:定位|位置|地址|地标)[^，,]{0,6}发(?:你|您|过去)|(?:我|这边|刚才|刚)(?:不小心)?发错(?:了)?/.test(
          sentence,
        )
      )
        return false;
