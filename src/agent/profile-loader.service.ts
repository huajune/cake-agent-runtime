...
import { AgentProfile } from '@shared-types/agent.types';
...
export class ProfileLoaderService implements OnModuleInit {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    // 开发模式: src/agent/ -> profiles
    // 生产模式: dist/agent/ -> profiles (需确保 profiles 目录被复制到 dist)
  ...
  private async buildCandidateConsultationProfile(): Promise<AgentProfile> {
    ...
    // 工具列表由 ToolRegistryService 统一管理，此处声明本场景相关的工具
    const allowedTools = [
      'duliday_job_list',
