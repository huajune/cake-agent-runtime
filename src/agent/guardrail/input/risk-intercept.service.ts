 ...
 * 候选人主动要求转人工的高置信短语。
 *
 * 缺这一档时，礼貌说"转人工"得不到任何确定性响应，候选人升级为辱骂才命中 abuse 拦截
 * ——等于教会候选人"骂人才能叫来真人"。
 *
...
/**
 * 残障身份主动披露（产品+运营裁定）：候选人主动披露残障身份或询问残障者能否应聘时，
 * 确定性静默转人工，由真人判断如何沟通；Agent 不得输出任何自动话术——"委婉拒绝"属残障
 * 就业歧视（《残疾人保障法》），绝不自动化，也不建岗位侧筛选字段。
 *
 * 边界（防误伤 + 防画像）：
...
const DISABILITY_DISCLOSURE_PATTERNS: readonly RegExp[] = [
  ...
  // 疑问句式（动词后置）：残疾人招吗 / 精神残疾招的？/ 听障收不。上一档的动词表只有
  // 能/可以/要不要/收不收/招不招，盖不到这些句式，Agent 会直答"招的"继续推岗。
  new RegExp(
