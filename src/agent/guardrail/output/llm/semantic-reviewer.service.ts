...
// —— 截断检测 ————————————————————————————————————————————————
// 模型在 JSON 字符串中输出未转义双引号时，约束解码可能生成「结构合法、
// 内容却断在半句」的 finding。schema 校验无法发现这类截断，必须按悬垂结尾识别。
 ...
 * 漏掉一类就会把该类证据支撑的合法回复误判成"凭空生成"。
 * 因此按字段清单遍历并做编译期双向穷尽断言：evidence 加新字段而清单没跟上，
 * 或清单写了不存在的字段，都会直接编译报错。
 ...
 *
 * 这是**审查触发器**，不是判据：它刻意比硬规则的正则宽，用来覆盖正则难以穷举的
 * 敏感筛选披露和反向探询形态。
 *
 * 刻意不收 `专业`：它在正常话术里太常见（"我们很专业"），当触发器会把大量无关回合
 * 拖进 LLM 审查；该轴由硬规则的反问式/排除式模式覆盖。
 */
...
export class SemanticReviewerService {
   ...
   * 结构上看不见"没有任何证据却凭空生成事实"。因此零证据回复只要给出薪资、距离、
   * 班次、具体门店岗位、完成态报名或外链等可核验事实，就必须进入审查。
   *
