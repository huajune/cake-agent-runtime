...
// ⚠️ 只拦"正在/已经"的假宣称；将来时承诺（"稍后/这就/马上再帮你提交"）是合法承接
// 话术，不是对账对象（用户裁定，与拉群守卫"只拦完成时态"同口径）。
...
/** 如实披露失败：只认本轮没有提交成功的事实。裸「没约上」也是如实披露——
 * 不补这形态时它反被 CONFIRMATION_PATTERN 的「约上」误当成功宣称拦下。 */
const BOOKING_FAILURE_ACKNOWLEDGED_PATTERN =
  /(?:提交|报名|预约|约)[^。！？\n]{0,10}(?:失败|没(?:有)?成功|未成功|不成功|没约上|没成|出了点问题|有点问题)|(?:失败|没能|未能|没有)[^。！？\n]{0,8}(?:提交|报名|预约|约上)|(?:没|未)(?:有|能)?(?:提交|报名|预约)成功|没(?:有|能)?约上/u;
...
/** 把 `15:00` / `2026/08/06 15:00` 里的钟点归一成分钟数，用于比对。 */
