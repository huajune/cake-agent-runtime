...
export function isHandoffPromiseOnlyReply(content: string): boolean {
  ...
  const residue = text
    ...
    .filter(Boolean)
    .filter((sentence) => !HANDOFF_PROMISE_PATTERNS.some((pattern) => pattern.test(sentence)))
    .join('\n');
...
export function detectHandoffPromiseWithoutHandoff(
  ...
  return {
    ...
    label:
      '回复承诺已让同事/负责人后续确认或联系候选人，但本轮没有成功的人工升级动作（request_handoff / raise_risk_alert / 预约失败自动暂停），属于无真实动作支撑的跟进承诺',
