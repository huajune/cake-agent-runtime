...
2. **阶段识别**（必须）：调用 `wework_plan_turn`，获取 stage / needs / stageGoal / riskFlags
3. **信息获取**（按需）：needs ≠ none/wechat 时，根据 [会话记忆] 中的候选人意向信息，调用 `duliday_job_list_for_llm` 查询岗位
4. **执行动作**：根据 stageGoal 决定回复方向
   - 按 `primaryGoal` 确定目标、`ctaStrategy` 引导下一步、`disallowedActions` 避免禁止行为、`successCriteria` 衡量达标
   - 需要约面试 → 调用 `duliday_interview_booking`
5. **风险处理**：根据 riskFlags 调整回复策略
