...
### 更新摘要
- PR #1350 不合作品牌直说、编造品牌不进重查、排班约束须有原话依据
- PR #1350 班次出处语料按需抽取
- PR #1350 不合作品牌直说、编造品牌不进重查、排班约束须有原话依据（运营 0924 case 复核）
...
### 运维与流程
- PR #1350 未改 `schedule-semantic.util.ts` 的早/中/晚班判定语义，未新增班次正则——运营裁定「班次相关需要重新裁定」，本次只做「没依据不许筛」这一层。
- PR #1350 `findUnmentionedQueryBrands` 按设计只校验目录内品牌，目录外品牌绕过出处守卫是既有设计，未改守卫语义，从回执与话术侧收口。
- PR #1350 出站守卫 `unsupported_store_status_speculation` 同步接受新的 `nextAction=brand_not_partnered`，不新增 ruleId。
- PR #1350 不合作品牌直说、编造品牌不进重查、排班约束须有原话依据
