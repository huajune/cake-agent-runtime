...
- PR #1134 收拢 MEMORY_SESSION_TTL_DAYS 默认值到共享常量消除漂移
...
- PR #1134 `tests/memory/memory.config.spec.ts` + `tests/biz/message/services/chat-session.service.spec.ts`：26 通过
- PR #1134 `pnpm run typecheck` / `pnpm run lint:check`：通过
<!-- release:pending:end -->
