...
- PR #1095 预约提交切换网关新 entryUser 接口 + labelList 线格式对齐
...
### 验证记录
- PR #1095 真实 case（0827 失败单）三发实测：旧格式复现字段判空 → `options` 格式通过校验被业务查重正确拦截（无重复工单产生）
- PR #1095 `sponge.service.spec` + `duliday-interview-booking.tool.spec` 43 例全绿；typecheck / lint 通过
- PR #1095 未实测 `code:0` 成功路径的响应结构（该 case 已被运营手工报名），schema 失配会安全降级为失败回执不会假成功；上线后盯第一笔成功单
<!-- release:pending:end -->
