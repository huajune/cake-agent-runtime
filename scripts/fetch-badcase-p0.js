#!/usr/bin/env node
/* 一次性脚本：从飞书 BadCase 多维表拉取最新 P0 未解决数据并落到 tmp/。 */
const fs = require('fs');
...
(async () => {
  ...
  const p0Unresolved = p0.filter((r) => r.status && r.status !== '已解决');

  const outAll = path.resolve(__dirname, '..', 'tmp', `badcase-p0-source-${todayStr}.json`);
  const outUnresolved = path.resolve(
    __dirname,
    '..',
    'tmp',
    `badcase-p0-unresolved-source-${todayStr}.json`,
  );

  fs.writeFileSync(
    outAll,
    JSON.stringify(
      { generatedAt: new Date().toISOString(), total: p0.length, records: p0 },
      null,
      2,
    ),
  );
    ...
    JSON.stringify(
      {
        ...
        total: p0Unresolved.length,
        records: p0Unresolved,
      },
      null,
      2,
    ),
  ...
  console.log(`P0 total: ${p0.length}`);
  console.log(`P0 unresolved: ${p0Unresolved.length}`);
  console.log('P0 status:', byStatus);
  console.log('P0 category:', byCategory);
