...
const _ARCHIVED_PLANS: Plan[] = [
  ...
  // === 2026-05-11 P1 数据探测后业务事实非缺陷判定 ===
  // 9dfyg8ef：候选人坚持"金汇龙湖"和"奉贤龙湖"是两家店，Agent 答"是同一个地方"
  // 实测 sponge 数据：在 56 条上海六姐 job 里搜"龙湖|金汇|奉贤|金海公路"，
  // storeId 只有 2 个不同的 store：307376（西渡连城店）与 309358（奉贤龙湖店，金海公路 3800 号龙湖天街）。
  // 金海公路 3800 号龙湖天街内**确实只有 1 家六姐门店**（即奉贤龙湖店），候选人混淆了商圈名/品牌名。
  // Agent 答案正确，属业务事实非缺陷。
  {
    badcaseId: '9dfyg8ef',
    recordId: 'recvj69lTkEFE1',
    current: '待分析',
    target: '已解决',
    reason:
      '业务事实非缺陷：实测 sponge 数据，金海公路 3800 号龙湖天街只有 1 家六姐门店（奉贤龙湖店 storeId=309358），并不存在"金汇龙湖店"——候选人混淆了商圈名。Agent 答"是同一个地方"在数据上正确',
  },
];
