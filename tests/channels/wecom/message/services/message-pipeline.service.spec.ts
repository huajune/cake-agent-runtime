...
import { FilterReason } from '@wecom/message/services/message-filter.service';
...
describe('MessagePipelineService', () => {
  ...
  const successAgentResult = {
    result: {
      rawHttpResponse: { status: 200, statusText: 'OK', headers: {} },
  ...
  describe('processSingleMessage', () => {
    ...
    it('should send fallback alert when agent returns fallback response', async () => {
      const fallbackAgentResult = {
        ...
        isFallback: true,
        result: {
          ...successAgentResult.result,
          fallbackInfo: { reason: 'Context too long' },
        },
      };
