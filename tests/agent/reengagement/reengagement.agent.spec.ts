...
describe('ReengagementAgent', () => {
  ...
  it('generates a structured message with assembled context for interview reminders', async () => {
    ...
    expect(llm.generateStructured.mock.calls[0][0].system).toContain('# 候选人证据材料');
  ...
  it('uses LLM for booking_incomplete instead of hard-coding missing fields', async () => {
    ...
    expect(llm.generateStructured.mock.calls[0][0].messages).toEqual([]);
    expect(llm.generateStructured.mock.calls[0][0].system).toContain('"collectedFields"');
    expect(llm.generateStructured.mock.calls[0][0].system).toContain('招聘顾问');
    // 内部类名不应作为人设泄漏进 prompt
    expect(llm.generateStructured.mock.calls[0][0].system).not.toContain('ReengagementAgent');
    expect(llm.generateStructured.mock.calls[0][0].system).toContain('booking_incomplete');
  ...
  it('injects processed memory (fact lines + cleaned recent messages) and logs it to the agent request', async () => {
    ...
    expect(request.system).toContain('address_missing');
    expect(request.system).toContain('# 本次需要完成的任务');
    expect(request.system).toContain('任务代码：address_missing');
