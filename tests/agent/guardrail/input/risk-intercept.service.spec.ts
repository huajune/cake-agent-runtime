...
describe('RiskInterceptService', () => {
  ...
  it('returns conversation_risk sideEffect intent when high-confidence input risk hits', async () => {
    await expect(service.evaluate(baseInput({ scanContent: '你们这帮人真是垃圾' }))).resolves.toEqual(
  ...
  it('does not execute side effects from precheck compatibility method', async () => {
    await expect(service.precheck(baseInput({ scanContent: '你们这帮人真是垃圾' }))).resolves.toEqual({
      hit: true,
      riskType: 'abuse',
      reason: expect.stringContaining('垃圾'),
      label: '辱骂/攻击',
  ...
  it('detects complaint risk keywords', async () => {
    await expect(service.precheck(baseInput({ scanContent: '你们是不是骗子，我要投诉' }))).resolves.toEqual(
      {
        hit: true,
  ...
  it('does NOT flag ordinary rolling words as abuse', async () => {
    await expect(service.precheck(baseInput({ scanContent: '麻烦把页面滚动一下，我看不到下面' }))).resolves.toEqual(
      { hit: false },
    );
  });

  it('detects interview result inquiry as a high-confidence input risk', async () => {
    await expect(service.precheck(baseInput({ scanContent: '上次面试结果怎么样' }))).resolves.toEqual(
      {
        hit: true,
