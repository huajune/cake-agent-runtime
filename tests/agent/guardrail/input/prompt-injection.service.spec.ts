...
describe('PromptInjectionService', () => {
  ...
  beforeEach(async () => {
    ...
    mockAlertService = {
      ...
      createPromptInjectionAlert: jest.fn((params) => ({
        ...
        source: {
          subsystem: 'security',
          component: 'PromptInjectionService',
  ...
  describe('GUARD_SUFFIX', () => {
    it('should be defined as a non-empty string', () => {
      expect(typeof PromptInjectionService.GUARD_SUFFIX).toBe('string');
      expect(PromptInjectionService.GUARD_SUFFIX.length).toBeGreaterThan(0);
    });

    it('should contain a security warning hint', () => {
      expect(PromptInjectionService.GUARD_SUFFIX).toMatch(/安全提示/);
    });
  });
});
