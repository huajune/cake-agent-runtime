...
jest.mock('ai', () => ({
  ...
  Output: {
    object: jest.fn().mockReturnValue({ type: 'object' }),
  },
  tool: jest.fn().mockImplementation((def) => def),
}));

describe('WeworkPlanTurnToolService', () => {
  let service: WeworkPlanTurnToolService;
  let mockRouter: { resolveByRole: jest.Mock };
  ...
  describe('buildTool', () => {
    ...
    it('should call generateText via RouterService on execute', async () => {
      mockGenerateText.mockResolvedValue({ output: basePlan });
    ...
    it('should return stageGoal from config', async () => {
      ...
      mockGenerateText.mockResolvedValue({ output: plan });
