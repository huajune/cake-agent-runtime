...
describe('社保族无适配器（2026-09-08 拆除，bitter-lessons L11）', () => {
  it('「社保缴纳情况」走通用道：模型作证给契约选项原文，代码只做逐字词表核验', () => {
    expect(routeOf(SOCIAL_INSURANCE_FIELD)).toBe('generic');
    expect(adapterFor(SOCIAL_INSURANCE_FIELD)).toBe(genericAdapter);
  });

  it('通用道逐字直配契约选项原文；自然语言短答（无/公司交）不产确定性提案，留给模型与交互', () => {
    expect(
      genericAdapter({
        field: SOCIAL_INSURANCE_FIELD,
        candidateText: '无公司在缴社保流水',
        answerBound: true,
      }),
    ).toMatchObject({ labelId: 12, value: '无公司在缴社保流水', optionCodes: ['2'] });
    for (const text of ['无', '公司交', '公司缴纳,佛山本地', '个人灵活社保']) {
      expect(
        genericAdapter({ field: SOCIAL_INSURANCE_FIELD, candidateText: text, answerBound: true }),
      ).toBeNull();
    }
  });
});
