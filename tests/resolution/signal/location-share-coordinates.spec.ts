...
describe('parseLocationShareCoordinates（定位分享坐标解析共享工具）', () => {
  it('解析标准定位分享（lat,lng 序，badcase 6a6846e2 南海万达）', () => {
    const coords = parseLocationShareCoordinates([
  ...
  it('引用块内的定位不算候选人自己的位置', () => {
    expect(
      parseLocationShareCoordinates(['[引用 经理：[位置分享] 门店位置 [经纬度:31.1,121.1]]好的']),
    ).toBeNull();
  });

  it('无 [位置分享] 标记的坐标残片不解析（岗位地址里的坐标不算）', () => {
    expect(parseLocationShareCoordinates(['门店坐标 [经纬度:31.1,121.1]'])).toBeNull();
  });

  it('空输入 / 非法数字返回 null', () => {
    expect(parseLocationShareCoordinates([])).toBeNull();
    expect(parseLocationShareCoordinates(['[位置分享] X [经纬度:abc,121.1]'])).toBeNull();
  });
});
