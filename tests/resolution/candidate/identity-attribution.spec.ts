...
describe('candidate identity attribution', () => {
  ...
  describe('确认式提案的真实问答对绑定', () => {
    ...
    it('真实 assistant 问句 + 紧邻候选人肯定短答才成立', () => {
      ...
      expect(isAssistantQuestionConfirmedInDialogue(question, '确认', messages)).toBe(true);
    });

    it('模型自报的问句未出现在 assistant 历史时拒绝', () => {
      expect(
        isAssistantQuestionConfirmedInDialogue(question, '确认', [message('user', '确认')]),
      ).toBe(false);
    });
    ...
    it('只认问句后的第一条 user，应答被其他消息隔开时拒绝', () => {
      ...
      expect(isAssistantQuestionConfirmedInDialogue(question, '确认', messages)).toBe(false);
    });
  });
});
