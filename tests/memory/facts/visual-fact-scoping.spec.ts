...
describe('规则轨 · sheet 授权域', () => {
  ...
  it('R1e：岗位卡薪资不再进 pref.salary（vkikct39 同案 6000元/月 污染）', () => {
    const withSheet = extractHighConfidenceFacts([JOB_DESC], [], {
    ...
    const withoutSheet = extractHighConfidenceFacts([JOB_DESC], []);
    expect(withoutSheet?.preferences.salary?.value).toBe('6000元/月');
  ...
  it('R1e：岗位卡门店城市不进 pref.city（x3pdj7qh 族）', () => {
    ...
    const facts = extractHighConfidenceFacts([desc], [], {
  ...
  it('B3：简历 sheet 身份可提但 phone 不经规则轨落 high', () => {
    ...
    expect(facts?.interview_info.name?.value).toBe('李耀海');
  ...
  it('certificate sheet：证件属自陈材料身份域开；无 sheet 时文本标记认不出证件、身份域关', () => {
    ...
    expect(withSheet?.interview_info.name?.value).toBe('毛梦港');
